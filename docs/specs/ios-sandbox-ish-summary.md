...
| Upstream Fork | `Solos/ish-arm64`, branch `feature-arm64` |
...
- Pre-creates `/var/solos/` subdirectories
...
- Automatic session-based mount/remount of `/var/solos/` directories
...
### 3.2 Per-Session Mounts (`/var/solos/`)

Each agent session mounts its own persistent directories:

| Guest Path | Host Path | Purpose |
|---|---|---|
| `/var/solos/attachments/` | `Library/SolosChat/solos/{sessionId}/attachments/` | Input files for commands |
| `/var/solos/offloads/` | `Library/SolosChat/solos/{sessionId}/offloads/` | Output from native offloads |
| `/var/solos/workspace/` | `Library/SolosChat/solos/{sessionId}/workspace/` | Session working directory |
| `/var/solos/browser/` | `Library/SolosChat/solos/{sessionId}/browser/` | Web browsing files |

### 3.3 Global Mounts

| Guest Path | Host Path | Purpose |
|---|---|---|
| `/var/solos/memory/` | `Library/SolosChat/solos/memory/` | Shared memory across sessions |
| `/var/solos/skills/` | `Library/SolosChat/solos/skills/` | Stored skill definitions |

### 3.4 DNS Mount

| Guest Path | Host Path |
|---|---|
| `/etc/resolv.conf` | `Library/SolosChat/dns/resolv.conf` |
...
- **Host → Guest**: Bind mounts make host files appear at `/var/solos/...`
- **solos:// URL scheme**: Resolved by `SolosImageProvider` to local images
...
- Guest stub creation
- Host path resolution (`/var/solos/...` → host path)
- stdin reading
...
│  ├─ Mount session dirs to /var/solos/               │
...
- Bind mounts are explicit and scoped to `/var/solos/`
