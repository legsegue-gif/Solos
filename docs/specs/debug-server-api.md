...
SolosApp includes a built-in JSON-RPC 2.0 debug server (DEBUG builds only) for runtime view inspection.
...
curl -s http://localhost:8321 -d '{
  "jsonrpc": "2.0", "id": 6,
  "method": "debug.ls",
  "params": { "path": "/var/solos", "recursive": true, "maxDepth": 2 }
}' | python3 -m json.tool
```
...
Toggle whether a specific model entry is exposed to the in-shell `solos-model-use` agent. Equivalent to the "Available in Agent Loop" toggle in the model entry detail screen — agent-loop visibility is what determines if the in-shell tool can list and invoke a model.
...
Toggle whether a model group is exposed to the in-shell `solos-model-use` agent (parallel to `provider.models.setAgentLoop` but for groups).
...
These methods provide remote access to the app's file-based logging system (`LoggingManager`). Logs are daily-rotated files stored in `Library/Logs/` with the naming pattern `solos-YYYY-MM-dd.log`.
...
```json
{
  ...
  "totalSize": 524288,
  "files": [
    { "name": "solos-2026-03-07.log", "size": 102400, "modified": "2026-03-07T14:30:00Z" },
    { "name": "solos-2026-03-06.log", "size": 421888, "modified": "2026-03-06T23:59:59Z" }
  ]
}
```
...
| Name     | Type     | Default  | Description                                    |
|----------|----------|----------|------------------------------------------------|
| `name`   | `string` | —        | **Required.** Log filename (e.g. `"solos-2026-03-07.log"`) |
| `offset` | `int`    | —        | Byte offset to start reading from              |
...
curl -s http://localhost:8321 -d '{
  ...
  "params": { "name": "solos-2026-03-07.log" }
}' | python3 -m json.tool
...
curl -s http://localhost:8321 -d '{
  "jsonrpc": "2.0", "id": 3,
  "method": "debug.logs.read",
  "params": { "name": "solos-2026-03-06.log", "offset": 92400, "limit": 10240 }
}' | python3 -m json.tool
```
...
# 10. Read a log file
curl -s localhost:8321 -d '{"jsonrpc":"2.0","id":10,"method":"debug.logs.read","params":{"name":"solos-2026-03-07.log"}}' | python3 -m json.tool
