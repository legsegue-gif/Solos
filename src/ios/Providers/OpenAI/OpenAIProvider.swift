...
final class OpenAIProvider: LLMProvider {
    ...
    /// /images/generations JSON body, so `solos-model-use` can pass provider-specific
    ...
    /// solos-model-use bridge from the input JSON's explicit `extra_body`
    ...
    ///   • Venice.ai (`api.venice.ai`) — [Solos#86] resells deepseek /
    ...
    /// [Solos#163] Talking to xAI's own API (api.x.ai), as opposed to a
    ...
    /// including `/openai`, and Solos instructs them to put `?api-version=…` on
    /// the base. We:
    ...
    func sendMessage(
        ...
        //   • `solos-model-use run --model gpt-5.5 --input X.json`
    ...
    /// so plain `streamMessage` calls (solos-model-use, title gen, etc.)
