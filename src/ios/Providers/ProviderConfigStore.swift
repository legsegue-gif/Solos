...
struct ProviderConfig: Codable, Equatable {
    ...
    /// ModelEntry IDs for individual models available in agent loop (solos-model-use).
    var agentLoopModelEntryIds: [String]
    /// ModelGroup IDs whose members are available in agent loop (solos-model-use).
...
final class ProviderConfigStore: ObservableObject {
    ...
    nonisolated static var configFileURLForBackup: URL {
        ...
        return library.appendingPathComponent("SolosChat/provider-config.json")
    }
    ...
    /// a model the account cannot even call (Solos#83: the 400 renders as an
    ...
    /// [T-ios-solos-config-entry-id-composite] Normalize an entry reference in
    ...
    func applyMergedConfigFromSync(_ newConfig: ProviderConfig) {
        ...
        if let db {
            ...
            Task.detached {
                ...
                // selection on every merge (Solos#98 defect 2). The
...
enum ProviderKeychainHelper {
    ...
    static func saveAPIKey(_ key: String, instanceId: String, caller: String = #function) {
        let service = "com.solos.app.provider.\(instanceId)"
