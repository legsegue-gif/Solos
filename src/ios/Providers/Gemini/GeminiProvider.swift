...
final class GeminiProvider: LLMProvider {
    ...
    /// a thinking config.
    ///
    /// [T-gemini-tts-thinking-400 / Solos#226] Keyed off the DECLARED MODALITY rather
    ...
    private func buildRequestBody(
        ...
        // [Solos#226] Audio-output models reject systemInstruction outright. Dropping
        ...
        // Merge extra generation config (e.g. aspect ratio from solos-model-use)
    ...
    private func buildRequest(path: String, body: [String: Any]) async throws -> URLRequest {
        ...
        if useCloudCode {
            ...
            // (Solos/<marketing>) instead. Never overrides the Cloud Code UA above.
            request.setValue(SolosUserAgent.default, forHTTPHeaderField: "User-Agent")
        }
