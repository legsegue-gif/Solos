...
/// Used by both the agent loop (AIChatViewModel) and solos-model-use offload bridge.
@MainActor
enum LLMProviderFactory {
    ...
    static func applyCustomUserAgent(_ provider: OpenAIProvider, instance: ProviderInstance) -> OpenAIProvider {
        ...
        // marketing version (Solos/1.10 …) instead of URLSession's build-number
        // default (Solos/1 CFNetwork/… Darwin/…). Don't clobber a UA another
        // builder already set (e.g. some future provider-specific UA).
        if provider.extraHeaders["User-Agent"] == nil {
            provider.extraHeaders["User-Agent"] = SolosUserAgent.default
        }
        return provider
    }
    ...
    static func makeAnthropicProvider(instance: ProviderInstance, model: LLMModel) -> AnthropicProvider {
        ...
        case .apiKey:
            ...
            // No user custom UA → send the app default (Solos/<marketing>) so the
            ...
            return AnthropicProvider(apiKey: key, model: model, basePath: customBase, appendV1Suffix: appendV1, customUserAgent: ua ?? SolosUserAgent.default)
        case .oauth:
    ...
    static func makeOpenRouterProvider(instance: ProviderInstance, model: LLMModel) -> OpenAIProvider {
        ...
        provider.extraHeaders = [
            "HTTP-Referer": "https://github.com/legsegue-gif/Solos",
            "X-Title": "Solos App",
        ]
