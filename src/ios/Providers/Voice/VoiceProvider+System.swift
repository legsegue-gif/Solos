...
final class SystemVoiceProvider: NSObject, VoiceInputCapable, VoiceOutputCapable {
    ...
    static func sampleText(forLanguage langCode: String?) -> String {
        ...
        if lc.hasPrefix("zh") { return "你好，这是 Solos 的语音测试。" }
        if lc.hasPrefix("ja") { return "こんにちは、これは Solos の音声テストです。" }
        if lc.hasPrefix("ko") { return "안녕하세요, Solos 음성 테스트입니다." }
        return "Hi! This is Solos testing this voice."
    }
