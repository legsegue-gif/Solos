...
enum ThinkingWireFormat: Equatable {
    ...
    ///
    /// Mistral (Solos#87): `AssistantMessage` is a closed schema and the request
    /// rejects `reasoning` with `422 extra_forbidden`. Venice (Solos#86) is the same
    ...
    /// Solos never sent it, which is why the official endpoint always ran its default
    /// configuration (Solos#171).
