...
struct ThinkingResolveContext {
    ...
    /// [Solos#163] The catalog affirmatively declares this model has NO
    ...
    /// [Solos#163] Endpoint is xAI's own API (api.x.ai), not a relay that
...
/// Why a particular wire shape was chosen. Design §8 / Solos#100: the resolved
...
enum ThinkingRuleResolver {
    ...
    static func builtInRules(for ctx: ThinkingResolveContext) -> [ThinkingRule] {
        ...
        // Mistral — Solos#87 / 4592ca9b / 29065ca0. Total prohibition: the request
    ...
    private static func emit(
        ...
        case .reasoningEffort:
            ...
            // [Solos#163] xAI-scoped skip. grok-build-0.1 answers
        ...
        case .extraBodyToggle(let path):
            ...
            // because that is how users think about it (GH Solos#171: DeepSeek's real
            // switch is extra_body.thinking.enabled and Solos never sent it).
            setValue(true, at: path, in: &body, when: ctx.level.isEnabled, otherwiseWrite: false)
            return (nil, nil)
    ...
    ///               later in the chain would shadow it (Solos#226).
    ...
    static func geminiThinkingConfig(modelId: String, level: ThinkingLevel) -> [String: Any] {
        ...
        // [T-gemini-tts-thinking-400 / Solos#226] Specialized modalities take
        ...
        if id.contains("gemini-3") {
            ...
            // (verified via solos-model-use run), so 3.7+ Flash floors at "low" like
