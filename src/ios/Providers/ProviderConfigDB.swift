...
actor ProviderConfigDB {
    ...
    /// Default DB path: same SolosChat folder as `provider-config.json`.
    ...
    /// skip, or it will erase the user's agent-loop choice. (Solos#98.)
    ...
    init(url: URL? = nil) throws {
        ...
        // WAL mode: same as solos.db. Tolerates concurrent reads while we
    ...
    func bulkReplace(from config: ProviderConfig, preserveLocalOnlyStateIfEmpty: Bool = false) {
        ...
        // silently erased the user's agent-loop selection (Solos#98).
        //
    ...
    ///      Solos#98 and cost days to characterise.
    ...
    /// state on nil instead. (Solos#98 defect 2.)
