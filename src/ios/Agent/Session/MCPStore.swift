...
//
//  The server list lives in SolosConfig/mcp-servers/servers.json (mcpServers
//  object form, Claude-Desktop compatible), bind-mounted into the guest at
//  /var/solos/mcp-servers — the SAME file the in-guest `solos-mcp-cli`
...
struct MCPServerConfig: Codable, Identifiable, Hashable {
    ...
    /// first MCP `initialize` (Solos config, not MCP protocol). Enforcement
    /// lives entirely in the in-guest `solos-mcp-cli` daemon; the native layer
...
final class MCPStore: ObservableObject {
    ...
    /// Host path of the guest's /var/solos/mcp-servers/servers.json. Lives in
    /// the SolosConfig persistent dir (same mechanism as skills/shared/memory:
    ...
    /// ensureSolosSymlinks). NOT solosAppGroupRoot: servers.json carries
    ...
    nonisolated static var syncFileURL: URL {
        AIChatViewModel.solosMcpServersPersistentDir.appendingPathComponent("servers.json")
    }
    ...
    nonisolated private static var legacyServersFileURL: URL {
        FileManager.default.urls(for: .documentDirectory, in: .userDomainMask)[0]
            .appendingPathComponent("alpine-rootfs/data/var/solos/mcp-servers", isDirectory: true)
            .appendingPathComponent("servers.json")
    }

    /// One-time migration of servers.json from the rootfs data tree into the
    /// SolosConfig persistent dir. Runs before load() on every launch; no-op
    ...
    nonisolated private static func migrateLegacyServersFile() {
        ...
        do {
            ...
            // a symlink by ensureSolosSymlinks/performMount.
    ...
    private var dbPath: String {
        ...
        return library.appendingPathComponent("SolosChat/skills.db").path
    }
    ...
    // MARK: - servers.json (co-read/write with solos-mcp-cli)
    ...
    private struct ServerEntry: Codable {
        ...
        var args: [String]?
        var env: [String: String]?
        var startupTimeoutSeconds: Int?   // Solos STDIO startup timeout; round-tripped verbatim
    }
    ...
    func systemPromptSnippet(for sessionId: String) -> String? {
        ...
        var lines = "Available MCP Servers (use solos-mcp-cli to discover and call):\n"
        for s in selected {
            var note = s.note ?? ""
        ...
        lines += "\nTo use: run `solos-mcp-cli tools <server>` to see available tools,\n"
        lines += "then `solos-mcp-cli call <server> <tool> [args]` to invoke."
        // Agent-facing guidance on the runtime env placeholder; English-only,
        // not localized (this is prompt text, never shown in the UI).
        lines += "\nWhen adding or modifying an MCP server config (via solos-mcp-cli add / the UI), use $$VARNAME in env/headers/url values as a placeholder resolved at runtime from the system/App environment variables — do not hardcode secrets; reference an existing App environment variable as $$NAME."
        return lines
    }

    // MARK: - Tools refresh (native → in-guest CLI) [T-mcp-tools-refresh]

    /// One tool row from `solos-mcp-cli tools --refresh`.
    ...
    /// (`solos-mcp-cli refresh <name>`), so tools the remote added after the
    ...
    /// it initializes includes /var/solos/mcp-servers.
    func refreshTools(server: String) async throws -> [MCPToolInfo] {
        ...
        let result = try await ISHExecutionCoordinator.shared.execute(
            ...
            command: "solos-mcp-cli refresh \(quoted)",
            timeout: 120,
            lineCallback: { _ in },
            pidCallback: { _ in }
        )
    ...
    /// Detect edits that bypassed the CRUD paths (in-guest solos-mcp-cli
