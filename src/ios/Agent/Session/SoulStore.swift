...
/// [T-soul-icon-config-images] Turns whatever `solos-config set soul.icon`
...
enum SoulIconSource {
        ...
        var errorDescription: String? {
            ...
            case .outsideAllowedDirs(let p):
                return "\(p) is outside the directories this tool may read. "
                     + "Use a solos:// URL (e.g. solos://attachments/icon.png) "
                     + "or a path under the session's solos directories."
            case .badURL(let s):
    ...
    static func looksLikeImageSource(_ raw: String) -> Bool {
        ...
        if s.hasPrefix("solos://") { return true }
    ...
    static func resolveToDataURI(_ raw: String) async throws -> String {
        ...
        } else if s.hasPrefix("solos://") {
            data = try readSolosURL(s)
    ...
    /// `solos://` goes through the app's own resolver, which is what enforces
    /// session scoping — this must not reach into another session's files.
    private static func readSolosURL(_ s: String) throws -> Data {
    ...
    /// A raw filesystem path is accepted only inside the solos persistent
    ...
    /// the ACTIVE session only, matching `resolveSolosURL`'s isolation rule.
    private static func allowedRoots() -> [URL] {
        var roots: [URL] = [
            AIChatViewModel.solosSkillsPersistentDir,
            AIChatViewModel.solosMemoryPersistentDir,
            AIChatViewModel.solosSharedPersistentDir,
        ]
        if let sid = AIChatViewModel.activeSessionId {
            roots.append(AIChatViewModel.solosPersistentBase
...
enum SoulStore {
    ...
    static var fileURL: URL {
        AIChatViewModel.solosMemoryPersistentDir.appendingPathComponent("SOUL.md")
    }

    // MARK: - Body length rules (unified token count)
    //
    // The personality body has a single hard cap of 2000 tokens, applied
    // at every write surface (Settings UI Save button, solos-config writer,
...
extension Notification.Name {
    ...
    static let soulMdChanged = Notification.Name("SolosSoulMdChanged")
}
...
enum SystemPromptBuilder {
    ...
    /// expectations ("Solos, capable AI assistant, iSH Linux shell")
    ...
    static func identitySection() -> String {
        ...
        let soulEditHint =
            ...
            "1. Tool: call `solos-config` to propose changes (user must approve).\n" +
...
/// Renders the current SOUL.md `name` (falling back to "Solos") and
/// auto-refreshes whenever SoulStore posts `.soulMdChanged`. Use this in
/// any place that previously hard-coded "Solos" as a label.
@MainActor
