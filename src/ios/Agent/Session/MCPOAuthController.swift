...
//
//  Ownership split with the in-guest solos-mcp-cli:
...
//       authorizes there.
//    2. The guest bridge file (<SolosConfig>/mcp-servers/oauth/<name>.json,
...
struct MCPOAuthConfig: Codable, Hashable {
    ...
    var scopes: String?
    /// Custom redirect URI. Default solos-mcp://oauth/callback; Google installed
...
final class MCPOAuthController: NSObject, ObservableObject {
    ...
    nonisolated private static let keychainService = "com.solos.app.mcp-oauth"
    ...
    /// `solos-mcp-cli add --oauth-client-secret` can't reach the Keychain from
    ...
    static func importPendingSecretIfAny(server: String) -> Bool {
        let url = AIChatViewModel.solosMcpServersPersistentDir
    ...
    /// (bind-mounted at /var/solos/mcp-servers/oauth/<name>.json).
    nonisolated static func bridgeFileURL(server: String) -> URL {
        AIChatViewModel.solosMcpServersPersistentDir
            .appendingPathComponent("oauth", isDirectory: true)
