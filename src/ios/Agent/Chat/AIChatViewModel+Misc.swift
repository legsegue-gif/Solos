...
// MARK: - Kernel Boot + Prompt Queue + Solos paths + Dynamic Max Tokens
...
extension AIChatViewModel {
        ...
        Task {
            ...
            do {
                if !ISHKernel.shared.isBooted {
                    ...
                    // /var/solos/* access already routes correctly.
                    SolosFsRouter.shared.installHook()
                }
    ...
    // MARK: - /var/solos/ Shared Directory
    //
    // Unified bidirectional file area between iOS and iSH:
    //   /var/solos/attachments/  — screenshots, images, media
    //   /var/solos/offloads/     — large tool outputs (migrated from /var/offloads/)
    //   /var/solos/workspace/    — general session working area
    //
    // Persistent storage: Library/SolosChat/solos/<sessionId>/{attachments,offloads,workspace}/
    // iSH-visible path:   /var/solos/{attachments,offloads,workspace}/  (session-unaware)
    //
    // On session load/switch, the current session's files are synced into the
    // iSH-visible directory so the model and shell commands always see /var/solos/.

    nonisolated static let solosLinuxBaseDir = "/var/solos"
    nonisolated static let solosAttachmentsLinuxDir = "/var/solos/attachments"
    nonisolated static let solosOffloadsLinuxDir = "/var/solos/offloads"
    nonisolated static let solosWorkspaceLinuxDir = "/var/solos/workspace"
    nonisolated static let solosBrowserLinuxDir = "/var/solos/browser"
    nonisolated static let solosMemoryLinuxDir = "/var/solos/memory"
    nonisolated static let solosSkillsLinuxDir = "/var/solos/skills"
    nonisolated static let solosSharedLinuxDir = "/var/solos/shared"
    nonisolated static let solosMcpServersLinuxDir = "/var/solos/mcp-servers"
    nonisolated static let solosMountsLinuxDir = "/var/solos/mounts"

    /// iOS persistent base for all solos data (Library/SolosChat/solos/).
    nonisolated static var solosPersistentBase: URL {
        let lib = FileManager.default.urls(for: .libraryDirectory, in: .userDomainMask).first!
        return lib.appendingPathComponent("SolosChat/solos", isDirectory: true)
    }

    /// Persistent storage directory for a specific session's offloads.
    nonisolated static func solosOffloadsPersistentDir(for sid: String) -> URL {
    ...
    nonisolated static func solosAttachmentsPersistentDir(for sid: String) -> URL {
        solosPersistentBase.appendingPathComponent(sid, isDirectory: true)
            .appendingPathComponent("attachments", isDirectory: true)
    }

    /// Persistent storage directory for a specific session's uploaded attachments.
    static func solosUploadsDir(for sid: String) -> URL {
        solosAttachmentsPersistentDir(for: sid)
