...
extension AIChatViewModel {
    ...
    func executeSingleToolUse(
        ...
        case "shell_execute":
            ...
            let preSnapshot = snapshotSolosFiles()
            ...
            do {
                ...
                result = try await executeCommand(command, timeout: timeout) { [weak self] line in
                    ...
                    let (cleanedLine, capturedURLs) = SolosURLMarker.extract(from: line)
                    if !capturedURLs.isEmpty {
                        Task { @MainActor in
                            for raw in capturedURLs {
                                if let u = URL(string: raw),
                                   SolosOpenURLBroker.isSupportedScheme(u.scheme) {
                                    SolosOpenURLBroker.shared.offer(u)
                                }
                            }
                        }
                    }
            ...
            if msgIdx < messages.count, blockIdx < messages[msgIdx].blocks.count {
                ...
                if !hasStreamedContent {
                    let (cleaned, capturedURLs) = SolosURLMarker.extract(from: result.output)
                    for raw in capturedURLs {
                        if let u = URL(string: raw),
                           SolosOpenURLBroker.isSupportedScheme(u.scheme) {
                            SolosOpenURLBroker.shared.offer(u)
            ...
            // Scan for new/modified files under /var/solos/
            let postSnapshot = snapshotSolosFiles()
            ...
            if !newOrModified.isEmpty {
                ...
                toolOutput += "\n\n[solos] New/modified files:"
                for path in newOrModified.keys.sorted() {
                    if let url = linuxPathToSolosURL(path) {
                        toolOutput += "\n  \(url)"
                    }
                }
            }
        ...
        case "browser_use":
            ...
            if let b64 = browserResult.base64Image, let data = Data(base64Encoded: b64) {
                ...
                let persistDir = Self.solosBrowserPersistentDir(for: sid)
                ...
                let linuxPath = "\(Self.solosBrowserLinuxDir)/\(screenshotFilename)"
            ...
            if let fetchData = browserResult.fetchedFileData,
               let fetchName = browserResult.fetchedFileName {
                ...
                let linuxPath = "\(Self.solosBrowserLinuxDir)/\(fetchName)"
                if let solosURL = linuxPathToSolosURL(linuxPath) {
                    toolOutput += "\nsolos_url: \(solosURL)"
                }
            }
                   ...
                   browserTabPool.activeManager?.hasInflightDownloads == true {
                    ...
                    downloadReport = BrowserDownloadCenter.shared.agentReport(for: sid)
                        ?? "[browser_downloads] This action triggered a native browser file "
                         ...
                         + "be saved into /var/solos/workspace/ — do NOT re-download it with "
        ...
        case "read_image":
            ...
            let resolvedURL = await resolveSolosPath(pathArg)
            ...
            if let fileURL = resolvedURL,
               ...
               let uiImage = UIImage(data: fileData) {
                ...
                if pathArg.hasPrefix("/var/solos/") {
                    toolImageLinuxPath = pathArg
                } else if pathArg.hasPrefix("solos://") {
                    let tail = String(pathArg.dropFirst("solos://".count))
                    if !tail.isEmpty {
                        toolImageLinuxPath = "/var/solos/\(tail)"
                    }
                }
        ...
        if toolOutput.isEmpty {
            ...
            let offloadSolosURL = linuxPathToSolosURL(offloadResult.linuxPath)
