...
extension AIChatViewModel {
    ...
    /// Snapshot all files under /var/solos/ with their modification dates.
    func snapshotSolosFiles() -> [String: Date] {
        let fm = FileManager.default
        guard let hostBase = resolveHostPath(Self.solosLinuxBaseDir) else { return [:] }
        ...
        for fileURL in fileURLs {
            autoreleasepool {
                ...
                let linuxPath = Self.solosLinuxBaseDir + relativePath
    ...
    /// Resolve a solos:// URL string to a host filesystem URL.
    /// Global dirs (memory/skills/shared) live in the App Group container;
    /// session dirs live under solosPersistentBase/<sessionId>/<host>/.
    private func resolveSolosURL(_ urlString: String) -> URL? {
        guard let url = URL(string: urlString), url.scheme == "solos",
        ...
        let base: URL
        switch host {
        case "memory": base = Self.solosMemoryPersistentDir
        case "skills": base = Self.solosSkillsPersistentDir
        case "shared": base = Self.solosSharedPersistentDir
        case "mcp-servers": base = Self.solosMcpServersPersistentDir
        default:
            guard let sid = sessionId else { return nil }
            base = Self.solosPersistentBase
    ...
    /// Convert a Linux path under /var/solos/ to a solos:// URL, or nil if not under /var/solos/.
    func linuxPathToSolosURL(_ path: String) -> String? {
        let prefix = "/var/solos/"
    ...
    /// For /var/solos/ paths, queries ISHExecutionCoordinator's active bind mount table
    /// to get the actual host URL, avoiding races when mounts switch between concurrent sessions.
    /// Falls back to resolveHostPath for non-/var/solos/ paths (e.g. /tmp, /root).
    func resolvePathForDirectRead(_ linuxPath: String) async -> URL? {
        if linuxPath.hasPrefix("/var/solos/") || linuxPath == "/var/solos" {
            ...
            // Mount table has no entry — fall back to resolveSolosURL with self.sessionId
            if let solosURL = linuxPathToSolosURL(linuxPath),
               let resolved = resolveSolosURL(solosURL) {
                let exists = FileManager.default.fileExists(atPath: resolved.path)
    ...
    /// Resolve a solos:// URL or /var/solos/ Linux path for direct reads.
    /// Unifies both URL schemes into a single async resolution path.
    func resolveSolosPath(_ pathOrURL: String) async -> URL? {
        if pathOrURL.hasPrefix("solos://") {
            // Convert solos:// URL to Linux path, then resolve via mount table
            ...
            let linuxPath = "/var/solos/\(host)" + (subPath.isEmpty ? "" : "/\(subPath)")
    ...
    func executeFileRead(from json: String) async throws -> FileToolResult {
        ...
        var output = "\(header)\n\(content)"
        if let solosURL = linuxPathToSolosURL(path) {
            output += "\nsolos_url: \(solosURL)"
        }
    ...
    func executeFileWrite(from json: String) async throws -> FileToolResult {
        ...
        // If the target is under /var/solos/mounts/ (user-mounted external
        ...
        if path.hasPrefix("/var/solos/mounts/"), let sid = self.sessionId {
            #if DEBUG
            ...
            #if DEBUG
            if path.hasPrefix("/var/solos/mounts/") {
                print("[FileWrite] ensureMounted SKIPPED path=\(path) — sessionId is nil")
            }
            #endif
        }

        // Write-specific resolution for /var/solos/mounts/ targets:
        ...
        // to `resolveSolosURL` which returns a per-session fake path
        // (`solosPersistentBase/<sid>/mounts/...`) that's not inside the
        ...
        // Defense-in-depth: if the Linux path is under /var/solos/mounts/ but
        // the resolved hostURL landed anywhere other than the real external
        // folder (the fakefs `data/var/solos/mounts/` shadow, or a per-session
        // `solosPersistentBase/<sid>/mounts/` fallback path), the mount-table
        ...
        if path.hasPrefix("/var/solos/mounts/") {
            ...
            let fakefsMountsPrefix = RootfsManager.shared.dataPath
                .appendingPathComponent("var/solos/mounts").standardizedFileURL.path
            let sessionFallbackPrefix = Self.solosPersistentBase
