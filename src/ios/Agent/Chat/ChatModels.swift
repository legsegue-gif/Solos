...
struct AttachmentMeta: Identifiable, Equatable {
    let id = UUID()
    /// Linux path, e.g. `/var/solos/attachments/uploads/photo.jpg`
    let path: String
    let size: Int
    let modified: Date

    /// Derive a `solos://` URL from the Linux path.
    var solosURL: String {
        guard path.hasPrefix("/var/solos/") else { return path }
        let rel = String(path.dropFirst("/var/solos/".count))
        return "solos://\(rel)"
    }
