...
/// Shared percent-decoding helpers for resolving `solos://` URLs to on-disk
/// subpaths.
///
/// A correctly-formed `solos_url` percent-encodes the filename exactly once
/// (see `linuxPathToSolosURL`), so `URL(string:).path` decodes it back to the
...
enum SolosURLPathDecoding {
    /// Candidate subpaths for a `solos://` URL, in priority order.
