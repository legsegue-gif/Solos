...
// MARK: - Request-level image budget + Solos paths + tool-output offload
...
extension AIChatViewModel {
    ...
    /// directory and return the iSH-visible `/var/solos/attachments/spillover/...`
    ...
    private static func ensureSpillover(_ data: Data, mimeType: String, sessionId: String) -> String? {
        ...
        let attachmentsRoot = solosPersistentBase
        ...
        return "/var/solos/attachments/spillover/\(sha).\(ext)"
    }
    ...
    nonisolated static func solosWorkspacePersistentDir(for sid: String) -> URL {
        solosPersistentBase.appendingPathComponent(sid, isDirectory: true)
            .appendingPathComponent("workspace", isDirectory: true)
    }

    /// Persistent storage directory for a specific session's browser snapshots.
    nonisolated static func solosBrowserPersistentDir(for sid: String) -> URL {
        solosPersistentBase.appendingPathComponent(sid, isDirectory: true)
    ...
    /// memory) here — anything else leaks into "On My iPhone → Solos".
    nonisolated static var solosAppGroupRoot: URL {
        FileManager.default.containerURL(
            forSecurityApplicationGroupIdentifier: SharedContainerStore.appGroupID
        )!.appendingPathComponent("SolosFileProvider", isDirectory: true)
    }
    ...
    /// Sibling of `solosAppGroupRoot` inside the same App Group container.
    nonisolated static var solosConfigRoot: URL {
        let url = FileManager.default.containerURL(
            forSecurityApplicationGroupIdentifier: SharedContainerStore.appGroupID
        )!.appendingPathComponent("SolosConfig", isDirectory: true)
    ...
    nonisolated static var solosMemoryPersistentDir: URL {
        solosAppGroupRoot.appendingPathComponent("memory", isDirectory: true)
    }

    /// Persistent storage directory for skills (shared across all sessions).
    /// Stored in the App Group container so the FileProvider extension can access it.
    nonisolated static var solosSkillsPersistentDir: URL {
        solosAppGroupRoot.appendingPathComponent("skills", isDirectory: true)
    }

    /// Persistent storage directory for shared files (shared across all sessions).
    /// Stored in the App Group container so the FileProvider extension can access it.
    nonisolated static var solosSharedPersistentDir: URL {
        solosAppGroupRoot.appendingPathComponent("shared", isDirectory: true)
    }

    /// Persistent storage directory for MCP server configs (servers.json,
    /// daemon log). Deliberately under SolosConfig, NOT solosAppGroupRoot:
    ...
    nonisolated static var solosMcpServersPersistentDir: URL {
        solosConfigRoot.appendingPathComponent("mcp-servers", isDirectory: true)
    }
    ...
    nonisolated static func resolveSolosURL(_ url: URL) -> URL? {
        ...
        if let sid = activeSessionId {
            for subPath in subPaths {
                let candidate = solosPersistentBase
        ...
        let globalDirs: [(String, URL)] = [
            ("skills", solosSkillsPersistentDir),
            ("memory", solosMemoryPersistentDir),
            ("shared", solosSharedPersistentDir),
        ]
        ...
        // Scan all sessions
        if let sessions = try? fm.contentsOfDirectory(atPath: solosPersistentBase.path) {
            for sid in sessions {
                for subPath in subPaths {
                    let candidate = solosPersistentBase
                        .appendingPathComponent(sid, isDirectory: true)
    ...
    func offloadToolOutput(_ output: String, toolName: String, toolId: String) -> OffloadResult {
        ...
        let linuxPath = "\(Self.solosOffloadsLinuxDir)/\(fileName)"
