...
extension AIChatViewModel {
    ...
    private func offloadContextContent(_ content: String, toolId: String, toolName: String, ext: String = "txt") -> String {
        ...
        let persistDir = Self.solosOffloadsPersistentDir(for: sid)
        ...
        return "\(Self.solosOffloadsLinuxDir)/tools/\(fileName)"
    }
    ...
    /// Decide whether a file path (Linux view) or solos:// URL is backed by
    /// persistent session storage under `/var/solos/*`. Paths in those
    ...
    /// context.
    private static func isPersistentSolosPath(_ path: String) -> Bool {
        // Linux view: bind-mounted persistent directories under /var/solos
        if path.hasPrefix("/var/solos/workspace/") { return true }
        if path.hasPrefix("/var/solos/browser/") { return true }
        if path.hasPrefix("/var/solos/attachments/") { return true }
        if path.hasPrefix("/var/solos/offloads/") { return true }
        // solos:// URL view maps 1:1 to the directories above
        if path.hasPrefix("solos://workspace/") { return true }
        if path.hasPrefix("solos://browser/") { return true }
        if path.hasPrefix("solos://attachments/") { return true }
        if path.hasPrefix("solos://offloads/") { return true }
        return false
    }
    ...
    private static func originalImagePath(
        ...
        // browser_use (any action) appends `solos_url: solos://...` to the
        // result content when it persists a screenshot or fetched file to
        // /var/solos/browser/. Extract that URL.
        if toolName == "browser_use" || toolName.isEmpty {
            if let urlRange = content.range(of: #"solos://\S+"#, options: .regularExpression) {
    ...
    /// original path lives under `/var/solos/*` (workspace, browser,
        ...
        for mi in agentHistory.indices {
            ...
            for pi in parts.indices {
                ...
                case .toolResult(let id, let name, let content, let isError, let imgData, let mime, let pageURL, _):
                    ...
                    // original path is under /var/solos/* (persistent).
                    let snapshotPath: String?
                    if let op = originalPath, Self.isPersistentSolosPath(op) {
                        snapshotPath = nil
                    } else {
    ...
    func offloadContextIfNeeded(model: LLMModel, lastContextTokens: Int, force: Bool = false) {
        ...
        if offloadedCount > 0 {
            ...
            let sid = sessionId ?? "unknown"
            let toolsDir = Self.solosOffloadsPersistentDir(for: sid)
                .appendingPathComponent("tools", isDirectory: true)
            if let files = try? FileManager.default.contentsOfDirectory(
                ...
                logger.info("  /var/solos/offloads/tools/ — \(files.count) files (showing top \(top20.count)):")
                for file in top20 {
    ...
    /// Remove persistent solos data for a given session.
    private func cleanupSolos(for sid: String) {
        let fm = FileManager.default
        let persistDir = Self.solosPersistentBase.appendingPathComponent(sid, isDirectory: true)
        try? fm.removeItem(at: persistDir)
    }

    /// Sync a single solos subdirectory bidirectionally between persistent storage and iSH-visible path.
    /// One-time migration: moves old `Library/SolosChat/offloads/<sid>/` to
    /// `Library/SolosChat/solos/<sid>/offloads/`.
    func migrateOffloadsIfNeeded(for sid: String) {
        let fm = FileManager.default
        let lib = FileManager.default.urls(for: .libraryDirectory, in: .userDomainMask).first!
        let oldDir = lib.appendingPathComponent("SolosChat/offloads/\(sid)", isDirectory: true)
        guard fm.fileExists(atPath: oldDir.path) else { return }

        let newDir = Self.solosOffloadsPersistentDir(for: sid)
        ...
        logger.info("Migrated offloads for session \(sid) to new solos path")
    }

    /// Bind-mount a session's solos directories into iSH-visible /var/solos/.
    /// Delegates to ISHExecutionCoordinator for serialized mount management.
    func mountSolos(for sid: String) {
        // Always ensure symlinks exist in data/ for the file browser,
        // even when the iSH kernel isn't booted yet.
        ensureSolosSymlinks(for: sid)
    ...
    /// reinstall; contents migrate). All /var/solos symlinks carry absolute
    ...
    private static func noteContainerMigrationIfNeeded() {
        ...
        if let previous, previous != current {
            AppLogger(category: "SolosSymlink").warning("[SolosSymlink] Data container migrated \(previous.prefix(8))… → \(current.prefix(8))… (normal after an app update; contents were migrated by iOS). Session symlinks will be re-pointed as sessions mount.")
        }
        if previous != current {
    ...
    /// can access solos directories without requiring the iSH kernel to be booted.
    private func ensureSolosSymlinks(for sid: String) {
        let fm = FileManager.default
        let dataPath = RootfsManager.shared.dataPath
        Self.noteContainerMigrationIfNeeded()

        // Ensure /var/solos/ exists in data/
        let solosDataDir = dataPath.appendingPathComponent("var/solos")
        try? fm.createDirectory(at: solosDataDir, withIntermediateDirectories: true)

        let mappings: [(persistDir: URL, linuxDir: String)] = [
            (Self.solosOffloadsPersistentDir(for: sid), Self.solosOffloadsLinuxDir),
            (Self.solosAttachmentsPersistentDir(for: sid), Self.solosAttachmentsLinuxDir),
            (Self.solosWorkspacePersistentDir(for: sid), Self.solosWorkspaceLinuxDir),
            (Self.solosBrowserPersistentDir(for: sid), Self.solosBrowserLinuxDir),
            (Self.solosMemoryPersistentDir, Self.solosMemoryLinuxDir),
            (Self.solosSkillsPersistentDir, Self.solosSkillsLinuxDir),
            (Self.solosSharedPersistentDir, Self.solosSharedLinuxDir),
            (Self.solosMcpServersPersistentDir, Self.solosMcpServersLinuxDir),
        ]
        ...
        for (persistDir, linuxDir) in mappings {
            ...
            if exists && (lstatBuf.st_mode & S_IFMT) == S_IFLNK {
                ...
                logger.info("[SolosSymlink] \(linuxDir): stale symlink (-> \(resolved), want \(expected)), replacing")
                    ...
                    for item in contents {
                        ...
                        if !fm.fileExists(atPath: dest.path) {
                            do {
                                ...
                                logger.error("[SolosSymlink] \(linuxDir): migrate FAILED for \(item.lastPathComponent): \(error.localizedDescription)")
                            }
                        } else {
                            skipped += 1
                        }
                ...
                logger.info("[SolosSymlink] \(linuxDir): real dir -> persistDir migration, moved=\(moved) skipped(existing)=\(skipped)")
                try? fm.removeItem(at: hostPath)
            } else if exists {
                // Regular file — unexpected, remove
                logger.info("[SolosSymlink] \(linuxDir): unexpected regular file at \(hostPath.path), removing")
                unlink(hostPath.path)
            }
            ...
            do {
                ...
                logger.info("[SolosSymlink] \(linuxDir): symlink created -> \(persistDir.path)")
            } catch {
                logger.error("[SolosSymlink] \(linuxDir): createSymbolicLink FAILED: \(error.localizedDescription)")
            }
        }
    ...
    /// Create/refresh `/var/solos/mounts/<name>` symlinks for every entry in
    /// `MountedFoldersManager`. Removes stale symlinks whose mounts were deleted.
    ///
    /// Called from:
    ///   - `ensureSolosSymlinks` on each session mount
    ...
    /// Reconcile `/var/solos/mounts/<name>` against `snapshot`. Runs entirely
    ...
    nonisolated private static func performMountedFolderSymlinkRefresh(
        ...
        let fm = FileManager.default
        let dataPath = RootfsManager.shared.dataPath
        let mountsDir = dataPath.appendingPathComponent("var/solos/mounts", isDirectory: true)
    ...
    /// Returns a `solos://attachments/<filename>` URL string, or nil on failure.
    func saveAttachment(_ data: Data, filename: String) -> String? {
        ...
        let persistDir = Self.solosAttachmentsPersistentDir(for: sid)
        try? fm.createDirectory(at: persistDir, withIntermediateDirectories: true)
        ...
        return "solos://attachments/\(encoded)"
    }
