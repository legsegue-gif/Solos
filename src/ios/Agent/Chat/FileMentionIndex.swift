...
struct FileMentionEntry: Identifiable, Equatable, Hashable {
    ...
    /// iSH-visible path (e.g. `/var/solos/workspace/foo/bar.py`). Inserted into input.
    ...
    /// Path shown in the menu (strips the `/var/solos/` prefix for density).
...
final class FileMentionIndex: ObservableObject {
    ...
    private var scanQueue = DispatchQueue(label: "com.solos.filementionindex", qos: .userInitiated)
    ...
    private static func sessionRoots(sessionId: String?) -> [ScanRoot] {
        ...
        return [
            ScanRoot(
                scope: .workspace,
                hostURL: AIChatViewModel.solosWorkspacePersistentDir(for: sid),
                linuxPrefix: AIChatViewModel.solosWorkspaceLinuxDir,
                mountName: nil
            ),
            ScanRoot(
                scope: .attachments,
                hostURL: AIChatViewModel.solosAttachmentsPersistentDir(for: sid),
                linuxPrefix: AIChatViewModel.solosAttachmentsLinuxDir,
                mountName: nil
            ),
        ]
    }
    ...
    private static func sharedRoots() -> [ScanRoot] {
        [
            ScanRoot(
                scope: .shared,
                hostURL: AIChatViewModel.solosSharedPersistentDir,
                linuxPrefix: AIChatViewModel.solosSharedLinuxDir,
                mountName: nil
            ),
            ScanRoot(
                scope: .skills,
                hostURL: AIChatViewModel.solosSkillsPersistentDir,
                linuxPrefix: AIChatViewModel.solosSkillsLinuxDir,
                mountName: nil
            ),
            ScanRoot(
                scope: .memory,
                hostURL: AIChatViewModel.solosMemoryPersistentDir,
                linuxPrefix: AIChatViewModel.solosMemoryLinuxDir,
                mountName: nil
            ),
        ]
    }
    ...
    private static func mountRoots() -> [ScanRoot] {
        ...
        return mgr.entries.compactMap { entry -> ScanRoot? in
            ...
            let linuxPrefix = "\(AIChatViewModel.solosMountsLinuxDir)/\(entry.name)"
    ...
    private struct ScanRoot {
        ...
        let hostURL: URL
        /// Linux-visible path prefix (e.g. `/var/solos/workspace`).
    ...
    /// substring. Lets `@mounts` match `/var/solos/mounts/<name>/...` even
    ...
    /// Is this the immediate child of a scope root (e.g. `/var/solos/skills/foo`,
    /// `/var/solos/mounts/bar`, `/var/solos/shared/baz`)? These top-level
