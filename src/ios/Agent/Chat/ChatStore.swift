...
/// Reference to a media file stored in Library/SolosChat/solos/<sessionId>/
struct MediaRef: Codable, Hashable {
    ...
    /// `/var/solos/attachments/uploads/<name>`, `/var/solos/browser/<sid>/...`).
...
actor ChatStore {
    ...
    /// Base URL for per-session media storage: Library/SolosChat/solos/
    let solosBaseURL: URL
    ...
    init() {
        ...
        let baseURL = libraryURL.appendingPathComponent("SolosChat", isDirectory: true)
        self.dbURL = baseURL.appendingPathComponent("solos.db")
        self.solosBaseURL = baseURL.appendingPathComponent("solos", isDirectory: true)

        try? FileManager.default.createDirectory(at: baseURL, withIntermediateDirectories: true)
        try? FileManager.default.createDirectory(at: solosBaseURL, withIntermediateDirectories: true)
    ...
    /// unlock (BGTask / CloudKit push / location session). solos.db carries
    ...
    private func armRebootGuardIfDegraded() {
        ...
        dbDegraded = true
        logger.error("[RebootGuard] solos.db unusable after open — likely launched before first unlock; arming post-unlock reopen")
    ...
    func reopenIfDegraded() {
        ...
        logger.info("[RebootGuard] solos.db reopened after unlock — session store restored")
    }
    ...
    /// [T-sessions-cli-truncate] Full-text extraction for solos-sessions-cli
    ...
    /// Lightweight session metadata for the solos-sessions-cli offload.
    struct SessionMeta {
        let id: String
        let title: String?
        let preview: String?
    ...
    func searchMessages(sessionIds: [String]? = nil, keywords: [String], limit: Int = 50, startDate: Date? = nil, endDate: Date? = nil) -> [(sessionId: String, sessionTitle: String?, messageId: String, role: String, createdAt: Date, snippet: String)] {
        ...
        // owning session's title to solos-sessions-cli — otherwise the CLI
    ...
    private func markSessionForCloudDeletion(_ sessionId: String) {
        ...
        let sessionDir = solosBaseURL.appendingPathComponent(sessionId, isDirectory: true)
    ...
    /// user removes a file inside `Library/SolosChat/solos/<sid>/...`.
    ...
    /// Files are saved to `Library/SolosChat/solos/<sessionId>/<subdir>/<fileId>.<ext>`.
    func saveMedia(
        ...
        let fileURL = solosBaseURL.appendingPathComponent(relativePath)
    ...
    func mediaFileURL(for ref: MediaRef) -> URL {
        solosBaseURL.appendingPathComponent(ref.relativePath)
    }
    ...
    func deleteSessionMedia(_ sessionId: String) {
        ...
        for subdir in ["browser", "attachments", "images"] {
            let dirURL = solosBaseURL.appendingPathComponent("\(sessionId)/\(subdir)")
    ...
    func mediaFileURLResolver() -> (MediaRef) -> URL {
        let solos = solosBaseURL
        return { ref in
            solos.appendingPathComponent(ref.relativePath)
        }
    }
    ...
    private func scanAndMarkSessionFilesForResurrect(sessionId: String) -> Int {
        ...
        let sessionDir = library.appendingPathComponent("SolosChat/solos/\(sessionId)")
    ...
    func toChatMessage(mediaResolver: (MediaRef) -> URL, showThinking: Bool = true) -> ChatMessage {
        ...
        for part in parts {
            ...
            case .text(let s):
                ...
                // (`[attached image: /var/solos/...]` next to inline image data, and
...
extension ChatStore {
    ...
    func estimateFilesSize() -> Int64 {
        let library = FileManager.default.urls(for: .libraryDirectory, in: .userDomainMask).first!
        let solosBase = library.appendingPathComponent("SolosChat/solos")
        return directorySize(solosBase)
    }

    /// Estimated size of memory files.
    func estimateMemoriesSize() -> Int64 {
        return directorySize(AIChatViewModel.solosMemoryPersistentDir)
    }

    /// Estimated size of skills (db entries + skill files).
    func estimateSkillsSize() -> Int64 {
        return directorySize(AIChatViewModel.solosSkillsPersistentDir)
    }

    /// Estimated size of provider config JSON.
    func estimateProvidersSize() -> Int64 {
        let library = FileManager.default.urls(for: .libraryDirectory, in: .userDomainMask).first!
        let configURL = library.appendingPathComponent("SolosChat/provider-config.json")
    ...
    func estimateEnvVarsSize() -> Int64 {
        let library = FileManager.default.urls(for: .libraryDirectory, in: .userDomainMask).first!
        let envURL = library.appendingPathComponent("SolosChat/env-vars.json")
