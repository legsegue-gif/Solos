...
final class AIChatViewModel: ObservableObject, SpeechControlling {
    ...
    init() {
        ...
        syncChangeObserver = NotificationCenter.default.addObserver(
            ...
            // "Solos is thinking" indicator. Flip a flag instead; the
    ...
    private static func shortFrame(_ raw: String) -> String {
        ...
        // "3   Solos   0x0000000104abcd12 $s5Solos... mangled ... + 40"
    ...
    /// Solos-2026-06-01-134710.ips: user switched sessions while both vms had
    ...
    /// Matches: ![...](solos://attachments/file.mp3?auto_play=true)
    ...
    static func triggerAutoPlayAudio(in text: String) {
        ...
        // Resolve solos:// to file URL
        let fileURL: URL?
        if cleanURL.scheme == "solos" {
            fileURL = resolveSolosFileURLForAutoPlay(url: cleanURL)
        } else {
            fileURL = cleanURL
        }
    ...
    /// Resolve solos:// URL to local file URL (standalone, no UI dependency).
    private static func resolveSolosFileURLForAutoPlay(url: URL) -> URL? {
        guard url.scheme == "solos", let host = url.host else { return nil }
        ...
        if globalDirs.contains(host) {
            ...
            for subPath in subPaths {
                let globalURL = library.appendingPathComponent("SolosChat/\(host)", isDirectory: true)
        ...
        if let sid = activeSessionId {
            for subPath in subPaths {
                let persistURL = solosPersistentBase
        ...
        // [T-ios-solosurl-cross-session-isolation] Fallback to the iSH-visible
        // /var/solos data dir ONLY for the global, non-session-scoped
        // namespaces (memory/skills/shared). For a session-scoped host like
        // `attachments`, /var/solos/<host> binds to whichever session is
        ...
        if globalDirs.contains(host) {
            ...
            for subPath in subPaths {
                let linuxPath = "/var/solos/\(host)/\(subPath)"
    ...
    private var baseSystemPrompt: String {
        ...
        SystemPromptBuilder.identitySection()
            ...
            // Solos has no in-app scheduler (see 'Scheduled tasks'), so the only
            ...
            + "IMPORTANT: solos:// URLs are app-internal — they are NOT web URLs. Do NOT pass solos:// action URLs (open_terminal, views, settings) to browser_use — those are app deep links, use Markdown links in chat instead. "
            + "However, solos:// resource URLs CAN be opened in browser_use with navigate. All directories under /var/solos/ are accessible: workspace, attachments, offloads, shared, etc. "
            + "The built-in browser fully supports solos:// — HTML pages and all sub-resources (JS, CSS, images, fonts, etc.) referenced via solos:// absolute URLs or relative paths resolve correctly within the current session. "
            + "When building multi-file web projects, use file_write to create files in the same directory (e.g. /var/solos/workspace/myapp/), "
            + "then reference sub-resources with relative paths in HTML (e.g. <link href=\"style.css\">, <script src=\"app.js\">, <img src=\"logo.png\">). "
            + "The browser resolves relative paths against the solos:// base URL automatically. "
            + "Cross-directory references also work with absolute solos:// URLs (e.g. <img src=\"solos://attachments/photo.png\"> from a workspace HTML page). "
            + "Navigate to the entry HTML to preview, e.g. solos://workspace/myapp/index.html.\n"
            ...
            + "IMPORTANT: solos:// URLs MUST be percent-encoded. Non-ASCII characters (Chinese, emoji, spaces, etc.) in filenames will break Markdown rendering if not encoded. "
            + "Use the solos_url from tool results directly — it is already encoded. "
            + "If you construct a solos:// URL manually, percent-encode the filename (e.g. %E4%B8%AD%E6%96%87 for non-ASCII characters).\n"
            ...
            + "Audio auto-play: append ?auto_play=true to an audio solos:// URL to auto-play when rendered (e.g. ![audio](solos://attachments/song.mp3?auto_play=true)). Use sparingly — only when the user explicitly asks to hear audio immediately.\n"
            ...
            + "Use Markdown links for all solos:// files — the user can tap to preview them directly in chat.\n\n"
            ...
            + "apple-alarm sets alarms and timers via AlarmKit (iOS 26+). Alarms can only be viewed in the Solos home screen (alarm icon in the top-right toolbar) or by opening solos://views/alarm. "
            + "After setting an alarm, tell the user it is visible on the Solos home screen and they can tap the alarm icon or open solos://views/alarm to manage it.\n"
            ...
            + "solos-model-use: Invoke other LLM models pre-configured by the user. "
            + "You have \(ProviderConfigStore.shared.resolvedAgentLoopEntries.count) model(s) available. "
            + "Use `solos-model-use list` to see them (includes each model's modality capabilities like image_output, audio_output, etc.), "
            ...
            + "solos-browser-use: CLI wrapper around the in-app browser_use tool — accepts the exact same actions and parameters as the browser_use tool call, just exposed as `<action> --flag value` pairs (or `--json '<obj>'`). "
            + "Run `solos-browser-use` with no arguments (or --help) for the full action list. "
            + "Example: `solos-browser-use navigate --url https://example.com`. "
            + "Prefer this over the browser_use tool call when you need multi-step or batch browser flows: write a bash script that chains multiple solos-browser-use invocations (scrape N pages in a loop, click-through forms, navigate→extract→navigate pipelines) and run it with shell_execute. "
            + "Output is JSON, same shape as the tool call result.\n"
            + "Interactive terminal: solos://open_terminal opens a terminal for tasks that require interactive stdin (passwords, ssh, TUI apps like htop/vi). "
            ...
            + "[Set ENV_NAME](solos://settings/environments?create_key=ENV_NAME&create_value=&create_note=Used%20by%20XYZ) — "
    ...
    /// user gets two overlapping notifications for one task: "Solos Task
    ...
    /// Currently active session ID — accessible from anywhere for solos:// URL resolution.
    ...
    func send() {
        ...
        currentTask = Task { [weak self] in
            ...
            let uploadsDir = Self.solosUploadsDir(for: sid)
            ...
            for (i, attachment) in pendingAttachments.enumerated() {
                ...
                let linuxPath = "/var/solos/attachments/uploads/\(safeName)"
            ...
            await MainActor.run {
                ...
                if !attachmentMetas.isEmpty {
                    NotificationCenter.default.post(name: .solosUserAttachmentsMounted,
                                                    object: userMsg.id)
                }
            }
            ...
            if !attachmentMetas.isEmpty {
                ...
                for meta in attachmentMetas {
                    xml += "  <file path=\"\(meta.path)\" url=\"\(meta.solosURL)\" size=\"\(meta.size)\" modified=\"\(nowStr)\" />\n"
                }
    ...
    func retryFromMessage(_ messageId: UUID, replacementAttachments: [InputAttachment]? = nil) {
        ...
        if let newAttachments = replacementAttachments, !newAttachments.isEmpty {
            let uploadsDir = Self.solosUploadsDir(for: sessionId ?? "draft")
            let fm = FileManager.default
            try? fm.createDirectory(at: uploadsDir, withIntermediateDirectories: true)
            ...
            for prompt in queued {
                if !prompt.attachments.isEmpty {
                    ...
                    if let chatMsg = self.messages.first(where: { $0.queuedPromptId == prompt.id }) {
                        ...
                        NotificationCenter.default.post(name: .solosUserAttachmentsMounted, object: chatMsg.id)
                    }
                }
    ...
    private func injectQueuedPromptsAsNewTurn(
        ...
        let qUploadsDir = Self.solosUploadsDir(for: qSid)
    ...
    private func handleUserCancelledCleanup() {
        ...
        if userDidCancel {
            ...
            // render as a bare "Solos" header bubble with the typing
