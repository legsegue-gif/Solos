...
final class ViewModelCache {
    ...
    /// processing. Called from SolosApp on scenePhase → .inactive so that
    ...
    ///
    /// The one caller is `SolosApp`'s `.id(appLanguage)` re-key: changing the
    ...
    /// that accumulated while backgrounded. Called from SolosApp on
