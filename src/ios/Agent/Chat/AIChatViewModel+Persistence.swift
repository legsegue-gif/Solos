...
extension AIChatViewModel {
    ...
    func loadSession() async {
        ...
        guard !rawMessages.isEmpty else {
            ...
            // Still mount solos even for empty sessions so /var/solos works in terminal
            logger.info("🔍MOUNT loadSession empty-session fallback, still mounting for \(sessionId)")
            mountSolos(for: sessionId)
            return
        }
        ...
        // Phase 4: Migrate offloads + mount solos
        ...
        logger.info("🔍MOUNT loadSession Phase4 calling mountSolos for \(sessionId) (msgs=\(rawMessages.count))")
        mountSolos(for: sessionId)
