...
// MARK: - Solos URL Capture Broker

/// Shared broker for URLs captured from shell tool stdout via the OSC
/// SolosOpenURL marker (emitted by /usr/local/bin/solos-open). Both
...
@MainActor
final class SolosOpenURLBroker: ObservableObject {
    static let shared = SolosOpenURLBroker()
    ...
    /// preview nested on top of itself instead. `solos://` resource
    ...
    /// Schemes that `solos-open` may emit and that the host knows how to
    /// route. `http`/`https`/`about` → WKWebView preview, `solos` → built-in
    /// file preview via `handleSolosURLTap`.
    nonisolated static func isSupportedScheme(_ scheme: String?) -> Bool {
        guard let s = scheme?.lowercased() else { return false }
        return s == "http" || s == "https" || s == "about" || s == "solos"
    }

    /// True for schemes that render as an in-chat WKWebView (safariURL /
    /// ToolLiveSheet.linkPreviewURL). `solos://` resources render as
...
// MARK: - Solos URL Marker Parser

/// Recognises the OSC 1337 `SolosOpenURL` escape sequence emitted by the
/// rootfs shim at `/usr/local/bin/solos-open` (see default_mount). The shim
/// replaces `xdg-open`, `sensible-browser`, etc. Whenever an in-sandbox
/// command tries to open a URL, it prints:
///
///     ESC ] 1337 ; SolosOpenURL = <url> BEL
///
...
enum SolosURLMarker {
    /// ESC ] 1337 ; SolosOpenURL = ... BEL
    /// Also accepts ST (ESC \) as terminator for robustness.
    static let pattern = "\u{1B}\\]1337;SolosOpenURL=([^\u{07}\u{1B}]*)(?:\u{07}|\u{1B}\\\\)"
    ...
    static func extract(from text: String) -> (cleaned: String, urls: [String]) {
        guard text.contains("SolosOpenURL="),
