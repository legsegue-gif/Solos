...
extension AIChatViewModel {
    ...
    nonisolated static func loadGlobalMemoryFragment() -> String? {
        let globalFile = solosMemoryPersistentDir.appendingPathComponent("GLOBAL.md")
    ...
    nonisolated static func loadRecentDailyMemoryFragment() -> String? {
        ...
        while fragments.count < 3 && dayOffset < maxLookback {
            ...
            let fileURL = solosMemoryPersistentDir.appendingPathComponent("\(dateStr).md")
    ...
    func executeMemoryWrite(from json: String) -> FileToolResult {
        ...
        let fm = FileManager.default
        let persistDir = Self.solosMemoryPersistentDir
        ...
        let linuxPath = "\(Self.solosMemoryLinuxDir)/\(fileName)"
    ...
    func executeMemoryGet(from json: String) -> FileToolResult {
        ...
        let fm = FileManager.default
        let memDir = Self.solosMemoryPersistentDir
