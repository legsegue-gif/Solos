...
final class SyncLogStore: ObservableObject {
    ...
    func exportSanitizedReport() -> String {
        var lines: [String] = []
        lines.append("=== SolosApp iCloud Sync Diagnostic Log ===")
...
final class CloudSyncEngine: ObservableObject {
    ...
    private lazy var container = CKContainer(identifier: "iCloud.com.solos.app")
    ...
    private init() {
        let library = FileManager.default.urls(for: .libraryDirectory, in: .userDomainMask).first!
        self.stateURL = library.appendingPathComponent("SolosChat/sync_state.json")
        self.sessionCacheDir = library.appendingPathComponent("SolosChat/sync_etag_cache")
        self.globalEtagCacheURL = library.appendingPathComponent("SolosChat/sync_etag_global.plist")
        ...
        let legacyCacheURL = library.appendingPathComponent("SolosChat/sync_record_cache.bin")
    ...
    func buildCKRecord(type: String, id: String) async -> CKRecord? {
        ...
        case "SessionFile":
            ...
            let library = FileManager.default.urls(for: .libraryDirectory, in: .userDomainMask).first!
            let fileURL = library.appendingPathComponent("SolosChat/solos/\(sessionId)/\(relativePath)")
    ...
    func processRemoteRecord(_ record: CKRecord) async {
        ...
        case "SessionFile":
            ...
            if let asset = record["asset"] as? CKAsset, let srcURL = asset.fileURL {
                let library = FileManager.default.urls(for: .libraryDirectory, in: .userDomainMask).first!
                let destURL = library.appendingPathComponent("SolosChat/solos/\(sessionId)/\(relativePath)")
        ...
        default:
            ...
            // solos-shared (e.g. when v1 is briefly active before pause
    ...
    static func mergeEnvVars(remoteJson: String) {
        ...
        let library = FileManager.default.urls(for: .libraryDirectory, in: .userDomainMask).first!
        let localURL = library.appendingPathComponent("SolosChat/env-vars.json")
        ...
        case .fetchedRecordZoneChanges(let zoneChanges):
            Task.detached { [weak self] in
                ...
                for modification in zoneChanges.modifications {
                    ...
                    // them when fetching solos-shared. Skip logging to
