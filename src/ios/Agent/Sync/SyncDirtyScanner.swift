...
final class SyncDirtyScanner {
    ...
    static func maxModDate(inSkill skillId: String) -> Date {
        let dir = AIChatViewModel.solosSkillsPersistentDir.appendingPathComponent(skillId)
    ...
    private func scanConfigFiles() async {
        ...
        let files: [(path: String, recordType: String, recordId: String)] = [
            ("SolosChat/provider-config.json", "ProviderConfig", "provider-config"),
            ("SolosChat/env-vars.json", "EnvVar", "env-vars"),
        ]
        for file in files {
        ...
        // SolosConfig persistent dir (bind-mounted at /var/solos/mcp-servers),
        // and is also written by the in-guest solos-mcp-cli — so an mtime scan
    ...
    private func scanMemoryFiles() async {
        let memDir = AIChatViewModel.solosMemoryPersistentDir
