...
final class ICloudBackupManager: ObservableObject {
    ...
    enum BackupCategory: String, CaseIterable, Identifiable {
        case sessions           // solos.db + media/ + solos/
    ...
    private let containerID = "iCloud.com.solos.app"
    ...
    private var solosBaseURL: URL {
        let lib = fm.urls(for: .libraryDirectory, in: .userDomainMask)[0]
        return lib.appendingPathComponent("SolosChat", isDirectory: true)
    }

    private var solosDBURL: URL { solosBaseURL.appendingPathComponent("solos.db") }
    private var skillsDBURL: URL { solosBaseURL.appendingPathComponent("skills.db") }
    private var mediaURL: URL { solosBaseURL.appendingPathComponent("media", isDirectory: true) }
    private var sessionSolosURL: URL { solosBaseURL.appendingPathComponent("solos", isDirectory: true) }
    private var skillsURL: URL { solosBaseURL.appendingPathComponent("skills", isDirectory: true) }
    private var memoryURL: URL { solosBaseURL.appendingPathComponent("memory", isDirectory: true) }
    ...
    private func stageSessionFiles(to dir: URL) async throws {
        // WAL checkpoint solos.db before copying
        try walCheckpoint(dbPath: solosDBURL.path)

        // Copy solos.db
        let dbDest = dir.appendingPathComponent("solos.db")
        if fm.fileExists(atPath: solosDBURL.path) {
            try fm.copyItem(at: solosDBURL, to: dbDest)
        }
        ...
        // Copy solos/ (session workspace files)
        if fm.fileExists(atPath: sessionSolosURL.path) {
            try fm.copyItem(at: sessionSolosURL, to: dir.appendingPathComponent("solos"))
        }
    }
    ...
    private func restoreSessionFiles(from dir: URL) async throws {
        ...
        let solosSrc = dir.appendingPathComponent("solos")
        if fm.fileExists(atPath: solosSrc.path) {
            if fm.fileExists(atPath: sessionSolosURL.path) {
                try fm.removeItem(at: sessionSolosURL)
            }
            try fm.copyItem(at: solosSrc, to: sessionSolosURL)
        }
    }
