...
enum ChatStoreSyncHydrators {
    ...
    private static func buildMemoryGlobal() async -> PortableRecord? {
        let url = AIChatViewModel.solosMemoryPersistentDir
    ...
    /// what made Solos#98 hard to characterise — the group just looked
    ...
    private static func mergeProviderModelGroupV3(record: PortableRecord) async {
        ...
        if applied {
            ...
            // (Solos#98 defect 1). A group references model ENTRIES, and an
