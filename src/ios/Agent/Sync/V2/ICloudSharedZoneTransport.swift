...
///
/// Zones:
///   - solos-shared   : Session/Message/CompactMarker/SessionFile/Skill records
///   - solos-devices  : SyncDevice heartbeat records
///   - solos-secrets  : ProviderConfig + EnvVar (separate so users can
...
final class ICloudSharedZoneTransport: NSObject, SyncTransport {
    ...
    static let containerIdentifier = "iCloud.com.solos.app"

    /// Fixed zone names. Never include device id.
    static let sharedZoneName  = "solos-shared"
    static let devicesZoneName = "solos-devices"
    static let secretsZoneName = "solos-secrets"
    ...
    private var etagCacheURL: URL {
        FileManager.default.urls(for: .libraryDirectory, in: .userDomainMask)[0]
            .appendingPathComponent("SolosChat/cloud-sync-v2/etag-cache.plist")
    }
    ...
    override init() {
        ...
        let dir = FileManager.default
            ...
            .appendingPathComponent("SolosChat/cloud-sync-v2", isDirectory: true)
    ...
    private static let etagFlushQueue = DispatchQueue(label: "com.solos.sync.etagFlush", qos: .utility)
    ...
    private func scheduleRecentFetchTimer() {
        ...
        let t = DispatchSource.makeTimerSource(queue: DispatchQueue(label: "com.solos.sync.recentFetch", qos: .utility))
    ...
    private func fetchRecentV2() async {
        ...
        // Provider*V3 records, MCPServersV2) live in solos-secrets; the old
        // code hardcoded solos-shared for every query, so those types always
...
extension ICloudSharedZoneTransport: CKSyncEngineDelegate {
    ...
    nonisolated func nextRecordZoneChangeBatch(
        ...
        await MainActor.run { [weak self] in
            ...
            // of its former rate: Solos#141's backlog became ~190 CKSyncEngine
    ...
    private func handleFetchedZoneChanges(_ zoneChanges: CKSyncEngine.Event.FetchedRecordZoneChanges) {
        ...
        autoreleasepool {
            for modification in zoneChanges.modifications {
                ...
                // entire private database; solos-* zones belong to v2 only.
