...
enum MigrationPhase: String, Codable {
    ...
    case v2ZonesCreate     // 2: ensure solos-shared / -devices / -secrets exist
...
/// Library/SolosChat/cloud-sync-v2/migration-state.json.
...
final class MigrationEngine {
    ...
    struct ProgressSummary {
        ...
        /// pushing. That combination is exactly what Solos#141 reported as
    ...
    func requestMigration() {
        ...
        // round and the migration could never converge — the reason Solos#154
    ...
    /// next attempt starts from a clean slate. Solos#154's actual request.
    ///
    ...
    func progressSummary() async -> ProgressSummary? {
        ...
        // Solos#154 shipped a screenshot reading "41 / 14 (100%)" after the
    ...
    private func phaseV2ZonesCreate(_ s: inout MigrationState) async throws {
        logger.info("[SyncMigration] phase=v2ZonesCreate start: zones=[solos-shared,solos-devices,solos-secrets]")
    ...
    private func phaseV1ZoneDelete(_ s: inout MigrationState) async throws {
        ...
        // loop with nothing being guarded. Solos#154 sat here for five
        ...
        guard let actualCloud = cloudCount else {
            ...
            // two-minute burn (Solos#154: five days of exactly this).
            //
...
enum V1FetcherShim {
    ...
    enum ZoneKind: String {
        case v2     // solos-shared / solos-devices / solos-secrets
        case v1     // device-<id>
