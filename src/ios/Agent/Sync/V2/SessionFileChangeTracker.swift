...
actor SessionFileChangeTracker {
    ...
    func recordBatch(
        ...
        for (sid, rawPath, opCode, _) in events {
            ...
            guard let rel = Self.resolveSessionRelative(
                rawPath: rawPath, sessionId: sid, solosBaseURL: solosBaseURL
            ) else {
                totalIgnored &+= 1
                ignored += 1
    ...
    /// schedule races).
    func drainAllWithMtimeFilter(solosBaseURL: URL) -> [String: [String: Change]] {
    ...
    /// file:
    ///
    ///   • Guest path:  `/var/solos/<subdir>/<rel>` (fakefs path-normalize
    ///     left the bind-mount symlink alone, e.g. unlink/rename)
    ///   • Host path:   `<solosBaseURL>/<sid>/<subdir>/<rel>` (path-normalize
    ...
    nonisolated static func resolveSessionRelative(
        rawPath: String, sessionId: String, solosBaseURL: URL
    ) -> String? {
        // Guest-shape: trivial prefix strip.
        let guestPrefix = "/var/solos/"
        ...
        // Host-shape: must live under this session's per-session solos dir.
        ...
        let hostBase = solosBaseURL.path
...
///
/// iSH stays agnostic about Solos sessions — this Swift bridge knows the
/// per-session host directory layout (`<solosBaseURL>/<sid>/<subdir>/...`)
/// and the bind-mount guest layout (`/var/solos/<subdir>/...`), and resolves
...
func installSessionFileChangeTracker(kernel: ISHKernel) {
    ...
    installLogger.info("[SessionFileTracker] installing fakefs change handler — iSH realfs writes/unlinks/renames under /var/solos/{workspace,attachments,browser,offloads} (or the equivalent host APFS path) will be queued for iCloud SessionFile sync")
    // Captured at install time; solosBaseURL doesn't change for the
    // lifetime of the app process.
    let solosBaseURL = ChatStore.shared.solosBaseURL
    kernel.installFakefsChangeHandler { events in
        ...
        // the write/delete; SolosFsRouter maps that back to the sid that
        ...
        for event in events {
            ...
            if event.fsContext != 0 {
                resolvedSid = SolosFsRouter.shared.sid(for: event.fsContext) ?? fallbackSid
            } else {
        ...
        if !tuples.isEmpty {
            Task.detached(priority: .utility) {
                await SessionFileChangeTracker.shared.recordBatch(tuples, solosBaseURL: solosBaseURL)
            }
        }

        // Skills side-channel: if any event in this batch touched a
        // path under `/var/solos/skills/` (or the equivalent host APFS
        ...
        if FakefsSkillPathCheck.batchTouchesSkills(events: events, solosBaseURL: solosBaseURL) {
...
fileprivate enum FakefsSkillPathCheck {
    static func batchTouchesSkills(events: [ISHFakefsChangeEvent], solosBaseURL: URL) -> Bool {
        let guestPrefix = "/var/solos/skills"
        // host APFS path looks like ".../SolosChat/solos/skills/..."
        // — solosBaseURL = "<lib>/SolosChat/solos", so the host prefix
        //   to check is "<solosBaseURL>/skills".
        let hostPrefix = solosBaseURL.standardizedFileURL.path + "/skills"
        for evt in events {
            let p = evt.linuxPath
...
final class SkillFilesystemNotifier {
    ...
    /// True when at least one fakefs event touching /var/solos/skills/
