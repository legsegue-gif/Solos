...
extension BackupImporter {
    ...
    func snapshotForRollback(category: BackupCategory, staging: URL) throws -> BackupRollbackSnapshot {
        ...
        let liveDirs: [URL]
        switch category {
        case .sharedFiles: liveDirs = [AIChatViewModel.solosSharedPersistentDir]
        case .memory: liveDirs = [AIChatViewModel.solosMemoryPersistentDir]
    ...
    private func importSharedFiles(root: URL, fileIndex: [BackupFileIndexEntry]) throws
        ...
        let base = AIChatViewModel.solosSharedPersistentDir
        let files = try restoreFileTree(
    ...
    private func importSkills(root: URL, fileIndex: [BackupFileIndexEntry]) async throws
        ...
        let files = try restoreFileTree(
            ...
            destinationFor: { path in
                ...
                return AIChatViewModel.solosSkillsPersistentDir
                    .appendingPathComponent(parts[1], isDirectory: true)
                    .appendingPathComponent(parts[2])
            })
        report.filesWritten = files.written
    ...
    private func importMemory(root: URL) async throws -> CategoryReport {
        ...
        let dst = AIChatViewModel.solosMemoryPersistentDir
    ...
    func containmentRoot(for category: BackupCategory) -> URL {
        switch category {
        case .chats: return AIChatViewModel.solosPersistentBase
        case .sharedFiles: return AIChatViewModel.solosSharedPersistentDir
        case .skills: return AIChatViewModel.solosSkillsPersistentDir
        case .memory: return AIChatViewModel.solosMemoryPersistentDir
        case .providers, .mcpServers, .voiceCorrections, .environmentVariables:
            ...
            return AIChatViewModel.solosAppGroupRoot
        }
    }
    ...
    ///     still looked contained. `solosAppGroupRoot`'s shared/skills/memory
