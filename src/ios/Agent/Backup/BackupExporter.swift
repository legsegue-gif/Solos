...
/// Builds a `.solosbak` package (docs/backup-restore-design.md §2, §9 stage 1).
///
...
actor BackupExporter {
    ...
    private func exportBody(options: Options,
        ...
        // a bare `backup-f9c34c.solosbak` with no date, leaving a folder of
    ...
    private func exportChats(dataDir: URL, trees: BackupFileTreeExporter,
        ...
        for sid in sessionIds {
            ...
            // workspace / browser.
            let dir = AIChatViewModel.solosPersistentBase
    ...
    private func exportSharedFiles(trees: BackupFileTreeExporter) throws
        -> BackupManifest.CategoryStat {
        let r = try trees.export(root: AIChatViewModel.solosSharedPersistentDir,
    ...
    private func exportMemory(dataDir: URL) throws -> BackupManifest.CategoryStat {
        let src = AIChatViewModel.solosMemoryPersistentDir
    ...
    /// `<device>-<yyyyMMdd>-<sortable-id>.solosbak`, e.g.
    /// `Sams-iPhone-20260823-mf3k9q2phz.solosbak`. Three deliberate changes
    /// from the old `backup-20260823-1259-f69c00.solosbak`:
    ///
    ...
    ///    `backup-` prefix is dropped to pay for it — `.solosbak` already says
    ///    what the file is.
    ...
    /// `Sams-iPhone-20260823-mf3k9q2phz-encrypted.solosbak`. Whether a
    ...
    /// A filename-safe, ASCII device token, e.g. `Sams-iPhone`.
    ///
    ...
    static func filenameDeviceToken(_ raw: String, fallback: String = "device") -> String {
        ...
        for ch in raw {
            if ch.isASCII && (ch.isLetter || ch.isNumber) {
                ...
                // the overwhelmingly common device name is "Sam's iPhone",
                // and splitting on the apostrophe yields `Sam-s-iPhone`,
                ...
                // a single dash instead of emitting `Sam--s---iPhone`.
                out.append("-")
