...
enum BackupCrypto {
    ...
    static let scheme = "solosbak-enc/1"
    ...
    final class Keys {
        ...
        fileprivate init(kek: SymmetricKey) {
            ...
            dataKey = sub("solosbak/data")
            secretsKey = sub("solosbak/secrets")
            macKey = sub("solosbak/mac")
            verifierKey = sub("solosbak/verify")
        }

        /// `HMAC(verifier_key, "solosbak-v1")` truncated to 16 bytes (§5.2).
        ...
        var verifier: String {
            ...
            mac.update(data: Data("solosbak-v1".utf8))
