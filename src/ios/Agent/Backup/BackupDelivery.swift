...
/// Hands a finished `.solosbak` package to the user
...
enum BackupDelivery {
    ...
    /// The registered UTI for `.solosbak` (Info.plist `UTExportedTypeDeclarations`).
    ///
    /// Registering it matters for two reasons beyond tidiness:
    ///   1. `SolosShareSheet.sanitizedShareURL` copies the file to a `.bin`
    ...
    ///      will want.
    static let contentTypeIdentifier = "com.solos.app.solosbak"
    ...
    /// Where delivered packages live: `<AppGroup>/SolosFileProvider/Backups/`.
    static var backupsDirectory: URL {
        AIChatViewModel.solosAppGroupRoot
    ...
    ///   1. `shared/` is bind-mounted into the guest at `/var/solos/shared`
    ...
    static func migrateLegacySharedBackups() {
        let legacy = AIChatViewModel.solosSharedPersistentDir
            .appendingPathComponent(backupsDirectoryName, isDirectory: true)
    ...
    /// cloud folder in Files and authorised it in Solos, a backup can be
    ...
    /// TRUNCATED file carrying a perfectly valid `.solosbak` name, which then
