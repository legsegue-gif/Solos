...
enum BackupExportJournal {
    ...
    ///
    /// Deliberately still named `solosbak-<backupId>`: `archive()` zips this
    ...
    static func stagingRoot(backupId: String) -> URL {
        supportDir.appendingPathComponent("solosbak-\(backupId)", isDirectory: true)
    }
    ...
    static func sweepAbandoned() {
        let fm = FileManager.default
        let keep = interrupted().map { "solosbak-\($0.backupId)" }

        var removed = 0
        for name in (try? fm.contentsOfDirectory(atPath: supportDir.path)) ?? [] {
            guard name.hasPrefix("solosbak-"), name != keep else { continue }
        ...
        //   solosbak-        pre-resume export staging; import/peek work dirs
        ...
        //
        // Plus ANY `*.solosbak` in tmp/, which is what actually catches the
        ...
        let prefixes = ["solosbak-", "backup-", "restore-", "server-restore-"]
