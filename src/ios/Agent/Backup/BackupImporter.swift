...
/// Restores a `.solosbak` package (docs/backup-restore-design.md §8).
///
...
actor BackupImporter {
    ...
    private func importBody(from packageURL: URL,
        ...
        let work = fm.temporaryDirectory
            .appendingPathComponent("solosbak-import-\(UUID().uuidString)", isDirectory: true)
    ...
    func inspect(packageURL: URL) throws -> BackupManifest {
        let work = fm.temporaryDirectory
            .appendingPathComponent("solosbak-peek-\(UUID().uuidString)", isDirectory: true)
        defer { try? fm.removeItem(at: work) }
        let root = try unpack(packageURL, to: work)
    ...
    /// reverse; the archive nests everything under `solosbak-<uuid>/`, so the
    ...
    /// REFUSED `solosbak/1.1` even though §2.2 rule 2 says same-major must
