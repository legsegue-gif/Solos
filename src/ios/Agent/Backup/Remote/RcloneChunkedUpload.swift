...
/// server always holds a clean, self-contained `.solosbak` a user can grab
...
enum RcloneTransfer {
    ...
    private static func sweepAbandonedPartials(fs: String,
        ...
        for e in list {
            ...
            // `.pkg.solosbak.partial` still ends in `.partial` and is still
    ...
    /// Every `.solosbak` in `remote`'s backup directory, newest first.
    ///
    /// In-flight scratch files never show up as restorable backups: they are
    /// named `<package>.solosbak.partial`, whose suffix is `.partial`, and the
    /// filter below requires the name to END in `.solosbak`.
    ...
    /// `.solosbak` files directly inside it.
    ///
