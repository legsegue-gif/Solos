...
enum RcloneBridge {
    ...
    ///
    /// `SolosRcloneRPC` is a BLOCKING cgo call — nothing on the Swift side can
    ...
    private static func rpcUnchecked(_ method: String,
        ...
        var status: Int32 = 0
        guard let raw = SolosRcloneRPC(cMethod, cInput, &status) else {
            throw RPCError(status: -1, payload: "")
        }
        defer { SolosRcloneFreeString(raw) }
