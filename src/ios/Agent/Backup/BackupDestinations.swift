...
/// Solos, and iOS's FileProvider handles the protocol. From here it is an
/// ordinary directory.
@MainActor
enum BackupDestinations {
    ...
    /// folders in front of the user and invited them to scatter `.solosbak`
    ...
    /// probe (`isWritable`) and the user's own "don't let Solos write here"
    ...
    ///
    /// The local copy in Solos ▸ Backups is written before this runs and is
    ...
    /// A `.solosbak` found inside a mounted destination.
