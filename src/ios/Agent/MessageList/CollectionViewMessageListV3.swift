...
extension Notification.Name {
    ...
    static let solosUserAttachmentsMounted = Notification.Name("solosUserAttachmentsMounted")
}
...
/// Header: "✦ Solos" label at the top of each assistant turn.
...
private struct BridgedAssistantHeaderV3: View {
    ...
    /// environment.
    ///
    /// `\.solosURL` is set on AIChatView, but each cell here is hosted in
    ...
    var body: some View {
        HStack(spacing: 6) {
            ...
            Text(soulMeta.name.isEmpty ? "Solos" : soulMeta.name)
...
private struct BridgedAssistantBlockV3: View {
    ...
    var body: some View {
        AssistantBlockView(
            ...
            // [T-selection-menu-solos-tts] Selection-menu TTS: whole-reply
...
extension CollectionViewMessageListV3 {
    ...
    final class Coordinator: NSObject, UICollectionViewDelegate, UIScrollViewDelegate, UICollectionViewDataSourcePrefetching {
        ...
        // already-inserted user message; see .solosUserAttachmentsMounted.
        ...
        /// `.solosAttachmentSizeChanged` bursts (a six-image message fires up
        ...
        /// The actual `.solosAttachmentSizeChanged` work, run at most once per
        ...
        private func updateBridge(_ bridge: CellStateBridgeV2, message: ChatMessage, in messages: [ChatMessage],
            ...
            bridge.onReadAloud = (message.role == .assistant)
                ...
                : nil
            // [T-selection-menu-solos-tts] "Read Selection" from the text
            // selection menu — speaks the selected snippet via the Solos TTS
        ...
        private func applySnapshot(messages rawMessages: [ChatMessage], caller: String = "?") {
            ...
            if attachmentSizeChangedSub == nil {
                attachmentSizeChangedSub = NotificationCenter.default.publisher(for: .solosAttachmentSizeChanged)
                    .receive(on: DispatchQueue.main)
            ...
            if attachMountSub == nil {
                attachMountSub = NotificationCenter.default.publisher(for: .solosUserAttachmentsMounted)
                    .receive(on: DispatchQueue.main)
                    .sink { [weak self] notification in
            ...
            if let layout = viewController?.messageListLayout {
                ...
                for (i, item) in newItems.enumerated() {
                    ...
                    switch item {
                    case .assistantHeader:
                        // Header is always a fixed "sparkles Solos" label row (measured: 28pt)
            ...
            if let cv = viewController?.collectionView {
                ...
                // TALLER height it measured while "Solos is thinking…" was
        ...
        /// engine: an offscreen SelectableMarkdownTextView (SolosLayoutManager +
