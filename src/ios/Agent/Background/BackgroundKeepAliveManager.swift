...
final class BackgroundKeepAliveManager: NSObject, ObservableObject, CLLocationManagerDelegate {
    ...
    /// Solos briefly foregrounds → immediately backgrounds again), stopping the
    ...
    func setup() {
        ...
        // entry clears the badge in `SolosApp.handleScenePhaseChange`.
        Publishers.CombineLatest(
            SessionActivityTracker.shared.$activeSessions,
    ...
    private func reevaluate(sessions: Set<String>, enabled: Bool) {
        ...
        if shouldBeActive && !isActive {
            ...
            // when the user taps it and Solos foregrounds.
    ...
    /// clear the badge separately in `SolosApp.handleScenePhaseChange`.
