...
final class BrowserTabPool: ObservableObject {
    ...
    /// grace window. This is the `solos-browser-use` CLI path: a human/script
    ...
    private func wireManager(_ manager: BrowserUseManager) {
        ...
        // Downloads land in this session's /var/solos/workspace/ so the agent
    ...
    private static func storeURL(for sessionId: String) -> URL {
        let dir = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first!
            .appendingPathComponent("SolosChat/browser_tabs", isDirectory: true)
