...
///
/// Mitigation: before every browser_use / solos-browser-use action (throttled
/// to once per 60s) we
...
final class CookieBackupStore {
    ...
    private static func renderNetscape(_ cookies: [BackupCookie]) -> String {
        var lines = ["# Netscape HTTP Cookie File", "# Written by Solos CookieBackupStore — do not edit while the app runs", ""]
