...
final class BrowserUseManager: NSObject, ObservableObject {
    ...
    /// Downloads are saved into that session's /var/solos/workspace/ directory.
    ...
    /// Shared scheme handler for `solos://` URLs across all browser tabs
    /// and in-app previews that embed a WKWebView.
    static let sharedSolosSchemeHandler = SolosURLSchemeHandler()
    ...
    private static func makeWebView(profile: UserAgentProfile,
        ...
        config.setURLSchemeHandler(Self.sharedSolosSchemeHandler, forURLScheme: "solos")

        // Bridge JS window.print() to the native iOS print dialog. Injected on
        // every page (solos:// and external http/https) at document start so a
    ...
    nonisolated static let printMessageHandlerName = "solosPrint"
    ...
    private func presentPrintDialog() {
        ...
        printInfo.jobName = pageTitle.isEmpty ? "Solos" : pageTitle
    ...
    private func navigate(to urlString: String?) async throws -> BrowserActionResult {
        ...
        // Only allow HTTP(S) and solos:// schemes for direct navigation
        let scheme = url.scheme?.lowercased() ?? ""
        if !["http", "https", "solos"].contains(scheme) {
            return .error("Cannot navigate to \(scheme):// URLs. Only http://, https://, and solos:// are supported.")
        }
    ...
    private func getCookies(keywords: [String]?, fuzzy: Bool) async -> BrowserActionResult {
        ...
        let offloadsDir = RootfsManager.shared.dataPath
            .appendingPathComponent("var/solos/offloads", isDirectory: true)
        ...
        let envGuestPath = "/var/solos/offloads/\(envFilename)"
    ...
    static func wrapExecuteJSReturn(_ script: String) -> String {
        ...
        """
        const __solosNormalize = (function () {
            const MAX_DEPTH = 32;
        ...
        const __solosResult = await (async () => {
        \(script)
        })();
        return __solosNormalize(__solosResult);
        """
    }
...
extension BrowserUseManager: WKNavigationDelegate {
    ...
    nonisolated func webView(
        ...
        let allowedSchemes: Set<String> = ["http", "https", "about", "blob", "solos"]
            ...
            Task { @MainActor [weak self] in
                ...
                if let cont = self.navigationContinuation {
                    ...
                    cont.resume(throwing: URLError(.unsupportedURL, userInfo: [
                        NSLocalizedDescriptionKey: "Blocked navigation to \(scheme):// — only http(s) and solos:// are allowed in the browser."
                    ]))
                }
            }
            return
...
extension BrowserUseManager: WKDownloadDelegate {
    nonisolated func download(
        ...
        Task { @MainActor in
            ...
            let dir = AIChatViewModel.solosWorkspacePersistentDir(for: sid)
            try? FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
...
// MARK: - SolosURLSchemeHandler

/// Handles `solos://` URLs in WKWebView by resolving them to local files.
/// Supports `solos://workspace/file.html`, `solos://shared/...`, etc.
final class SolosURLSchemeHandler: NSObject, WKURLSchemeHandler {
    private let logger = AppLogger(category: "SolosScheme")

    func webView(_ webView: WKWebView, start urlSchemeTask: any WKURLSchemeTask) {
        let url = urlSchemeTask.request.url!
        logger.info("[SolosScheme] start \(url.absoluteString)")

        guard let fileURL = AIChatViewModel.resolveSolosURL(url) else {
            logger.warning("[SolosScheme] not found: \(url.absoluteString)")
            urlSchemeTask.didFailWithError(URLError(.fileDoesNotExist))
            return
        }

        guard let data = try? Data(contentsOf: fileURL) else {
            logger.warning("[SolosScheme] read failed: \(fileURL.path)")
        ...
        logger.info("[SolosScheme] served \(url.absoluteString) → \(fileURL.lastPathComponent) (\(data.count) bytes, \(mimeType))")
    }
...
final class BrowserDownloadCenter: ObservableObject {
    ...
    func began(sessionId: String, filename: String, progress: Progress,
        ...
        queueAgentEvent(sessionId: sessionId, filename: filename,
                        line: "\(filename): download started → saving to solos://workspace/\(filename)")
        return id
    }
    ...
    func finished(id: UUID, sizeText: String) {
        ...
        queueAgentEvent(sessionId: e.sessionId, filename: e.filename,
                        line: "\(e.filename): download COMPLETED (\(sizeText)) → solos://workspace/\(e.filename) (shell path: /var/solos/workspace/\(e.filename))")
    }
    ...
    func agentReport(for sessionId: String) -> String? {
        ...
        var lines: [String] = inflight.map { e in
            ...
            return "\(e.filename): downloading \(pct) (\(done) / \(total)) → saving to solos://workspace/\(e.filename)"
        }
        ...
        return "[browser_downloads] The browser is handling file download(s) NATIVELY "
            ...
            + "shell `ls -l /var/solos/workspace/`) instead of downloading in parallel."
    }
