...
//  SolosApp
//
//  Routes guest paths under /var/solos/{offloads,attachments,workspace,browser}
...
//  hook looks up the token here and rewrites to ~/Library/.../solos/<sid>/<bucket>/...
//

import Foundation

final class SolosFsRouter: @unchecked Sendable {
    static let shared = SolosFsRouter()

    /// Guest path prefixes that route per-session. Anything under one of these
    /// becomes ~/Library/.../solos/<sid>/<bucket>/<tail>. Paths under
    /// /var/solos/{memory,skills,shared} stay global and are NOT listed here —
    /// they fall through to the legacy g_bind_mounts[] table.
    private let perSessionBuckets: [(linuxPrefix: String, hostSubdir: String)] = [
        (AIChatViewModel.solosOffloadsLinuxDir,    "offloads"),
        (AIChatViewModel.solosAttachmentsLinuxDir, "attachments"),
        (AIChatViewModel.solosWorkspaceLinuxDir,   "workspace"),
        (AIChatViewModel.solosBrowserLinuxDir,     "browser"),
    ]
    ...
    /// Host base URL (~/Library/SolosChat/solos). Captured once at install time.
    private let solosBaseURL: URL

    private init() {
        self.solosBaseURL = AIChatViewModel.solosPersistentBase
    }
    ...
    /// Reverse hook: given a host APFS path under <solosBaseURL>/<sid>/<bucket>,
    /// return the canonical guest path /var/solos/<bucket>/<tail>.
    ...
    private func reverse(hostPath: String) -> String? {
        let basePath = solosBaseURL.path
