...
actor ISHExecutionCoordinator {
    ...
    /// handled by SolosFsRouter's hook, not by bind_mount, so they don't
    /// need to be re-mounted on session switch.
    private var staticMountsInitialized: Set<String> = []

    /// Maps linux mount paths (e.g. "/var/solos/memory") to their host
    ...
    /// SolosFsRouter to resolve their host paths.
    ...
    func execute(
        ...
        let fsContext = SolosFsRouter.shared.context(for: sessionId)
    ...
    /// routed by SolosFsRouter's hook now (no bind-mount swap needed), so
    ...
    func mountForSession(_ sessionId: String) {
        ...
        _ = SolosFsRouter.shared.context(for: sessionId)
        ensureStaticMountsInitialized(for: sessionId)
        mountedSessionId = sessionId
    }
    ...
    private func ensureStaticMountsInitialized(for sessionId: String) {
        ...
        // they're hooked by SolosFsRouter.
    ...
    /// Resolve a Linux path under /var/solos/ to its current host URL.
    /// Tries the static bind-mount table first (memory/skills/shared +
    /// external folders), then falls back to SolosFsRouter for the per-
    ...
    func hostURL(for linuxPath: String, sessionId: String? = nil) -> URL? {
        ...
        if let sid = sessionId ?? mountedSessionId,
           let url = SolosFsRouter.shared.hostURL(forGuest: linuxPath, sid: sid) {
            return url
        }
        return nil
    }
    ...
    private func performMount(_ sid: String) {
        ...
        ensureParentDirsInMetaDB(for: "\(AIChatViewModel.solosLinuxBaseDir)/placeholder")
        ensureFakefsMetadata(for: AIChatViewModel.solosLinuxBaseDir, isDirectory: true)
        ...
        // routed dynamically by SolosFsRouter's path-translate hook based on
        ...
        let subdirs: [(persistDir: URL, linuxDir: String)] = [
            (AIChatViewModel.solosMemoryPersistentDir, AIChatViewModel.solosMemoryLinuxDir),
            (AIChatViewModel.solosSkillsPersistentDir, AIChatViewModel.solosSkillsLinuxDir),
            (AIChatViewModel.solosSharedPersistentDir, AIChatViewModel.solosSharedLinuxDir),
            (AIChatViewModel.solosMcpServersPersistentDir, AIChatViewModel.solosMcpServersLinuxDir),
        ]
        ...
        for (idx, (persistDir, linuxDir)) in subdirs.enumerated() {
            ...
            if let hostDir = resolveHostPath(linuxDir) {
                ...
                if lstatOk && (lstatBuf.st_mode & S_IFMT) == S_IFLNK {
                    ...
                    // Safety: only remove paths under data/var/solos/ to avoid accidentally
                    // deleting other fakefs directories (e.g. data/dev/) due to path bugs.
                    let dataVarSolos = RootfsManager.shared.dataPath.appendingPathComponent("var/solos").path
                    guard hostDir.path.hasPrefix(dataVarSolos) else {
                        logger.error("MOUNT [\(idx)] SAFETY: refusing removeItem outside var/solos: \(hostDir.path)")
                        break
                    }
        ...
        // them just like /var/solos/shared.
    ...
    private func mountExternalFoldersLocked() {
        ...
        // Ensure the parent /var/solos/mounts dir exists in meta.db so fakefs
        // can list it.
        ensureParentDirsInMetaDB(for: "\(AIChatViewModel.solosMountsLinuxDir)/placeholder")
        ensureFakefsMetadata(for: AIChatViewModel.solosMountsLinuxDir, isDirectory: true)
