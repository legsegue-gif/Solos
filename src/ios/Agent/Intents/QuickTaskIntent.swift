...
/// e.g. "Solos analyze sleep" (or the same phrase pattern in the user's locale).
...
struct QuickTaskIntent: AppIntent {
    ...
    static var description = IntentDescription("Runs a predefined Solos task like sleep analysis, weather check, or morning briefing.")
    ...
    func perform() async throws -> some IntentResult & ReturnsValue<SendPromptResult> & ProvidesDialog {
        ...
        if sendCompletionNotification {
            ShortcutNotification.post(
                ...
                title: AppLocalized("Solos: \(taskName)"),
        ...
        if waitForResult {
            ...
            if sendCompletionNotification {
                ShortcutNotification.post(
                    ...
                    title: AppLocalized("Solos: \(taskName) Done"),
        ...
        Task { @MainActor in
            ...
            if capturedSendCompletionNotification {
                ShortcutNotification.post(
                    ...
                    title: AppLocalized("Solos: \(capturedTaskName) Done"),
