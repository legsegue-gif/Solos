...
private let logger = AppLogger(category: "AskSolosIntent")

/// Siri-facing "ask Solos" entry point. Unlike `SendPromptIntent` (which runs
...
/// conversation — matching the "Hey Siri, ask Solos to …" experience.
///
...
struct AskSolosIntent: AppIntent {
    static var title: LocalizedStringResource = "Ask Solos"
    static var description = IntentDescription("Opens Solos, sends your prompt, and shows the conversation. Starts a new session, or continues an existing one when you pick a session.")
    ...
    @Parameter(title: "Prompt", requestValueDialog: "What would you like to ask Solos?")
    var prompt: String
    ...
    func perform() async throws -> some IntentResult & ProvidesDialog {
        ...
        if let session = session {
            ...
            if let placeholder = placeholderSid, let realSid = vm.sessionId {
                BackgroundKeepAliveManager.shared.armEagerlyForShortcut(
                    sessionId: realSid, caller: "AskSolosIntent.swap")
                SessionActivityTracker.shared.setInactive(placeholder,
                    source: "AskSolosIntent.eager.placeholderSwap")
            }
        }
        ...
        logger.info("AskSolos send sid=\(sid.prefix(8)) new=\(session == nil)")
        ...
        return .result(dialog: "On it — opening Solos.")
    }
}
