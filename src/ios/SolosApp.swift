...
struct SolosApp: App {
    ...
    /// [review S14] Drives the root-level restore sheet for a `.solosbak`
    ...
    init() {
        ...
        // of ours below `SolosApp.$main()`, i.e. a main-runloop turn during
        ...
        // first render instead of the `.default` stub ("Solos"). Without this the
        ...
        AgentLiveActivityManager.shared.cleanupStaleActivities(source: "SolosApp.init")
            ...
            ZStack {
                ...
                .onReceive(SessionLockStore.shared.$appIsLocked) { locked in
                    ...
                    if BackupOpenRouter.handle(url) {
                        // .solosbak → restore flow, not the attachment pipeline.
                ...
                .id(appLanguage)
                // Confirmation gate for every solos-config write. Mounted
                ...
                // [review S14] Restore flow for a `.solosbak` opened from
                ...
                .sheet(item: Binding(
                    ...
                    NavigationStack {
                        ...
                        // loaded. Someone who just tapped a .solosbak is mid
                ...
                // for `solos://open?session=…&path=…`). Mounted at the
                ...
                .onAppear {
                    ...
                    if #available(iOS 17.0, *) {
                        SolosShortcutsProvider.updateAppShortcutParameters()
                    }
                    ...
                    // which is bind-mounted into the guest at /var/solos/shared
                    ...
                    // Create /var/solos/mounts/<name> symlinks in the fakefs now
    ...
    private func handleScenePhaseChange(_ phase: ScenePhase) {
        ...
        case .background:
            ...
            // its own file, deliberately outside solos.db). Without a checkpoint
    ...
    private static let fileProviderDomain = NSFileProviderDomain(
        identifier: NSFileProviderDomainIdentifier("com.solos.app.files"),
        displayName: "Solos"
    )
    ...
    /// (`AppGroup/SolosConfig/fp-sync-trace.log`, same format as FPSyncTraceLog)
    ...
    private static func logAppUpdateMarkerForFPTrace() {
        ...
        guard let container = fm.containerURL(forSecurityApplicationGroupIdentifier: "group.com.solos.app") else { return }
        let dir = container.appendingPathComponent("SolosConfig", isDirectory: true)
    ...
    private static func registerFileProviderDomain() {
        ...
        let root = AIChatViewModel.solosAppGroupRoot
        let fm = FileManager.default
        ...
        NSFileProviderManager.getDomainsWithCompletionHandler { domains, error in
            ...
            if needsForceReset && alreadyRegistered {
                ...
                // App Group `SolosFileProvider/` path that we hardcoded
                // and that iOS does not own. So `.removeAll` SHOULD leave
                // our data intact, but to be safe we snapshot to
                // `SolosConfig/_reset_backup_v3/` first and restore if we
                ...
                let backupRoot = AIChatViewModel.solosConfigRoot.appendingPathComponent("_reset_backup_v3", isDirectory: true)
                ...
                NSFileProviderManager.remove(fileProviderDomain, mode: .removeAll) { preservedLocation, removeErr in
                    ...
                    let providerRoot = AIChatViewModel.solosAppGroupRoot
                    let fm = FileManager.default
            ...
            let stale = domains.filter { $0.identifier.rawValue.contains("com.solos") }
    ...
    /// 1. Library/SolosChat/shared/ → App Group SolosFileProvider/shared/
    /// 2. Library/SolosChat/memory/ → App Group SolosFileProvider/memory/
    /// 3. Library/SolosChat/skills/ → App Group SolosFileProvider/skills/
    /// 4. Old App Group SolosShared/ → App Group SolosFileProvider/shared/
    private static func migrateSharedDirToAppGroup() {
        let fm = FileManager.default
        let library = fm.urls(for: .libraryDirectory, in: .userDomainMask).first!
        let container = fm.containerURL(forSecurityApplicationGroupIdentifier: "group.com.solos.app")!

        let migrations: [(source: URL, dest: URL, label: String)] = [
            // Legacy Library/SolosChat/shared → new shared
            (library.appendingPathComponent("SolosChat/shared", isDirectory: true),
             AIChatViewModel.solosSharedPersistentDir, "shared"),
            // Legacy Library/SolosChat/memory → new memory
            (library.appendingPathComponent("SolosChat/memory", isDirectory: true),
             AIChatViewModel.solosMemoryPersistentDir, "memory"),
            // Legacy Library/SolosChat/skills → new skills
            (library.appendingPathComponent("SolosChat/skills", isDirectory: true),
             AIChatViewModel.solosSkillsPersistentDir, "skills"),
            // Old App Group SolosShared → new shared
            (container.appendingPathComponent("SolosShared", isDirectory: true),
             AIChatViewModel.solosSharedPersistentDir, "SolosShared→shared"),
        ]
    ...
    private static func logFPSyncTracePaths() {
        let fm = FileManager.default
        let groupID = "group.com.solos.app"
    ...
    /// `solos://open?session=…&path=…` deep link (solos.app launcher
