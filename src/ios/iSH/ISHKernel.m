...
//
// Every guest process lives inside Solos.app's own address space, so a burst
...
#define SOLOS_FORK_GUARD_FOOTPRINT_FRACTION 4          // 1/4 of physical RAM
#define SOLOS_FORK_GUARD_FLOOR_BYTES   (500ULL * 1024 * 1024)
#define SOLOS_FORK_GUARD_CEIL_BYTES    (2048ULL * 1024 * 1024)
#define SOLOS_FORK_GUARD_POLL_US       (50 * 1000)     // 50ms between checks
#define SOLOS_FORK_GUARD_MAX_WAIT_US   (2 * 1000000)   // give up after 2s
...
static uint64_t g_fork_guard_max_footprint = SOLOS_FORK_GUARD_FLOOR_BYTES;

static uint64_t solos_fork_guard_compute_ceiling(void) {
    uint64_t ram = [NSProcessInfo processInfo].physicalMemory;
    if (ram == 0)
        return SOLOS_FORK_GUARD_FLOOR_BYTES;
    uint64_t ceiling = ram / SOLOS_FORK_GUARD_FOOTPRINT_FRACTION;
    if (ceiling < SOLOS_FORK_GUARD_FLOOR_BYTES)
        ceiling = SOLOS_FORK_GUARD_FLOOR_BYTES;
    if (ceiling > SOLOS_FORK_GUARD_CEIL_BYTES)
        ceiling = SOLOS_FORK_GUARD_CEIL_BYTES;
    return ceiling;
}

static void solos_fork_guard_start_pressure_source(void) {
...
static uint64_t solos_current_phys_footprint(void) {
...
static int solos_fork_memory_guard(void) {
    ...
    uint64_t footprint = solos_current_phys_footprint();
    ...
    while (waited_us < SOLOS_FORK_GUARD_MAX_WAIT_US) {
        usleep(SOLOS_FORK_GUARD_POLL_US);
        waited_us += SOLOS_FORK_GUARD_POLL_US;

        footprint = solos_current_phys_footprint();
...
@implementation ISHKernel {
    ...
    NSString *_dnsHostPath; // Host path to resolv.conf (Library/SolosChat/dns/resolv.conf)
...
- (instancetype)init {
    ...
    if (self) {
        ...
        _commandQueue = dispatch_queue_create("com.solosapp.ish.command", DISPATCH_QUEUE_SERIAL);
    }
    return self;
}
...
- (int)bootWithRootPath:(NSString *)rootPath {
    ...
    // the app's footprint into the Jetsam limit (see solos_fork_memory_guard).
    ...
    g_fork_guard_max_footprint = solos_fork_guard_compute_ceiling();
    solos_fork_guard_start_pressure_source();
    ish_set_fork_guard(solos_fork_memory_guard);
    NSLog(@"ISHKernel: [ForkGuard] installed — max footprint %.0fMB "
          ...
          SOLOS_FORK_GUARD_MAX_WAIT_US / 1000);
    ...
    // Registered in every build: the `solos-debug logs` subcommand reads the
    // app's own runtime log in-process (OSLogStore + LoggingManager) and must
    // work on Release devices (T-ios-solos-debug-logs-oslogstore). The
...
/// provider handshakes could never start (GH#175), and the solos-mcp-cli daemon
/// had to fall back to loopback TCP.
///
/// Upstream iSH fixed this in 2019 (`7704024a`) inside `app/AppDelegate.m`.
/// Solos does not compile that file — this class is its equivalent, and it
...
/// The host file lives in Library/SolosChat/dns/resolv.conf and can be freely
/// updated by the app at any time — changes are instantly visible inside iSH.
- (void)mountDnsConfig {
    NSFileManager *fm = [NSFileManager defaultManager];

    // Determine host path: Library/SolosChat/dns/resolv.conf
    NSString *library = [NSSearchPathForDirectoriesInDomains(NSLibraryDirectory, NSUserDomainMask, YES) firstObject];
    NSString *dnsDir = [library stringByAppendingPathComponent:@"SolosChat/dns"];
    ...
    // Bind mount: /etc/resolv.conf -> Library/SolosChat/dns/resolv.conf
...
#define KERNEL_ENVP_APPEND(s) do { \
    ...
    if (envp_pos + _len < sizeof(envp_buf) - 256) { \
        ...
        KERNEL_ENVP_APPEND("BROWSER=/usr/local/bin/solos-open");
...
- (void)handleCommandOutputInternal:(NSString *)output {
    ...
    // Common prompt patterns: "$ ", "# ", "root@solos:", etc.
...
- (void)beginBackgroundCPUGovernor {
    ...
    if (!g_gov_queue)
        g_gov_queue = dispatch_queue_create("com.solos.ish.cpugovernor",
