...
final class TerminalEmulator: ObservableObject {
    ...
    private func handleOSC(command: Int, payload: String) {
        ...
        case 2: // Set Window Title
            title = payload
        case 1337: // iTerm2 proprietary — used by /usr/local/bin/solos-open
            // Payload looks like `SolosOpenURL=https://example.com`. Forward
            // the URL to SolosOpenURLBroker so the host can present the in-app
    ...
    private func handleITermOSC(payload: String) {
        // Payload is `key=value` — we only recognise `SolosOpenURL=<url>`,
        // emitted by `/usr/local/bin/solos-open` in the rootfs overlay.
        guard let eq = payload.firstIndex(of: "="),
              payload[..<eq] == "SolosOpenURL" else { return }
        ...
        Task { @MainActor in
            guard SolosOpenURLBroker.isSupportedScheme(url.scheme) else { return }
            SolosOpenURLBroker.shared.offer(url)
        }
    }
