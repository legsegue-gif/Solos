...
enum WebAppPathClassifier {
    ...
    static func classify(hostURL: URL) -> Classified? {
        ...
        //    Library/SolosChat/solos/<sid>/{attachments,workspace}/...
        let solosBase = AIChatViewModel.solosPersistentBase.standardizedFileURL.path
        if target.hasPrefix(solosBase + "/") {
            let rel = String(target.dropFirst(solosBase.count + 1))
            // <sid>/<subdir>/<rest>
            ...
            if parts.count >= 3 {
                ...
                default:
                    ...
                    classifierLogger.info("classify: solos subdir not supported sub=\(sub) rel=\(rel)")
                    return nil
                }
            }
            return nil
        ...
        let sharedBase = AIChatViewModel.solosSharedPersistentDir.standardizedFileURL.path
