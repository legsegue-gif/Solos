...
enum WebAppPathResolver {
    ...
    static func resolve(_ shortcut: WebAppShortcut) throws -> Resolved {
        ...
        case .sessionAttachment:
            ...
            base = AIChatViewModel.solosAttachmentsPersistentDir(for: sid)
        case .sessionWorkspace:
            guard let sid = shortcut.scopeContext else { throw ResolveError.missingContext }
            base = AIChatViewModel.solosWorkspacePersistentDir(for: sid)
        case .shared:
            base = AIChatViewModel.solosSharedPersistentDir
        case .mount:
        ...
        // (Library/SolosChat/…) has no symlinked segment, so the two shapes
        // happened to agree.
        //
        ...
        case .sessionAttachment, .sessionWorkspace:
            // <solosPersistentBase>/<sid>/   ← parent of attachments/ and workspace/
