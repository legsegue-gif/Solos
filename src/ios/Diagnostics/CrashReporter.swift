...
final class CrashReporter: NSObject, MXMetricManagerSubscriber {
    ...
    /// (Solos.app/Frameworks/SomeTweak.dylib via CydiaSubstrate) variants hit.
    ...
    private func writeReport(
        ...
        if !foreign.isEmpty {
            ...
            report += "\n         (third-party tweak/hook libraries loaded in-process — crashes may originate there, not in Solos)"
        }
    ...
    private func pruneOldReports() {
        let fm = FileManager.default
        let dir = crashReportsDir
        // Only prune crash reports (crash-*.log). Running logs (solos-*.log) now
