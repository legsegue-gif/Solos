...
#define DEFINE_HOOK(NAME, ORIG_VAR) \
    static NSObject *solos_attrQuery_##NAME(id self, SEL _cmd, NSAttributedStringKey name, NSUInteger index, NSRangePointer range) { \
...
+ (void)install {
    ...
    dispatch_once(&once, ^{
        ...
        gOrigConcrete = swizzleOne(concrete, (IMP)solos_attrQuery_Concrete);
        gOrigMutable  = swizzleOne(mutable,  (IMP)solos_attrQuery_Mutable);
        gOrigBase     = swizzleOne(base,     (IMP)solos_attrQuery_Base);
