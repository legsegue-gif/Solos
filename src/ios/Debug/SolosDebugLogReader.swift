...
/// In-process log reader for the `solos-debug logs` CLI subcommand.
///
/// Unlike the rest of the solos-debug CLI (which routes JSON-RPC through
...
///   2. `LoggingManager`'s daily file (`solos-YYYY-MM-DD.log`) — the same NSLog
...
/// (T-ios-solos-debug-logs-oslogstore).
@objc public final class SolosDebugLogReader: NSObject {

    @objc(sharedInstance)
    public static let shared = SolosDebugLogReader()
