...
/// [T-config-debug-rpc] Remote driver for `solos-config`.
///
/// WHY THIS EXISTS. `solos-config` is a binary inside the iSH guest that calls into
...
enum DebugRPCConfig {
    ...
    ///
    /// Mirrors `solos-config get <path> [--filter …] [--page N] [--page-size N]`.
    ...
    /// `config.topics` — every registered topic, i.e. `solos-config --help`'s index.
    ...
    /// `config.topicHelp` — the field schema for one topic, as `solos-config <topic>
