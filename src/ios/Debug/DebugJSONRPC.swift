...
final class DebugJSONRPC: @unchecked Sendable {
    ...
    private func dispatch(method: String, params: [String: Any]) async throws -> Any {
        ...
        // [T-config-debug-rpc] solos-config, driven remotely through the same
    ...
    private func handleVoicePanel(params: [String: Any]) async throws -> Any {
        ...
        case "mic":
            await MainActor.run {
                NotificationCenter.default.post(name: Notification.Name("SolosDebugVoiceMicTap"), object: nil)
            }
        default:
    ...
    ///   zone        (string, default "solos-shared")
    ...
    private func handleSyncCKQuery(params: [String: Any]) async -> Any {
        ...
        let zoneName = (params["zone"] as? String) ?? "solos-shared"
        ...
        let container = CKContainer(identifier: "iCloud.com.solos.app")
        do {
    ...
    /// (solos-shared / solos-devices / solos-secrets) when migration
    ...
    private func handleSyncZoneStats(params: [String: Any]) async -> Any {
        let containerId = "iCloud.com.solos.app"
        ...
        if let z = requestedZone {
            ...
            zonesToScan = [
                ("solos-shared", ["SessionV2", "MessageV2", "CompactMarkerV2", "SessionFileV2", "SkillV2"]),
                ("solos-devices", ["SyncDeviceV2"]),
                ("solos-secrets", ["ProviderConfigV2", "EnvVarV2", "EnvVarItem"]),
    ...
    private func resolveHostPath(_ path: String) -> URL {
        ...
        if stripped.hasPrefix("AppGroup/") {
            let fm = FileManager.default
            if let container = fm.containerURL(forSecurityApplicationGroupIdentifier: "group.com.solos.app") {
    ...
    private func handleShellExecute(params: [String: Any]) async throws -> Any {
        ...
        // fs_context, so /var/solos/{offloads,attachments,workspace,browser}
        // route to that session's bucket. Omit (or pass nil) for the default
        // global view (fs_context = 0).
        let sessionId = params["sessionId"] as? String
        let fsContext: UInt64 = sessionId.map { SolosFsRouter.shared.context(for: $0) } ?? 0
    ...
    private func handleTap(params: [String: Any]) async throws -> Any {
        ...
        // SolosApp's app-embedded synthesis can't reach. iCTRL's isLoopback
    ...
    /// installed XCTest automation service (bundle com.solos.ictrl,
    ...
    static func forwardTapToICTRL(params: [String: Any]) async -> [String: Any]? {
        ...
        // SpringBoard if none). Since debug.tap is called BY SolosApp asking
        // to tap something in ITSELF, pin the query to our own bundle id
        // unless the caller explicitly overrides via ictrl_bundle_id.
        let bundleId = (params["ictrl_bundle_id"] as? String) ?? Bundle.main.bundleIdentifier ?? "com.solos.app"
    ...
    private static func activateCellOrDescendant(_ root: UIView) -> String? {
        ...
        // invocation under SolosCatchObjCException and treat a raise as
        ...
        func attempt(_ label: String, _ body: () -> Bool) -> Bool {
            var accepted = false
            var reason: NSString?
            let ok = SolosCatchObjCException({ accepted = body() }, &reason)
            if !ok {
                caughtReason = reason
    ...
    private func performTargetTap(strategy: String, lookup: () -> DebugViewInspector.TapTarget?) -> [String: Any] {
        ...
        case .control(let control):
            ...
            if !SolosCatchObjCException({ control.sendActions(for: .touchUpInside) }, &sendReason) {
    ...
    private func jsonString(_ obj: Any) -> String {
        ...
        var data: Data?
        var reason: NSString?
        let ok = SolosCatchObjCException({
    ...
    private func knownDatabases() -> [(name: String, url: URL)] {
        // Every app SQLite db lives under Library/SolosChat/ (verified against
        ...
        let fm = FileManager.default
        let library = fm.urls(for: .libraryDirectory, in: .userDomainMask).first!
        let chatBase = library.appendingPathComponent("SolosChat", isDirectory: true)
        let rootfs = RootfsManager.shared.rootfsPath
        return [
            ("solos", chatBase.appendingPathComponent("solos.db")),
            ("skills", chatBase.appendingPathComponent("solos").appendingPathComponent("skills.db")),
