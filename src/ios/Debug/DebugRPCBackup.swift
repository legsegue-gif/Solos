...
enum DebugRPCBackup {
    ...
    /// `debug.backup.restore` — import a `.solosbak` package (stage 2, Merge).
    ///
    /// Params:
    ...
    /// `debug.backup.open` — drive the "opened a .solosbak from Files" entry
    ...
    /// Scoped to Documents and to the `.solosbak` extension so it cannot touch
    /// anything else.
