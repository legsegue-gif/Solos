...
final class DebugAuthenticator: @unchecked Sendable {
    ...
    func openEnvelope(_ obj: [String: Any]) throws -> (plaintext: String, sealResponse: (String) -> String) {
        ...
        let kMac = Self.hmac(key, Data("solos-dbg-v1|mac".utf8) + nonce)
        ...
        let kEncReq = Self.hmac(key, Data("solos-dbg-v1|enc-req".utf8) + nonce)
        ...
        let sealResponse: (String) -> String = { responseJSON in
            let kEncResp = Self.hmac(key, Data("solos-dbg-v1|enc-resp".utf8) + nonce)
    ...
    func handlePair(body: [String: Any], isLoopback: Bool) async -> (status: String, json: String) {
        ...
        if let clientPub {
            do {
                ...
                let kWrap = Self.hmac(sharedData, Data("solos-dbg-pair".utf8) + clientPub + devicePub)
                let pad = Self.hmac(kWrap, Data("solos-dbg-wrap".utf8))
