...
final class LoggingManager: ObservableObject {
    ...
    private func safeWrite(_ data: Data) -> Bool {
        ...
        var reason: NSString?
        let ok = SolosFileHandleSafeWrite(handle, data, &reason)
        if !ok {
    ...
    private func rotateLogFileIfNeeded() {
        ...
        let fileURL = logDirectory.appendingPathComponent("solos-\(today).log")
    ...
    /// pattern (`solos-YYYY-MM-DD.log`, `crash-YYYYMMDD-HHmmss.log`) — the
    ...
    private static func logDate(of url: URL) -> Date? {
        ...
        df.timeZone = .current
        if name.hasPrefix("solos-") {
            df.dateFormat = "yyyy-MM-dd"
            if let d = df.date(from: String(name.dropFirst("solos-".count))) { return d }
    ...
    /// `"running"` for `solos-*.log`, `"crash"` for `crash-*.log` (written by
    ...
    func deleteLog(at url: URL) {
        ...
        writerQueue.async { [weak self] in
            ...
            if url.lastPathComponent == "solos-\(today).log" {
                self.logFileHandle?.closeFile()
                self.logFileHandle = nil
                self.currentLogDate = ""
            }
