...
extension Notification.Name {
    /// Posted by `DeepLinkRouter` when `solos://open?session=…&path=…` lands
    /// from the solos.app launcher. `userInfo["shortcut"]` is a
...
/// Parses `solos://` URLs into navigation actions and dispatches them
...
///   solos://share
///   solos://views/alarm
///   solos://open_terminal[?init_command=…]
///   solos://open?session=<sid>&path=<scope-prefixed-path>  (solos.app launcher round-trip)
///   solos://session/<id>      (legacy singular alias)
///   solos://sessions/<id>     (canonical — matches solos-sessions-cli)
///   solos://settings
///   solos://settings/providers[/<instanceId>]
///   solos://settings/model-groups[/<groupId>]   (alias: model_groups)
///   solos://settings/usage                      (alias: usage-stats, usage_stats)
///   solos://settings/skills
///   solos://settings/memory
///   solos://settings/storage
///   solos://settings/mount-external             (alias: mount_external, mounts, mounted-folders, mounted_folders)
///   solos://settings/shared-folders             (alias: shared_folders)
///   solos://settings/logs
///   solos://settings/appearance
///   solos://settings/background
///   solos://settings/about
///   solos://settings/permissions
///   solos://settings/environments[?create_key=…&create_value=…&create_note=…]
///   solos://settings/rootfs                     (alias: mirrors, rootfs-management, rootfs_management)
///
...
enum DeepLinkRouter {
    ...
    static func handle(url: URL, shareCoordinator: ShareCoordinator) {
        guard url.scheme == "solos", let host = url.host else {
            deepLinkLog.info("ignored — non-solos or missing host: \(url.absoluteString)")
            return
        }
        ...
        case "session", "sessions":
            ...
            // matches solos-sessions-cli) both route to the same place. Post
    ...
    private static func handleSettings(url: URL, coord: DeepLinkCoordinator) {
        ...
        // screen exists — so a `solos://settings/soul` link (or the agent
        // generating one) quietly under-delivered.
        case "soul":
            coord.pendingSettingsTarget = .soul

        // [T-mcp-oauth-deeplink] solos://settings/mcp-servers/<serverId> —
        // jump straight to the server's edit form (Authorize button). The
        // AUTH_REQUIRED error from solos-mcp-cli embeds this link. Server
    ...
    // MARK: - solos.app launcher round-trip

    /// Parses a `solos://open?session=…&path=…` URL fired by the
    /// solos.app launcher when the pinned home-screen tile is
    ...
    private static func handleWebAppLauncherReturn(url: URL) {
        ...
        guard let rawPath = components?.queryItems?.first(where: { $0.name == "path" })?.value,
            ...
            deepLinkLog.warning("solos://open missing path")
            return
        }
        ...
        if rawPath.hasPrefix("attachments/") {
            guard let sid = session, !sid.isEmpty else {
                deepLinkLog.warning("solos://open path=attachments/… missing session")
                return
            }
            ...
            guard let sid = session, !sid.isEmpty else {
                deepLinkLog.warning("solos://open path=workspace/… missing session")
                return
            }
            ...
            guard let slash = rest.firstIndex(of: "/") else {
                deepLinkLog.warning("solos://open path=mount:… missing /<rest>")
                return
            }
            scope = .mount
            ...
            deepLinkLog.warning("solos://open unknown path prefix in \(rawPath)")
            return
        }
        ...
        deepLinkLog.info("solos://open scope=\(scope.rawValue) ctx=\(ctx ?? "nil") htmlPath=\(htmlPath)")
