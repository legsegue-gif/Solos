...
enum ConfigAuditActor: String, Codable {
    /// LLM agent invoked `solos-config <topic> set …`.
    case agent
    ...
    /// e.g. toggling the master `permissions.solosConfig.enabled` switch).
    case user
    /// Agent invoked `solos-config audit revert <id>`.
...
struct ConfigAuditEntry: Identifiable, Codable, Hashable {
    ...
    let revertOf: String?
    /// Caller-supplied description (`solos-config set --caption "..."`),
