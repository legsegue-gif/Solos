...
///
/// Storage: a dedicated `solos-config-audit.db` next to the chat db.
/// Keeping it separate from `solos.db` means the audit table never
...
final class ConfigAuditLog: ObservableObject {
    ...
    init() {
        ...
        let base = library.appendingPathComponent("SolosChat", isDirectory: true)
        try? FileManager.default.createDirectory(at: base, withIntermediateDirectories: true)
        self.dbURL = base.appendingPathComponent("solos-config-audit.db")
        openAndMigrate()
    ...
    /// callable through `solos-config` (would let the agent erase its
    /// own trail).
    func clearAll() {
        guard db != nil else { return }
    ...
    ///
    /// This database is deliberately separate from `solos.db` so the audit table
    /// never lands in a sync-dirty query — but that also left it out of
    /// `ICloudBackupManager.walCheckpoint`, which covers solos.db and skills.db.
    ...
    ///
    /// This is the defensive half of Solos#98 defect 3. The report's "audit
