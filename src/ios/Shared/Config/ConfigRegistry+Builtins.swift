...
/// `SolosApp.onAppear` via `ConfigRegistry.shared.registerBuiltinsIfNeeded()`.
extension ConfigRegistry {
    ...
    // <solosMemoryPersistentDir>/SOUL.md). The four fields below mirror
    ...
    private static func registerSoul(into r: ConfigRegistry) {
        ...
        //
        // An image may be given as a data URI, bare base64, a `solos://`
        // resource, a path inside the solos directories, or an http(s) URL.
        ...
        // `solos-config get` never floods the context with base64, and the
        ...
        r.register(ClosureField(
            ...
            // `solos-config topic-help soul` renders it verbatim, so every
            ...
            description: "The Soul's identity icon — also called its avatar, its persona image, or its "
                ...
                + "  • solos:// resource, e.g. solos://attachments/icon.png or solos://workspace/icon.png\n"
                + "  • local path inside the solos directories, e.g. /var/solos/attachments/icon.png\n"
                ...
                + "  solos-config set soul.icon '\"https://example.com/icon.png\"'\n"
                + "  solos-config set soul.icon '\"data:image/png;base64,iVBORw0KGgo...\"'\n"
                ...
                + "  solos-config set soul.icon --file /tmp/icon-value.json\n"
                + "Simpler still: point at the file directly with solos:// or a path and skip base64 entirely.",
    ...
    private static func parseModelSource(_ s: String,
        ...
        if s.hasPrefix("entry:") {
            ...
            // [T-ios-solos-config-entry-id-composite] Same normalize-then-
            ...
            guard !raw.isEmpty,
                ...
                throw ConfigError.invalidValue(
                    "Unknown model entry id: \(raw) (expected {instanceId}/{modelId} composite form; run `solos-config get models` for valid entry_id values)"
                )
            }
    ...
    private static func registerProviderCollections(into r: ConfigRegistry) {
        ...
        // [T-thinking-rules-solos-config] One `order` field per provider instance.
    ...
    private static func registerDefaults(into r: ConfigRegistry) {
        ...
        r.register(ClosureField(
            ...
            description: "Model entries available via solos-model-use. Replace with full list to set.",
            valueSchema: .array(.string()),
            risk: .sensitive, revertable: true,
            // [T-ios-solos-config-entry-id-composite] Emit normalized ids so
            ...
            writer: { v in
                ...
                // [T-ios-solos-config-entry-id-composite] Entry ids became
                ...
                if !skipped.isEmpty {
                    AppLogger(category: "SolosConfig").info("[SolosConfig] skipped unknown agentLoopEntries ids: \(skipped.joined(separator: ", "))")
                }
        ...
        r.register(ClosureField(
            ...
            writer: { v in
                ...
                if !skipped.isEmpty {
                    AppLogger(category: "SolosConfig").info("[SolosConfig] skipped unknown agentLoopGroups ids: \(skipped.joined(separator: ", "))")
                }
    ...
    /// mirrors the exact condition SolosApp uses to decide which engine to boot.
    @MainActor
    ...
    private static func registerSync(into r: ConfigRegistry) {
        ...
        // solos-config answers with a clear feature_unavailable instead of
        ...
        // `solos-config get sync.enabled` could report `false` while the UI
        ...
        // dead code: SolosApp starts it when v2 is off, and `shouldPauseV1()`
    ...
    private static func registerSelfMeta(into r: ConfigRegistry) {
        ...
        // `SolosConfigPermissionStore.shared.enabled` will short-circuit
        ...
        // /var/solos currently points at, or that the iOS data container UUID
        ...
        r.register(ReadOnlyField(
            ...
            description: "Chat session whose storage /var/solos currently points at. Each session has its own attachments/offloads/workspace — files from other sessions are not visible here. Empty when no session is mounted. Read-only.",
            valueSchema: .string(maxLength: 64),
            reader: {
        ...
        r.register(ClosureField(
            ...
            risk: .destructive,
            revertable: false,
            reader: { .bool(SolosConfigPermissionStore.shared.enabled) },
            writer: { _ in
    ...
    private static func registerBackground(into r: ConfigRegistry) {
        ...
        // where the UI hides the toggle, solos-config returns
