...
/// Funnels every solos-config write through user confirmation.
///
    ...
    private func notifyIfBackgrounded(_ change: PendingConfigChange) {
        ...
        if let caption, !caption.isEmpty {
            ...
            content.body = change.items.count > 1
                ? "\(change.items.count) changes need your review — open Solos to approve or reject."
                : "A change needs your review — open Solos to approve or reject."
        }
