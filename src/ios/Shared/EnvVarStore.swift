...
final class EnvVarStore: ObservableObject {
    ...
    nonisolated private static let keychainService = "com.solos.app.envvar"
    ...
    /// Keychain-stored value. Used by solos-config so the agent can
    ...
    nonisolated func allAsDict() -> [String: String] {
        ...
        let libraryURL = FileManager.default.urls(for: .libraryDirectory, in: .userDomainMask).first!
        let fileURL = libraryURL.appendingPathComponent("SolosChat/env-vars.json")
