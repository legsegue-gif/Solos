...
final class ShareCoordinator: ObservableObject {
    ...
    /// Set to true when a `solos://share` URL is received.
    ...
    /// genuinely arrives twice on every share — SolosApp.onOpenURL and
    ...
    func consumeBuffer() -> PendingShare? {
        ...
        guard age < Self.bufferTTL else {
            ...
            // explicitly shared content into Solos — tell them it didn't make
