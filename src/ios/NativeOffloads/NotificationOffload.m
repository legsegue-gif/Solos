...
static int cmd_schedule(int argc, char **argv, int stdout_fd, int stderr_fd, BOOL compact, BOOL quiet) {
    ...
    if (actionSpec.length) {
        ...
        NSString *categoryId = [@"solos.cli." stringByAppendingString:
    ...
    dispatch_once(&onceToken, ^{
        notifAuthQueue = dispatch_queue_create("com.solos.notification.auth", DISPATCH_QUEUE_SERIAL);
    });
    ...
    if (!granted) {
        NSDictionary *err = noff_json_error(TOOL_NAME, @"schedule",
                                             ...
                                             @"Notification permission not granted. "
                                              "To grant access, open Settings > Notifications > Solos "
