...
//  SolosApp
//
//  Native offload handler for `solos-debug`.
...
//  RetryDiag etc. without attaching Xcode (T-ios-solos-debug-logs-oslogstore).
//
...
#if __has_include("Solos-Swift.h")
#import "Solos-Swift.h"
#elif __has_include("SolosApp-Swift.h")
#import "SolosApp-Swift.h"
#endif
...
static NSString *const TOOL_NAME = @"solos-debug";

static NSString *const HELP_TEXT =
    @"solos-debug - Debug-build CLI for the in-app DebugJSONRPC dispatcher (in-process, no TCP)\n"
     "\n"
     "USAGE:\n"
     "  solos-debug <command> [options]\n"
     "\n"
     "COMMANDS:\n"
     ...
     "\n"
     "EXAMPLES:\n"
     "  solos-debug discover\n"
     "  solos-debug viewTree --maxDepth 4\n"
     "  solos-debug search Chat --scope type\n"
     "  solos-debug inspect 0x10abc1234\n"
     "  solos-debug highlight 0x10abc1234 --color green --duration 1.5\n"
     "  solos-debug ls /var/solos/attachments\n"
     "  solos-debug read /var/solos/log.txt --limit 4096\n"
     "  solos-debug exec ls -la /var/solos\n"
     "  solos-debug snapshot list --type markdown\n"
     "  solos-debug overlay mode --mode byTypePrefix --type Selectable\n"
     "  solos-debug logs --grep StopDiag --last 50\n"
     "  solos-debug logs --minutes 5 --grep RetryDiag\n";
...
/// Read the app's own runtime log via the Swift SolosDebugLogReader bridge
...
static int cmd_logs(int argc, char **argv, int stdout_fd, int stderr_fd, BOOL compact, BOOL quiet) {
    Class reader = NSClassFromString(@"SolosDebugLogReader");
    if (!reader) {
        NSDictionary *err = noff_json_error(TOOL_NAME, @"logs", NOFF_ERR_INTERNAL_ERROR,
                                             @"SolosDebugLogReader bridge unavailable");
    ...
    if (!shared || ![shared respondsToSelector:sel]) {
        NSDictionary *err = noff_json_error(TOOL_NAME, @"logs", NOFF_ERR_INTERNAL_ERROR,
                                             @"SolosDebugLogReader.readLogsJSON missing");
        noff_emit_json(stdout_fd, err, compact, quiet);
        return NOFF_EXIT_ERROR;
    }
...
static NSDictionary *_Nullable call_rpc(NSString *method, NSDictionary *params, NSError **outError) {
    ...
    if (![parsed isKindOfClass:[NSDictionary class]]) {
        if (outError) {
            *outError = [NSError errorWithDomain:@"SolosDebugOffload"
                                            code:-1
...
static int cmd_search(int argc, char **argv, int stdout_fd, int stderr_fd, BOOL compact, BOOL quiet) {
    ...
    if (!kw) {
        NSDictionary *err = noff_json_error(TOOL_NAME, @"search", NOFF_ERR_INVALID_ARGS,
                                             @"search requires a keyword. Usage: solos-debug search <keyword> [--scope all|text|type]");
        noff_emit_json(stdout_fd, err, compact, quiet);
...
static int cmd_inspect(int argc, char **argv, int stdout_fd, int stderr_fd, BOOL compact, BOOL quiet) {
    ...
    if (!addr) {
        NSDictionary *err = noff_json_error(TOOL_NAME, @"inspect", NOFF_ERR_INVALID_ARGS,
                                             @"inspect requires a view address. Usage: solos-debug inspect <address>");
        noff_emit_json(stdout_fd, err, compact, quiet);
        return NOFF_EXIT_INVALID_ARGS;
    }
...
static int cmd_highlight(int argc, char **argv, int stdout_fd, int stderr_fd, BOOL compact, BOOL quiet) {
    ...
    if (!addr) {
        NSDictionary *err = noff_json_error(TOOL_NAME, @"highlight", NOFF_ERR_INVALID_ARGS,
                                             @"highlight requires a view address. Usage: solos-debug highlight <address> [--color red] [--duration 2.0]");
        noff_emit_json(stdout_fd, err, compact, quiet);
        return NOFF_EXIT_INVALID_ARGS;
    }
...
static int cmd_read(int argc, char **argv, int stdout_fd, int stderr_fd, BOOL compact, BOOL quiet) {
    ...
    if (!path) {
        NSDictionary *err = noff_json_error(TOOL_NAME, @"read", NOFF_ERR_INVALID_ARGS,
                                             @"read requires a path. Usage: solos-debug read <path> [--offset N] [--limit N] [--base64]");
        noff_emit_json(stdout_fd, err, compact, quiet);
        return NOFF_EXIT_INVALID_ARGS;
    }
...
static int cmd_write(int argc, char **argv, int stdin_fd, int stdout_fd, int stderr_fd, BOOL compact, BOOL quiet) {
    ...
    if (!path) {
        NSDictionary *err = noff_json_error(TOOL_NAME, @"write", NOFF_ERR_INVALID_ARGS,
                                             @"write requires a path. Usage: solos-debug write <path> --content <text> [--encoding utf8|base64] [--mode 0644]");
        noff_emit_json(stdout_fd, err, compact, quiet);
        return NOFF_EXIT_INVALID_ARGS;
    }
...
static int cmd_exec(int argc, char **argv, int stdout_fd, int stderr_fd, BOOL compact, BOOL quiet) {
    ...
    if (pos.count < 2) {
        NSDictionary *err = noff_json_error(TOOL_NAME, @"exec", NOFF_ERR_INVALID_ARGS,
                                             @"exec requires a command. Usage: solos-debug exec <command...>");
        noff_emit_json(stdout_fd, err, compact, quiet);
        return NOFF_EXIT_INVALID_ARGS;
    }
...
static int cmd_snapshot(int argc, char **argv, int stdout_fd, int stderr_fd, BOOL compact, BOOL quiet) {
    ...
    if (!sub) {
        NSDictionary *err = noff_json_error(TOOL_NAME, @"snapshot", NOFF_ERR_INVALID_ARGS,
                                             @"snapshot requires a subcommand. Usage: solos-debug snapshot <list|get|clear|enable|disable> [--type markdown|messageList] [--id X]");
        noff_emit_json(stdout_fd, err, compact, quiet);
        return NOFF_EXIT_INVALID_ARGS;
    }
...
static int cmd_overlay(int argc, char **argv, int stdout_fd, int stderr_fd, BOOL compact, BOOL quiet) {
    ...
    if (!sub) {
        NSDictionary *err = noff_json_error(TOOL_NAME, @"overlay", NOFF_ERR_INVALID_ARGS,
                                             @"overlay requires a subcommand. Usage: solos-debug overlay <enable|disable|mode> [--mode all|byType|byTypePrefix|byAddress] [--type X] [--address X]");
        noff_emit_json(stdout_fd, err, compact, quiet);
        return NOFF_EXIT_INVALID_ARGS;
    }
    NSString *method = nil;
...
static int hangtest_handler(int argc, char **argv,
    ...
    const char *msg = "solos-hangtest: wedged; send a signal to abort\n";
    ...
    for (int i = 0; i < 600 * 50; i++) {
        if (atomic_load_explicit(&g_hangtest_abort, memory_order_acquire)) {
            ...
            const char *done = "solos-hangtest: aborted\n";
...
void debug_offload_register(void) {
    int err = native_offload_add_handler("solos-debug", debug_handler);
    if (err == 0) {
        noff_ensure_guest_stub("/usr/local/bin/solos-debug");
        NSLog(@"NativeOffloads: solos-debug handler registered (logs available in all builds; RPC subcommands DEBUG-only)");
    } else {
        NSLog(@"NativeOffloads: failed to register solos-debug handler (err=%d)", err);
    }

#if DEBUG
    if (native_offload_add_handler("solos-hangtest", hangtest_handler) == 0) {
        noff_ensure_guest_stub("/usr/local/bin/solos-hangtest");
        native_offload_set_abort_handler("solos-hangtest", hangtest_abort_requested);
        NSLog(@"NativeOffloads: solos-hangtest handler registered (DEBUG only)");
    }
#endif
}
