...
static NSString *const HELP_TEXT =
    @"apple-speech - Speech recognition (audio → text)\n"
     ...
     "  apple-speech transcribe --source /var/solos/attachments/meeting.m4a\n"
...
static dispatch_queue_t authQueue(void) {
    ...
    dispatch_once(&onceToken, ^{
        q = dispatch_queue_create("com.solos.speech.auth", DISPATCH_QUEUE_SERIAL);
    });
    return q;
}
...
static int cmd_transcribe(int argc, char **argv, int stdout_fd, BOOL compact, BOOL quiet) {
    ...
    if (!micSource) {
        ...
        if ([normalized isEqualToString:@"audio-file"] || [normalized isEqualToString:@"audio_file"]) {
            NSDictionary *err = noff_json_error(TOOL_NAME, @"transcribe",
                                                 NOFF_ERR_INVALID_ARGS,
                                                 @"When --source is an audio file, pass the file path directly (e.g. --source /var/solos/attachments/audio.m4a).");
            noff_emit_json(stdout_fd, err, compact, quiet);
            return NOFF_EXIT_INVALID_ARGS;
        }
    ...
    if (!micSource) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (recordingFormat.channelCount == 0 || recordingFormat.sampleRate <= 0) {
                recognitionError = [NSError errorWithDomain:@"SolosSpeech" code:-3 userInfo:@{
            ...
            if (!tapInstalled) {
                recognitionError = [NSError errorWithDomain:@"SolosSpeech" code:-4 userInfo:@{
