...
@objc public class AlarmOffloadBridge: NSObject {
    ...
    // the label visible in the Solos AlarmListView (AlarmRowView already renders
    // `alarm.label` when non-empty).
    //
    // Backed by a tiny SQLite table (AlarmLabelStore) in the same SolosChat/
...
/// `Library/SolosChat/` folder. The connection is opened once with FULLMUTEX
