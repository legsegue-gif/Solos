...
@objc public class BrowserUseOffloadBridge: NSObject {
    ...
    private static func pool(for sid: String) async -> BrowserTabPool? {
        ...
        guard exists else {
            logger.warning("solos-browser-use invoked for deleted session \(sid.prefix(8))")
            return nil
        }
    ...
    /// guaranteed to be the session that owns the current `/var/solos/`
    /// bind mount for the lifetime of this CLI invocation.
    ///
    /// When `withBase64` is false (default), any captured screenshot is
    /// persisted to that session's `/var/solos/browser/` directory and
    /// surfaced via `image_path` + `solos_url` instead of `image_base64`.
    ...
    /// solos_url?, image_base64?, fetched_file?, fetched_bytes?,
    /// fetched_path?, fetched_solos_url?.
    ...
    private static func encode(_ r: BrowserActionResult, withBase64: Bool, sid: String) -> NSDictionary {
        ...
        // out mid-flight. `solosBrowserPersistentDir` is a pure path join
        // against Library/SolosChat/solos/<sid>/browser/, which is exactly
        // what /var/solos/browser/ bind-mounts to for that session.
        let browserHostDir: URL? = (sid == Self.unmountedSentinel)
            ? nil
            : AIChatViewModel.solosBrowserPersistentDir(for: sid)
        ...
        if let b64 = r.base64Image, !b64.isEmpty, let data = Data(base64Encoded: b64) {
            ...
            if let hostDir = browserHostDir {
                ...
                do {
                    ...
                    let linuxPath = "\(AIChatViewModel.solosBrowserLinuxDir)/\(filename)"
                    persistedImagePath = linuxPath
                    out["image_path"] = linuxPath
                    out["solos_url"] = "solos://browser/\(filename)"
                } catch {
                    logger.warning("Failed to persist screenshot to \(dest.path): \(error.localizedDescription)")
                }
            } else {
                logger.warning("No /var/solos/browser mount — falling back to base64-only output")
            }
        }
        ...
        if let name = r.fetchedFileName, !name.isEmpty {
            ...
            if let data = r.fetchedFileData {
                ...
                if let hostDir = browserHostDir {
                    ...
                    do {
                        try data.write(to: dest)
                        let linuxPath = "\(AIChatViewModel.solosBrowserLinuxDir)/\(name)"
                        out["fetched_path"] = linuxPath
                        out["fetched_solos_url"] = "solos://browser/\(name)"
                    } catch {
