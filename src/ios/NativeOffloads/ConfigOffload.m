...
//  SolosApp
//
//  Native offload handler for `solos-config`.
...
//  shell wrapper at /usr/local/bin/solos-config translates the friendly
...
static NSString *const TOOL_NAME = @"solos-config";
...
static NSString *const HELP_TEXT =
    @"solos-config - read or change Solos app settings (logged + revertable)\n"
     "\n"
     "USAGE:\n"
     "  solos-config <subcommand> [args]\n"
     "\n"
     "DISCOVERY:\n"
...
static int cmd_set(int argc, char **argv, int stdout_fd, int stderr_fd,
                   BOOL compact, BOOL quiet) {
    // [T-ios-solos-config-set-shell-escape] (issue #36) `--file <path>` reads
...
void config_offload_register(void) {
    int err = native_offload_add_handler("solos-config", config_handler);
    if (err == 0) {
        noff_ensure_guest_stub("/usr/local/bin/solos-config");
        NSLog(@"NativeOffloads: solos-config handler registered");
    } else {
        NSLog(@"NativeOffloads: failed to register solos-config handler (err=%d)", err);
    }
}
