...
//  SolosApp
//
//  Swift bridge for solos-model-use offload — lists, searches,
...
@objc public class ModelUseOffloadBridge: NSObject {
    ...
    private static func performRun(modelIdOrName: String,
        ...
        if isStreaming {
            ...
            // instead of through the /var/solos/attachments symlink, which
            ...
            if !callerSid.isEmpty {
                attachDir = AIChatViewModel.solosAttachmentsPersistentDir(for: callerSid).path
            } else {
                attachDir = RootfsManager.shared.dataPath
                    .appendingPathComponent("var/solos/attachments").path
            }
            ...
            for (idx, attachment) in response.mediaAttachments.enumerated() {
                ...
                if idx == 0, let outputHostPath, response.text.isEmpty, outputIsMedia {
                    ...
                    // structured file → /var/solos/attachments/.
    ...
    private static func resolveModelEntry(_ idOrName: String, providerFilter: String? = nil) throws -> ModelEntry {
        ...
        if let providerFilter, !providerFilter.isEmpty {
            ...
            if filtered.isEmpty {
                throw ModelUseError.modelNotFound("No provider matches --provider '\(providerFilter)'. Use 'solos-model-use list' to see provider labels.")
            }
            ...
            if let text = msg["content"] as? String {
                ...
                for part in parts {
                    ...
                    if partType == "text", let t = part["text"] as? String {
                        ...
                        // now throws on unresolvable URLs (solos:// to a
    ...
    private static func buildMediaResult(
        ...
        if !callerSid2.isEmpty {
            attachDir = AIChatViewModel.solosAttachmentsPersistentDir(for: callerSid2).path
        } else {
            attachDir = RootfsManager.shared.dataPath
                .appendingPathComponent("var/solos/attachments").path
        }
    ...
    ///   - `solos://<scope>/<path>` — resolve to host storage via the
    ...
    ///   - `/var/solos/<scope>/<path>` — bare Linux paths under
    ///     `/var/solos/`, mapped to the same scopes as `solos://`.
    ...
    private static func resolveImageURL(_ url: String) throws -> LLMMessage.ImageAttachment {
        ...
        // solos:// — route through the in-app solos URL resolver so
        // attachments/workspace/shared/etc. all work without the caller
        // knowing host paths.
        if url.hasPrefix("solos://") {
            guard let parsed = URL(string: url),
                  let resolved = AIChatViewModel.resolveSolosURL(parsed) else {
                throw ModelUseError.invalidInput("Could not resolve solos:// URL '\(url)' — file not found in any session scope. Try /var/solos/<scope>/<path> or file:///<host-path>.")
            }
            guard let data = FileManager.default.contents(atPath: resolved.path) else {
                throw ModelUseError.invalidInput("solos:// URL '\(url)' resolved to \(resolved.path) but the file is unreadable")
            }
            let mime = Self.imageMimeForExtension(url)
            return LLMMessage.ImageAttachment(mimeType: mime, data: data)
        }

        // /var/solos/<scope>/<path> — Linux bare path equivalent of
        // solos://<scope>/<path>. Rewrite to a solos:// URL and reuse
        // the same resolver so behavior is identical.
        if url.hasPrefix("/var/solos/") {
            let stripped = String(url.dropFirst("/var/solos/".count))
            // Split into scope + subpath. The solos URL "host" is the
            // first path component.
            let parts = stripped.split(separator: "/", maxSplits: 1).map(String.init)
            guard !parts.isEmpty, !parts[0].isEmpty else {
                throw ModelUseError.invalidInput("/var/solos/ path must include a scope (attachments, workspace, shared, etc.): '\(url)'")
            }
            let scope = parts[0]
            let sub = parts.count > 1 ? parts[1] : ""
            let solosURLStr = sub.isEmpty ? "solos://\(scope)" : "solos://\(scope)/\(sub)"
            guard let parsed = URL(string: solosURLStr),
                  let resolved = AIChatViewModel.resolveSolosURL(parsed) else {
        ...
        if url.hasPrefix("http://") || url.hasPrefix("https://") {
            throw ModelUseError.invalidInput("http(s):// image URLs are not supported by solos-model-use. Download first with `shell_execute` (curl/wget) into /var/solos/workspace/, then reference the local path.")
        }
        ...
        if url.hasPrefix("/") {
            ...
            throw ModelUseError.invalidInput("Image file not found at '\(url)'. For solos-scope files prefer /var/solos/<scope>/<path> or solos://<scope>/<path>.")
        }

        throw ModelUseError.invalidInput("Unsupported image_url '\(url)'. Use a data: URL, file:///host/path, /var/solos/<scope>/<path>, or solos://<scope>/<path>.")
    }

    /// Read a file given a Linux-side path. Tries the iSH rootfs data
    /// directory first (where bind-mounted /var/solos/* and /tmp/* land
    ...
    /// e.g. "/private/var/mobile/.../Documents/alpine-rootfs/data/var/solos/foo" → "/var/solos/foo"
    ...
    /// [T-model-use-media-path-mismatch] Derive the guest (`/var/solos/...`)
    ...
    ///   1. Session-persistent dirs under app Library (`.../solos/<sid>/{attachments,
    ...
    ///      via the per-session symlinks `ensureSolosSymlinks` maintains
    ///      (`/var/solos/attachments` → `.../<sid>/attachments`, etc.). Map by
    ///      matching the persistent-dir prefix for THIS caller session and
    ///      substituting the corresponding `/var/solos/<name>` root, preserving
    ///      any sub-path + filename.
    ///   2. A path already inside rootfs `dataPath` (e.g. a user `--output`
    ///      under `/var/solos/...` resolved by the CLI). `toLinuxPath` strips
    ///      it directly.
    ///
    /// The previous code hardcoded `/var/solos/attachments/<filename>` in the
    ...
    private static func guestLogicalPath(forSavedFile filePath: String, callerSid: String) -> String {
        ...
        if !callerSid.isEmpty {
            let mappings: [(persistDir: String, linuxDir: String)] = [
                (AIChatViewModel.solosAttachmentsPersistentDir(for: callerSid).path,
                 AIChatViewModel.solosAttachmentsLinuxDir),
                (AIChatViewModel.solosWorkspacePersistentDir(for: callerSid).path,
                 AIChatViewModel.solosWorkspaceLinuxDir),
                (AIChatViewModel.solosOffloadsPersistentDir(for: callerSid).path,
                 AIChatViewModel.solosOffloadsLinuxDir),
            ]
        ...
        return AIChatViewModel.solosAttachmentsLinuxDir + "/" + (filePath as NSString).lastPathComponent
    }
...
enum ModelUseError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .modelNotFound(let id):
            return "Model '\(id)' not found. Use 'solos-model-use list' to see available models."
        case .invalidInput(let msg):
            return msg
        case .modalityNotSupported(let model, let required, let supported):
            return "Model '\(model)' does not support \(required). Supported modalities: \(supported.joined(separator: ", ")). Use 'solos-model-use list' to find a model with the required capability."
        }
    }
}
