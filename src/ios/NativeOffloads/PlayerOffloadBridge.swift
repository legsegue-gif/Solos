...
//  using the shared UI components (SolosAudioPreviewView, SolosVideoFullscreenPlayer).
//
...
@MainActor @objc public class PlayerOffloadBridge: NSObject {
    ...
    // MARK: - Audio (delegates to GlobalAudioPlayer + SolosAudioPreviewView)
    ...
    private static func openAudioSession(_ session: PlayerSession, completion: @escaping (NSDictionary?, NSString?) -> Void) {
        ...
        // Present SolosAudioPreviewView
        presentAudioUI(for: session)
        completion(data, nil)
    }

    // MARK: - Video (own AVPlayer + SolosVideoFullscreenPlayer)
    ...
    private static func presentAudioUI(for session: PlayerSession) {
        ...
        let view = SolosAudioPreviewView(fileURL: session.fileURL)
    ...
    private static func presentVideoUI(for session: PlayerSession) {
        ...
        let view = SolosVideoFullscreenPlayer(fileURL: session.fileURL, externalPlayer: player)
        let hostingVC = UIHostingController(rootView: view)
