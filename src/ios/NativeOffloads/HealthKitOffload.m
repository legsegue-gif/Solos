...
static NSOperationQueue *authQueue(void) {
    ...
    dispatch_once(&onceToken, ^{
        ...
        q.name = @"com.solos.healthkit.auth";
    });
    return q;
}
...
static BOOL requestHealthKitAccess(NSSet<HKObjectType *> *readTypes,
    ...
    if (!granted && outError) {
        ...
        *outError = [NSString stringWithFormat:
            @"%@. To grant access, open Settings > Health > Data Access & Devices > Solos "
...
static NSString *const kHKOffloadErrorDomain = @"SolosHealthKitOffload";
    ...
    if (!saveWithAuthRetry(bpCorrelation, sysType, &saveErr)) {
        ...
        if (authDenied) {
            msg = [NSString stringWithFormat:
                @"%@. Open Settings > Health > Data Access & Devices > Solos "
