...
static NSString *const HELP_TEXT =
    @"apple-photos - Query and manage the iOS photo library\n"
     ...
     "  export        Export a photo/video to /var/solos/offloads/\n"
     ...
     "                       Accepts guest paths (/var/solos/attachments/a.jpg),\n"
     ...
     "                       Accepts guest paths (/var/solos/...), paths relative\n"
     ...
     "  apple-photos import /var/solos/offloads/pic.jpg\n"
     "  apple-photos import --path /var/solos/offloads/pic.jpg\n"
     "  apple-photos import --path /var/solos/offloads/pic.jpg --album-name \"My Album\"\n"
     "  apple-photos save --path /var/solos/offloads/clip.mov --album <album-id>\n"
...
static dispatch_queue_t authQueue(void) {
    ...
    dispatch_once(&onceToken, ^{
        q = dispatch_queue_create("com.solos.photos.auth", DISPATCH_QUEUE_SERIAL);
    });
    return q;
}
...
static int cmd_export(int argc, char **argv, int stdout_fd, int stderr_fd, BOOL compact, BOOL quiet) {
    ...
    NSString *guestDir = @"/var/solos/offloads";
...
//   1. Guest-absolute: "/var/solos/attachments/foo.jpg" → mapped into fakefs data dir.
