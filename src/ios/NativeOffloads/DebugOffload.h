...
//  SolosApp
//
//  Native offload handler for `solos-debug`. RPC-backed subcommands are
...
//  and is available in ALL builds (T-ios-solos-debug-logs-oslogstore), so the
...
/// Register the solos-debug native handler. Registered in every build so the
