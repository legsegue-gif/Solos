...
//  SolosApp
//
//  Native offload handler for `solos-sessions-cli`.
...
static NSString *const TOOL_NAME = @"solos-sessions-cli";

static NSString *const HELP_TEXT =
    @"solos-sessions-cli - Query and send Solos chat sessions\n"
     "\n"
     "USAGE:\n"
     "  solos-sessions-cli <command> [options]\n"
     "\n"
     "COMMANDS:\n"
     ...
     "\n"
     "EXAMPLES:\n"
     "  solos-sessions-cli list\n"
     "  solos-sessions-cli search --keywords \"API error\"\n"
     "  solos-sessions-cli messages --id <session_id> --full\n"
     "  solos-sessions-cli send \"分析我的睡眠数据\"\n"
     "  solos-sessions-cli send \"继续\" --session <session_id>\n"
     "  solos-sessions-cli send \"这张图是什么?\" --attach /tmp/photo.png\n"
     "  solos-sessions-cli retry --session <session_id>\n"
     "  solos-sessions-cli status --id <session_id>\n"
     "  solos-sessions-cli open <session_id>\n";
    ...
    if (!prompt || prompt.length == 0) {
        ...
        NSDictionary *err = noff_json_error(TOOL_NAME, @"send",
                                             ...
                                             @"send requires a prompt argument. "
                                              "Example: solos-sessions-cli send \"hello\"");
...
static int cmd_open(int argc, char **argv, int stdout_fd, int stderr_fd,
    ...
    // spec command shape `solos-sessions-cli open <session_id>`) or an
    ...
    if (!sessionId || sessionId.length == 0) {
        ...
        NSDictionary *err = noff_json_error(TOOL_NAME, @"open",
                                             ...
                                             @"open requires a session id. "
                                              "Example: solos-sessions-cli open <session_id>");
        noff_emit_json(stdout_fd, err, compact, quiet);
        return NOFF_EXIT_INVALID_ARGS;
    }
    ...
    if (!keywords || keywords.count == 0) {
        ...
        NSDictionary *err = noff_json_error(TOOL_NAME, @"search",
                                             ...
                                             @"--keywords is required for search. "
                                              "Example: solos-sessions-cli search --keywords \"API error\"");
        noff_emit_json(stdout_fd, err, compact, quiet);
        return NOFF_EXIT_INVALID_ARGS;
    }
...
void sessions_offload_register(void) {
    int err = native_offload_add_handler("solos-sessions-cli", sessions_handler);
    if (err == 0) {
        noff_ensure_guest_stub("/usr/local/bin/solos-sessions-cli");
        NSLog(@"NativeOffloads: solos-sessions-cli handler registered");
    } else {
        NSLog(@"NativeOffloads: failed to register solos-sessions-cli handler (err=%d)", err);
    }
}
