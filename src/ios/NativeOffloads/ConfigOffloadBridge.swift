...
@objc public class ConfigOffloadBridge: NSObject {
    ...
    /// `permissions.solosConfig.enabled` is on (default). Off means
    /// every CLI call short-circuits with exit 126.
    @objc public static func isEnabled() -> Bool {
        let sem = DispatchSemaphore(value: 0)
        var enabled = true
        Task.detached { @MainActor in
            enabled = SolosConfigPermissionStore.shared.enabled
            sem.signal()
        }
        sem.wait()
        return enabled
    ...
    /// JSON envelope for the "solos-config is disabled" error path.
    ...
    @objc public static func disabledErrorEnvelope() -> NSDictionary {
        return [
            ...
            "reason": "solos-config is disabled in Settings → Permissions.",
            "user_message": "I tried to change a setting but solos-config is currently disabled. You can enable it at [Settings → Permissions](solos://settings/permissions), then ask me again. Or change the setting yourself directly through the relevant Settings screen.",
        ]
    }
    ...
    /// prints these in `solos-config --help`.
    ...
    private static func paginationHint(path: String,
        ...
        if page > totalPages {
            return "Page \(page) is out of range (only \(totalPages) page\(totalPages == 1 ? "" : "s") available). Try: solos-config get \(path)\(filterFragment) --page \(totalPages)"
        }
        if totalPages <= 1 {
            return "Showing all \(total) item\(total == 1 ? "" : "s") (1 page)."
        }
        if page < totalPages {
            return "Showing page \(page) of \(totalPages) (\(pageCount) of \(total) items). To get more, use: solos-config get \(path)\(filterFragment) --page \(page + 1) --page-size \(pageSize)"
        }
    ...
    private static func readFieldSync(path: String) -> NSDictionary {
        ConfigRegistry.shared.registerBuiltinsIfNeeded()
        guard SolosConfigPermissionStore.shared.enabled else {
        ...
        if field.access == .hidden {
            return [
                "ok": false,
                "error": "permission_denied",
                "reason": "'\(path)' is intentionally not exposed to solos-config.",
            ]
        }
    ...
    private static func performWriteBatch(items: [NSDictionary],
        ...
        for raw in items {
            ...
            // This is how `solos-config set models.append <json>` and
            // `solos-config set models.remove <entry-id>` reach
            // ModelsCollection.add / .remove (T-solos-config-models-add).
            ...
            // [T-solos-config-provider-add] If this field holds a credential
        ...
        case .approved(let approvedRows):
            ...
            for (idx, row) in approvedRows.enumerated() {
                ...
                do {
                    ...
                    case .fieldWrite(let field, let oldValue, let newValue, let auditNewValue):
                        ...
                        // the response. [T-solos-config-provider-add]
            ...
            return [
                ...
                "applied": applied,
                "audit_ids": auditIds,
                "audit_url": "solos://settings/logs?tab=config-audit",
                "user_message": "Settings updated. Review or revert at [Logs → Config Changes](solos://settings/logs?tab=config-audit).",
            ]
        }
    }
    ...
    @objc public static func auditRevert(id: String,
        ...
        Task.detached { @MainActor in
            ...
            let res = await Self.performWriteBatch(
                items: [revertItem],
                caption: "solos-config audit revert \(entry.id.prefix(8))",
                actorRaw: actorRaw,
                sessionId: sessionId,
                skipConfirmation: skipConfirmation
            )
