...
//  SolosApp
//
//  Native offload handler for `solos-browser-use`.
...
static NSString *const TOOL_NAME = @"solos-browser-use";
...
static NSString *const HELP_TEXT =
    @"solos-browser-use - Drive the in-app WebView from the shell\n"
     ...
     "                   /var/solos/browser/ and referenced via image_path +\n"
     "                   solos_url, so shells don't get flooded with base64.\n"
     ...
     "                      /var/solos/browser/screenshot_<ms>.jpg\n"
     ...
     "  solos-browser-use screenshot --full-page\n"
...
static NSDictionary *buildInputJson(int argc, char **argv, NSString **errOut) {
    ...
    // (mirrors `solos-config set --file`). Parse failures are surfaced as an
...
void browser_use_offload_register(void) {
    int err = native_offload_add_handler("solos-browser-use", browser_use_handler);
    if (err == 0) {
        noff_ensure_guest_stub("/usr/local/bin/solos-browser-use");
        NSLog(@"NativeOffloads: solos-browser-use handler registered");
    } else {
        NSLog(@"NativeOffloads: failed to register solos-browser-use handler (err=%d)", err);
    }
}
