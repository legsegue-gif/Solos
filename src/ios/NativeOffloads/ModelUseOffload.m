...
//  SolosApp
//
//  Native offload handler for `solos-model-use`.
...
static NSString *const TOOL_NAME = @"solos-model-use";

static NSString *const HELP_TEXT =
    @"solos-model-use - List, search, and invoke LLM models\n"
     "\n"
     "USAGE:\n"
     "  solos-model-use <command> [options]\n"
     "\n"
     "COMMANDS:\n"
     ...
     "  solos-model-use list --modality audio\n"
     "  solos-model-use search gemini\n"
     "  solos-model-use search gemini --modality video\n"
     "  solos-model-use run --model GPT-5.5 --prompt 'Summarize: ...'\n"
     "  solos-model-use run --model GPT-5.5 --prompt-file /var/solos/workspace/text.txt\n"
     ...
     "  solos-model-use run --model gemini-2.5-flash --system 'You are a poet' --input msgs.json --output /var/solos/workspace/out.json\n";

// ── Path helpers ──

// Resolve a caller-supplied *input* file path to a readable host path.
//
// solos-model-use runs on the host, not inside the iSH guest, so guest-absolute
// paths have to be mapped into the fakefs data root. Guest paths under the
// bind-mounted prefixes (/var/solos/, /home/, /tmp/) are tried there first;
...
static NSString *_Nullable noff_resolve_existing_input_path(NSString *path) {
    ...
    if ([path hasPrefix:@"/var/solos/"] || [path hasPrefix:@"/home/"] || [path hasPrefix:@"/tmp/"]) {
...
static int cmd_search(int argc, char **argv, int stdout_fd, int stderr_fd, BOOL compact, BOOL quiet) {
    ...
    if (positional.count < 1) {
        NSDictionary *err = noff_json_error(TOOL_NAME, @"search",
                                             ...
                                             @"No search query provided. Usage: solos-model-use search <query>");
...
static int cmd_run(int argc, char **argv, int stdin_fd, int stdout_fd, int stderr_fd,
    ...
    if (!modelIdOrName) {
        NSDictionary *err = noff_json_error(TOOL_NAME, @"run",
                                             NOFF_ERR_INVALID_ARGS,
                                             @"--model is required. Usage: solos-model-use run --model <id_or_name>");
        noff_emit_json(stdout_fd, err, compact, quiet);
        return NOFF_EXIT_INVALID_ARGS;
    }
    ...
    // "You are Solos, an on-device AI assistant running on iOS." — a bare
    // identity assertion. Sub-models took it literally: they answered "who are
    // you" as Solos and invented a matching vendor (reproduced across three
    // providers, Solos#103). The damage is not limited to identity
    ...
    if (!systemPrompt) {
        systemPrompt = @"You are being invoked as a sub-agent inside an app called Solos. "
    ...
    // path. solos-model-use forwards to the host, which does NOT inherit the
    ...
    if (outputPath) {
        if (![outputPath hasPrefix:@"/"]) {
            NSString *msg = [NSString stringWithFormat:
                @"--output must be an absolute path, got \"%@\". solos-model-use runs on the host and does not inherit the shell's current directory, so relative paths can't be resolved. Use an absolute path like /var/solos/workspace/out.jpg.",
                outputPath];
            NSDictionary *err = noff_json_error(TOOL_NAME, @"run",
        ...
        if ([outputPath hasPrefix:@"/var/solos/"] || [outputPath hasPrefix:@"/home/"] || [outputPath hasPrefix:@"/tmp/"]) {
    ...
    if (!stream) {
        ...
        // caller can SEE what was prepended (Solos#103 option 3). The
...
void model_use_offload_register(void) {
    int err = native_offload_add_handler("solos-model-use", model_use_handler);
    if (err == 0) {
        noff_ensure_guest_stub("/usr/local/bin/solos-model-use");
        NSLog(@"NativeOffloads: solos-model-use handler registered");
    } else {
        NSLog(@"NativeOffloads: failed to register solos-model-use handler (err=%d)", err);
    }
}
