...
/// Regression tests for `.solosbak` format acceptance and manifest tolerance
...
final class BackupFormatCompatibilityTests: XCTestCase {
    ...
    func testFormatAcceptanceMatrix() {
        ...
        XCTAssertTrue(BackupImporter.isFormatSupported("solosbak/1"))
        // Same major with a minor component — §2.2 rule 2 requires this to
        // import. The pre-fix code REFUSED it (Int("1.1") == nil).
        XCTAssertTrue(BackupImporter.isFormatSupported("solosbak/1.1"))
        XCTAssertTrue(BackupImporter.isFormatSupported("solosbak/1.0.3"))
        ...
        XCTAssertFalse(BackupImporter.isFormatSupported("solosbak/2"))
        XCTAssertFalse(BackupImporter.isFormatSupported("solosbak/2.0"))
    }

    func testFormatAcceptanceRejectsMalformedStrings() {
        XCTAssertFalse(BackupImporter.isFormatSupported(""))
        XCTAssertFalse(BackupImporter.isFormatSupported("solosbak"))
        XCTAssertFalse(BackupImporter.isFormatSupported("solosbak/"))
        XCTAssertFalse(BackupImporter.isFormatSupported("solosbak/x"))
        XCTAssertFalse(BackupImporter.isFormatSupported("/1"))
        // Case matters: the prefix is compared literally.
        XCTAssertFalse(BackupImporter.isFormatSupported("SolosBak/1"))
    }
    ...
    func testUnknownFieldsAreIgnored() throws {
        let m = try decode("""
        {"format":"solosbak/1","device_name":"iPhone",
         "future_field":{"nested":[1,2,3]},"another":"x"}
        """)
        XCTAssertEqual(m.deviceName, "iPhone")
        XCTAssertEqual(m.format, "solosbak/1")
    }
    ...
    func testSnakeCaseKeysAreTheWireFormat() throws {
        let m = try decode("""
        {"format":"solosbak/1","device_name":"A","backup_id":"B",
    ...
    func testKDFAlgAndSaltAreRequired() {
        let base = """
        {"format":"solosbak/1","encryption":{"scheme":"solosbak-enc/1",
         "verifier":"dg==","kdf":{%@}}}
        """
