...
final class CorrectionAdmissionTests: XCTestCase {
    ...
    func testLatinPhoneticSimilarity() {
        ...
        // The reported bug: linux→solos. Skeletons lnx/mns — same syllable count
        // (2), "n" anchored at index 1, skeleton similarity 0.33 ≥ 0.30.
        XCTAssertTrue(f("linux", "solos"))
        ...
        XCTAssertFalse(f("solos", "minutes"))   // 2 vs 3 — a rewrite, correctly dropped
    ...
    func testLinuxSolosAdmitted() {
        // The specific user report, end-to-end through judge().
        XCTAssertEqual(judge("linux", "solos"), .latinPhonetic)
    ...
    func testRealDataAdmissionMatrix() {
        let shouldCollect: [(String, String)] = [
            ...
            ("linux", "solos"),   // Signal 4 — the GH report; nothing else catches it
        ]
