// Tests for [T-backup-package-name-device] — `.solosbak` filenames are now
// `<device>-<yyyyMMdd>-<sortable-id>.solosbak` so that packages from several
...
//
// Standalone (`swift BackupPackageNameTests.swift`) because the SolosTests
...
func packageFileName(backupId: String, at date: Date, deviceName: String,
    ...
    return "\(device)-\(stamp)-\(sortableID(backupId: backupId, at: date))\(suffix).solosbak"
}
...
do {
    checkEq("apostrophe + space", filenameDeviceToken("Sam's iPhone"), "Sams-iPhone")
    ...
    var allSafe = true
    for raw in ["Sam's iPhone", "a/b\\c:d*e?f\"g<h>i|j", "小王的 iPad", "🚀 phone", "   "] {
...
do {
    let t = Date(timeIntervalSince1970: 1_787_000_000)
    let name = packageFileName(backupId: "F69C0012-3456", at: t, deviceName: "Sam's iPhone")
    check("starts with the device", name.hasPrefix("Sams-iPhone-"))
    check("ends with the extension", name.hasSuffix(".solosbak"))
    ...
    let fields = simple.replacingOccurrences(of: ".solosbak", with: "").split(separator: "-")
    ...
    // point of the change.
    let a = packageFileName(backupId: "same", at: t, deviceName: "Sam's iPhone")
    let b = packageFileName(backupId: "same", at: t, deviceName: "Sam's iPad")
...
do {
    ...
    check("marker sits before the extension", enc.hasSuffix("-encrypted.solosbak"))
    ...
    check("still ends in .solosbak", enc.hasSuffix(".solosbak"))
    ...
    let fields = packageFileName(backupId: "abc", at: t, deviceName: "iPhone",
                                 encrypted: true)
        .replacingOccurrences(of: ".solosbak", with: "").split(separator: "-")
...
do {
    ...
    let t = Date(timeIntervalSince1970: 1_787_000_000)
    let a = packageFileName(backupId: "same-id", at: t, deviceName: "Sam Work iPhone")
    let b = packageFileName(backupId: "same-id", at: t, deviceName: "Sam Home iPhone")
    check("two custom names → different filenames", a != b)
    check("custom name leads the filename", a.hasPrefix("Sam-Work-iPhone-"))
