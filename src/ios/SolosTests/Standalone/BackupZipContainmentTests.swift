...
// path is a symlink.
//
// Standalone (`swift BackupZipContainmentTests.swift`) because the SolosTests
...
for (label, name) in [("wrapper + nested file", "solosbak-UUID/data/messages.jsonl"),
                      ("blob path", "solosbak-UUID/blobs/ab/abcdef0123.blob"),
                      ("unicode filename", "solosbak-UUID/文件/résumé.md"),
                      ("empty directory entry", "solosbak-UUID/blobs/"),
                      ("deep non-existent leaf", "solosbak-UUID/a/b/c/d.json"),
                      ("symlink staying inside root", "inner/ok.txt"),
                      ("dot in filename", "solosbak-UUID/data/a.b.c.jsonl"),
                      ("space in filename", "solosbak-UUID/my file.txt")] {
