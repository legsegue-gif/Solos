// Tests for [T-soul-icon-config-images] — `solos-config set soul.icon` now
// accepts images (data URI, bare base64, solos:// resource, local path,
// http(s) URL) and must funnel every one of them through the SAME rules the
// Settings picker uses.
//
// Standalone (`swift SoulIconSourceTests.swift`) because the SolosTests target
...
func looksLikeImageSource(_ raw: String) -> Bool {
    ...
    if s.hasPrefix("data:") || s.hasPrefix("solos://") { return true }
...
for (label, s, want) in [
    ...
    ("solos attachments", "solos://attachments/icon.png", true),
    ("solos workspace", "solos://workspace/icon.png", true),
    ("https", "https://example.com/icon.png", true),
    ("http", "http://example.com/icon.png", true),
    ("absolute path", "/var/solos/attachments/icon.png", true),
...
for needle in ["data:image/png;base64,iVBORw0KGgo",
               "solos://attachments/icon.png",
               "solos://workspace/icon.png",
               "/var/solos/attachments/icon.png",
...
check("help gives a runnable quoted example",
      builtinsSource.contains("solos-config set soul.icon '\\\"solos://attachments/icon.png\\\"'"))
