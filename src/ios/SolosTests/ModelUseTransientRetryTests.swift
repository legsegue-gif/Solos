...
/// the CLI-level bounded backoff for `Solos.LLMError.transientError` (Gemini
...
final class ModelUseTransientRetryTests: XCTestCase {
    ...
    private func transient(_ n: Int) -> Solos.LLMError {
    ...
    func testTransientErrorExhaustsRetriesThenThrows() async {
        ...
        } catch let Solos.LLMError.transientError(message) {
    ...
    func testProviderErrorFailsFastWithoutRetry() async {
        ...
        do {
            _ = try await ModelUseOffloadBridge.withTransientRetry(
                ...
                attempts += 1
                throw Solos.LLMError.providerError(message: "400 INVALID_ARGUMENT")
            }
            XCTFail("expected providerError")
        } catch let Solos.LLMError.providerError(message) {
    ...
    /// Non-Solos.LLMError failures are also untouched (no retry, error passes
    /// through unchanged).
    ...
    func testCanRetryFalseSurfacesTransientErrorImmediately() async {
        ...
        } catch Solos.LLMError.transientError {
            // expected
        } catch {
            XCTFail("unexpected error type: \(error)")
        }
