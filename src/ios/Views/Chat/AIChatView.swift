...
private let solosLogger = AppLogger(category: "SolosURL")
    ...
    init(sessionId: String?) {
        if let sessionId {
            ...
            if !isNew, ViewModelCache.shared.consumeStaleFlag(sessionId: sessionId) {
                solosLogger.info("🔑DRAFT CachedViewModel.init sessionId=\(sessionId) — STALE flag consumed, scheduling reload")
                Task { @MainActor [weak vm = cached] in
                    await vm?.loadSession()
                }
            }
            solosLogger.info("🔑DRAFT CachedViewModel.init sessionId=\(sessionId) vm=\(cached.vmInstanceId) isNew=\(isNew)")
        } else {
            let draft = ViewModelCache.shared.createDraft()
            self.vm = draft
            self.isNew = true
            solosLogger.info("🔑DRAFT CachedViewModel.init DRAFT vm=\(draft.vmInstanceId)")
        }
...
struct AIChatView: View {
    ...
    /// from SOUL.md (`name`), falls back to "Solos". Refreshed on .soulMdChanged.
    @State private var soulName: String = SoulStore.cachedMetadata.name.isEmpty
        ? "Solos" : SoulStore.cachedMetadata.name
    ...
    // solos:// link preview sheet state — hoisted from SolosOpenURLHandler so
    // sheet presentation originates from a stable window-hierarchy root and
    // plays its slide-up transition correctly. See SolosOpenURLHandler for rationale.
    ...
    /// Filename to show in the "file missing" alert. Set by `handleSolosURLTap`
    /// when `resolveSolosFileURL` returns nil for a tapped `solos://` link
    /// (file was deleted, the session was pruned, or iCloud hasn't synced yet).
    @State private var missingSolosFileName: String?
    ...
    var body: some View {
        ZStack {
            ...
            messagesArea
                ...
                .onChange(of: vm.isLoadingSession) { newValue in
                    ...
                    // chat.
                    solosLogger.warning("[SpinnerTrace] isLoadingSession → \(newValue) sid=\(vm.sessionId?.prefix(8) ?? "nil") msgs=\(vm.messages.count)")
                }
        ...
        .sheet(item: $locateDownloadTarget) { target in
            if let sid = vm.sessionId {
                NavigationStack {
                    FileBrowserView(
                        rootPath: AIChatViewModel.solosWorkspacePersistentDir(for: sid),
                        rootLabel: "/var/solos/workspace",
        ...
        .environment(\.solosURL, SolosURLAction { url in
            handleSolosURLTap(url)
        })
        ...
        .fullScreenCover(item: $previewImageFile) { fileURL in
            SolosImageFilePreviewView(fileURL: fileURL)
        }
        .alert(
            AppLocalized("File Not Found"),
            isPresented: Binding(
                get: { missingSolosFileName != nil },
                set: { if !$0 { missingSolosFileName = nil } }
            ),
            presenting: missingSolosFileName
        ) { _ in
            Button(AppLocalized("OK"), role: .cancel) { missingSolosFileName = nil }
        } message: { name in
        ...
        .fullScreenCover(item: $previewVideoFile) { fileURL in
            SolosVideoFullscreenPlayer(fileURL: fileURL)
        }
        .sheet(item: $previewAudioFile) { fileURL in
            SolosAudioPreviewView(fileURL: fileURL)
                .presentationDetents([.large])
                .presentationDragIndicator(.hidden)
        }
        .sheet(item: $previewTextFile) { fileURL in
            SolosTextPreviewView(fileURL: fileURL)
        }
        .sheet(item: $previewMarkdownFile) { fileURL in
            SolosMarkdownPreviewView(fileURL: fileURL)
        }
        .sheet(item: $previewDocumentFile) { fileURL in
            SolosDocumentPreviewView(fileURL: fileURL)
        }
        .sheet(item: $shareFile) { fileURL in
            SolosShareSheet(url: fileURL)
        }
        .sheet(item: $safariURL) { url in
            SolosLinkPreviewView(url: url, browserPool: vm.browserTabPool, onExpand: { _ in
        ...
        // an OSC SolosOpenURL marker (via /usr/local/bin/solos-open).
        //
        // Dispatch by scheme:
        ...
        //  - solos://...: chat-resource file preview (image/markdown/html/
        //    pdf/...). Routed through handleSolosURLTap which already
        ...
        .onReceive(SolosOpenURLBroker.shared.$pendingURL.dropFirst().compactMap { $0 }) { url in
            if SolosOpenURLBroker.isWebScheme(url.scheme) {
                ...
                guard !SolosOpenURLBroker.shared.toolSheetVisible,
                      !SolosOpenURLBroker.shared.terminalVisible else { return }
                withAnimation { safariURL = url }
            } else {
                _ = handleSolosURLTap(url)
            }
            SolosOpenURLBroker.shared.consume()
        }
        .sheet(item: $previewHTMLFile) { fileURL in
            SolosHTMLPreviewView(fileURL: fileURL, onExpand: { _ in
        ...
        .fullScreenCover(item: $fullBrowserURL) { url in
            ...
            SolosSafariView(url: url, localFile: wasLocal, onCollapse: {
                fullBrowserURL = nil
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.4) {
                    if wasLocal {
                        previewHTMLFile = url
        ...
        .sheet(isPresented: $showFileBrowser) {
            NavigationStack {
                ...
                FileBrowserView(rootPath: base, initialPath: base.appendingPathComponent("var/solos"), rootLabel: "/")
            }
        }
        ...
        .sheet(isPresented: $showMoveToSheet) {
            MoveToSessionSheet(currentSessionId: vm.sessionId) { targetId in
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + ViewModelCache.PendingTransfer.staleAfter) {
                    ...
                    solosLogger.info("[MoveTo] Transfer to \(targetId) was never consumed — restoring content to source session")
        ...
        .fullScreenCover(isPresented: $showTerminal) {
            ...
            NavigationStack {
                ISHTerminalView(sessionId: vm.sessionId, showCloseButton: true, initCommand: terminalInitCommand)
                    .onAppear {
                        if let sid = vm.sessionId {
                            solosLogger.info("🔍MOUNT Terminal onAppear — re-mounting solos for session \(sid)")
                            vm.mountSolos(for: sid)
                        } else {
                            solosLogger.info("🔍MOUNT Terminal onAppear — no sessionId, skipping mount")
                        }
                    }
            }
        }
        .fullScreenCover(isPresented: $showCamera, onDismiss: {
            solosLogger.info("[QuickAction] fullScreenCover(camera) onDismiss showCamera=\(showCamera)")
        }) {
            ...
            AnyView(
                CameraPicker { image in
                    solosLogger.info("[QuickAction] CameraPicker onCapture size=\(Int(image.size.width))x\(Int(image.size.height))")
                    vm.addImageAttachment(image)
                }
                .ignoresSafeArea()
                .onAppear {
                    solosLogger.info("[QuickAction] fullScreenCover(camera) content onAppear — notifying workflow")
        ...
        .fileImporter(
            ...
            case .failure(let error):
                solosLogger.error("File import failed: \(error.localizedDescription)")
            }
        }
        .onAppear {
            let sinceInit = (CFAbsoluteTimeGetCurrent() - AIChatViewModel.onAppearTimestamp) * 1000
            solosLogger.info("[SessionLoad] onAppear T+\(String(format: "%.0f", sinceInit))ms isNew=\(cached.isNew) msgs=\(vm.messages.count)")
            ...
            if let sessionId {
                if cached.isNew || (vm.messages.isEmpty && !vm.isLoadingSession) {
                    ...
                    solosLogger.info("🔄SESSION AIChatView.onAppear loading session \(sessionId) isNew=\(cached.isNew) msgs=\(vm.messages.count)")
                    ...
                    solosLogger.info("🔄SESSION AIChatView.onAppear WAITING for in-flight load session=\(sessionId)")
                    Task {
                        ...
                        if vm.messages.isEmpty && !vm.isLoadingSession {
                            solosLogger.warning("🔄SESSION AIChatView.onAppear RETRY load — messages still empty after in-flight load for \(sessionId)")
                            await vm.loadSession()
                        }
                    }
                } else {
                    let reuseStart = CFAbsoluteTimeGetCurrent()
                    solosLogger.info("🔄SESSION AIChatView.onAppear REUSING cached vm for \(sessionId) isProcessing=\(vm.isProcessing) msgs=\(vm.messages.count)")
                    // Remount solos for this session (in case another session took over)
                    vm.mountSolos(for: sessionId)
                    ...
                    solosLogger.info("[SessionLoad] \(sessionId) — REUSE: \(String(format: "%.1f", totalElapsed))ms [mount: \(String(format: "%.1f", mountElapsed)) | msgs: \(vm.messages.count)]")
                }
            } else {
                solosLogger.info("🔄SESSION AIChatView.onAppear nil sessionId — draft mode")
            ...
            solosLogger.info("[Share] AIChatView.onAppear: sessionId=\(sessionId ?? "nil") bufferVersion=\(shareCoordinator.bufferVersion) hasBuffer=\(shareCoordinator.pendingShareBuffer != nil)")
        ...
        .onChange(of: shareCoordinator.bufferVersion) { newVersion in
            ...
            solosLogger.info("[Share] AIChatView.onChange(bufferVersion)=\(newVersion) sessionId=\(sessionId ?? "nil") draftId=\(draftId ?? "nil") hasBuffer=\(shareCoordinator.pendingShareBuffer != nil)")
            injectPendingShareIfNeeded()
        }
        .onDisappear {
            ...
            solosLogger.info("🔑DRAFT AIChatView.onDisappear vm=\(vm.vmInstanceId) sessionId=\(sessionId ?? "nil") draftId=\(draftId ?? "nil") vm.sessionId=\(vm.sessionId ?? "nil") vm.isProcessing=\(vm.isProcessing)")
        }
    ...
    private func tryMarkWorkflowChatReady(reason: String) {
        ...
        if case .waitingForChatMount(_, let target) = QuickActionWorkflow.shared.state, target == sid {
            solosLogger.info("[QuickAction] tryMarkWorkflowChatReady reason=\(reason) sid=\(sid)")
    ...
    private func applyQuickAction(_ action: ChatLaunchAction) {
        solosLogger.info("[QuickAction] applyQuickAction action=\(String(describing: action)) showCamera=\(showCamera) hasOverlay=\(hasOverlayPresented) chatVisible=\(isChatViewVisible)")
        ...
        guard isChatViewVisible else {
            solosLogger.info("[QuickAction] applyQuickAction skipped — view not visible")
            return
        }
        switch action {
        case .startVoice:
        ...
        case .openCamera:
            ...
            inputFocused = false
            showCamera = true
            solosLogger.info("[QuickAction] openCamera — showCamera=true")
        }
    }
    ...
    private func injectPendingShareIfNeeded() {
        ...
        guard shareCoordinator.bufferTargets(sessionId, draftId: draftId) else {
            solosLogger.info("[Share] injectPendingShareIfNeeded — buffer is addressed to another session (mine: sessionId=\(sessionId ?? "nil") draftId=\(draftId ?? "nil")); leaving it")
            return
        }
        guard let pending = shareCoordinator.consumeBuffer() else {
            solosLogger.info("[Share] injectPendingShareIfNeeded — no buffer or expired")
            return
        }
        solosLogger.info("[Share] Injecting \(pending.items.count) items into chat")

        for (i, item) in pending.items.enumerated() {
            switch item.kind {
            case .inlineText:
                solosLogger.info("[Share] item[\(i)] inlineText: \(String(item.value.prefix(100)))")
            ...
            case .attachment:
                guard let dir = SharedContainerStore.sharedFileDirectory else {
                    solosLogger.error("[Share] item[\(i)] sharedFileDirectory is nil!")
                    continue
                }
                let fileURL = dir.appendingPathComponent(item.value)
                let exists = FileManager.default.fileExists(atPath: fileURL.path)
                solosLogger.info("[Share] item[\(i)] attachment: \(item.value) exists=\(exists) path=\(fileURL.path)")
                ...
                if pendingProviderImport == nil, let json = Self.providerExportJSON(at: fileURL) {
                    ...
                    solosLogger.info("[Share] item[\(i)] detected Provider export JSON — prompting import vs attachment")
                    continue
                }
                vm.addFileAttachment(from: fileURL)
                solosLogger.info("[Share] item[\(i)] addFileAttachment done, attachments count=\(self.vm.attachments.count)")
            }
        }
        ...
        solosLogger.info("[Share] Injection complete. inputText='\(String(self.vm.inputText.prefix(100)))' attachments=\(self.vm.attachments.count)")
    ...
    private func injectPendingTransferIfNeeded() {
        ...
        ViewModelCache.pendingTransfer = nil
        solosLogger.info("[MoveTo] Injecting transfer: text='\(String(transfer.inputText.prefix(50)))' attachments=\(transfer.attachments.count) existing=\(vm.attachments.count)")
        // Clean up any stale unsent attachments on the target VM before injecting
        if !vm.attachments.isEmpty {
            solosLogger.info("[MoveTo] Clearing \(vm.attachments.count) stale attachments from target session")
    ...
    private var titleView: some View {
        ...
        // the SOUL.md `name` (or "Solos") for draft sessions or before a
        ...
        // (only descender-less titles like "Solos" hid it). -3 keeps a ~2pt
        ...
        Task {
            ...
            await MainActor.run {
                ...
                if next?.id != titlePillSession?.id
                    ...
                    // old "Solos" string slide before the real title swaps
    ...
    private var messagesArea: some View {
        ZStack {
            CollectionViewMessageListV3(
                ...
                // handler every `solos://` link in the transcript uses, so the
                // identity row and an agent-authored
                // `[Soul](solos://settings/soul)` link land identically —
                // no second navigation path to keep in sync.
                onOpenSoulSettings: {
                    guard let url = URL(string: "solos://settings/soul") else { return }
                    _ = handleSolosURLTap(url)
                },
            ...
            // per-session vs cross-session /var/solos/ layout. Only shown
    ...
    // in SolosApp beside AudioPiPCapsule) so it persists across chat → home. No
    ...
    /// Handle a solos:// or http(s):// URL tap forwarded from a cell-level
    ...
    private func handleMarkdownImageTap(_ note: Notification) {
        ...
        for message in vm.messages {
            ...
            for block in message.blocks {
                ...
                imagePattern?.enumerateMatches(in: block.content, options: [], range: range) { match, _, _ in
                    ...
                    if solosVideoExtensions.contains(ext) || solosAudioExtensions.contains(ext) { return }
        ...
        let items: [GalleryItem] = refs.enumerated().map { (index, ref) in
            ...
            let fpKey = solosMediaCacheKey(for: canonical)
            return GalleryItem(
    ...
    /// Load a markdown image source (already canonicalized to solos:// or
    ...
    private static func loadMarkdownImage(source: String, fingerprintKey: String) async -> UIImage? {
        ...
        let img: UIImage?
        if url.scheme == "solos" {
            guard let fileURL = resolveSolosFileURLCached(url: url),
    ...
    private func handleSolosURLTap(_ url: URL) -> OpenURLAction.Result {
        if let scheme = url.scheme?.lowercased(), scheme == "http" || scheme == "https" {
            withAnimation { safariURL = url }
            return .handled
        }
        guard url.scheme == "solos" else { return .systemAction }

        // Terminal deep link: solos://open_terminal?init_command=...
        ...
        // View deep links: solos://views/alarm
        if url.host == "views" && url.path == "/alarm" {
            DeepLinkCoordinator.shared.showAlarmList = true
            return .handled
        }
        // Any solos://settings/<path> goes through DeepLinkRouter so all
        ...
        if let fileURL = resolveSolosFileURL(url: url) {
            let ext = url.pathExtension.lowercased()
            withAnimation {
                if solosImageExtensions.contains(ext) {
                    previewImageFile = fileURL
                } else if solosVideoExtensions.contains(ext) {
                    previewVideoFile = fileURL
                } else if solosAudioExtensions.contains(ext) {
                    previewAudioFile = fileURL
                } else if solosMarkdownExtensions.contains(ext) {
                    previewMarkdownFile = fileURL
                } else if solosTextExtensions.contains(ext) {
                    previewTextFile = fileURL
                } else if solosHTMLExtensions.contains(ext) {
                    previewHTMLFile = fileURL
                } else if solosDocumentExtensions.contains(ext) {
            ...
            missingSolosFileName = name.isEmpty ? url.absoluteString : name
        }
        return .handled
    }

    private func handleDropProviders(_ providers: [NSItemProvider]) {
        solosLogger.info("[Drop] handleDropProviders called with \(providers.count) provider(s)")
        for (index, provider) in providers.enumerated() {
            let types = provider.registeredTypeIdentifiers
            solosLogger.info("[Drop] provider[\(index)] types=\(types) suggestedName=\(provider.suggestedName ?? "nil")")
            ...
            solosLogger.info("[Drop] provider[\(index)] hasFileType=\(hasFileType) hasFileName=\(hasFileName)")

            guard hasFileType || hasFileName else {
                solosLogger.info("[Drop] provider[\(index)] SKIPPED — no file type or filename")
                continue
            }
            ...
            solosLogger.info("[Drop] provider[\(index)] hasFileURL=\(hasFileURL) canLoadImage=\(canLoadImage)")

            if hasFileURL {
                provider.loadItem(forTypeIdentifier: UTType.fileURL.identifier) { item, error in
                    if let error {
                        solosLogger.error("[Drop] provider[\(index)] loadItem fileURL error: \(error.localizedDescription)")
                        return
                    }
                    guard let data = item as? Data,
                          let url = URL(dataRepresentation: data, relativeTo: nil) else {
                        solosLogger.error("[Drop] provider[\(index)] loadItem fileURL failed to parse — item type: \(type(of: item))")
                        return
                    }
                    solosLogger.info("[Drop] provider[\(index)] fileURL loaded: \(url.path)")
            ...
            if canLoadImage {
                provider.loadObject(ofClass: UIImage.self) { item, error in
                    if let error {
                        solosLogger.error("[Drop] provider[\(index)] loadObject UIImage error: \(error.localizedDescription)")
                        return
                    }
                    guard let image = item as? UIImage else {
                        solosLogger.error("[Drop] provider[\(index)] loadObject UIImage returned nil")
                        return
                    }
                    solosLogger.info("[Drop] provider[\(index)] UIImage loaded: \(image.size)")
            ...
            if let contentType = types.first(where: { id in
                ...
                solosLogger.info("[Drop] provider[\(index)] fallback loadFileRepresentation contentType=\(contentType)")
                provider.loadFileRepresentation(forTypeIdentifier: contentType) { url, error in
                    if let error {
                        solosLogger.error("[Drop] provider[\(index)] loadFileRepresentation error: \(error.localizedDescription)")
                        return
                    }
                    guard let url else {
                        solosLogger.error("[Drop] provider[\(index)] loadFileRepresentation returned nil URL")
                        return
                    }
                    ...
                    do {
                        ...
                        solosLogger.info("[Drop] provider[\(index)] file copied to: \(tmp.path)")
                        DispatchQueue.main.async { vm.addFileAttachment(from: tmp) }
                    } catch {
                        solosLogger.error("[Drop] provider[\(index)] copyItem failed: \(error.localizedDescription)")
                    }
                }
                continue
            }
            solosLogger.info("[Drop] provider[\(index)] NO handler matched — dropped")
        }
    }
    ...
    private func performSend() {
        ...
        solosLogger.info("🔑DRAFT performSend vm=\(vm.vmInstanceId) vm.sessionId=\(vm.sessionId ?? "nil") draftId=\(draftId ?? "nil") inputText='\(String(vm.inputText.prefix(50)))' isProcessing=\(vm.isProcessing)")
...
/// `/var/solos/` as a 2-column folder grid so the user understands what's
/// available before typing.
///
/// Source-of-truth for the descriptions: AIChatViewModel.swift system-prompt
/// directory listing (`Shared directory /var/solos/ ...`). Keep them aligned
