...
struct SolosVideoPlayerView: View {
    let url: URL
    let fileURL: URL
    ...
    private func notifyFileReadyOnce() {
        ...
        NotificationCenter.default.post(name: .solosAttachmentSizeChanged, object: url.absoluteString)
    }
    ...
    private func loadThumbnail() {
        ...
        if let cached = SolosMediaCache.shared.thumbnail(for: cacheKey) {
            thumbnail = cached
        ...
        generator.generateCGImagesAsynchronously(
            ...
            if let cgImage {
                ...
                SolosMediaCache.shared.setThumbnail(img, for: cacheKey)
...
struct SolosVideoFullscreenPlayer: View {
    let fileURL: URL
    ...
    var body: some View {
        ZStack {
            Color.black.opacity(1.0 - Double(max(dragOffset, 0) / dismissThreshold) * 0.4)
                .ignoresSafeArea()
            if let player {
                SolosAVPlayerLayerView(player: player)
                    .ignoresSafeArea()
                    .offset(y: dragOffset)
            }
...
struct SolosAVPlayerLayerView: UIViewRepresentable {
    let player: AVPlayer
