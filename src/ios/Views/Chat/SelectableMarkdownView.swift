...
private let imgLogger = AppLogger(category: "SolosImage")
...
// MARK: - solos:// URL encoding [T-solos-url-fullwidth-pipe-ios]

/// Percent-encode the path of a `solos://` destination that contains raw,
/// unencoded bytes (e.g. a filename with U+FF5C `｜`, CJK, emoji the LLM emitted
/// unencoded inside a solos:// link), returning a parseable URL (or nil if it
...
func encodeRawSolosURL(_ destination: String) -> URL? {
    let prefix = "solos://"
...
extension NSAttributedString.Key {
    static let inlineCodeBackground = NSAttributedString.Key("solosInlineCodeBackground")
    static let inlineCodeText = NSAttributedString.Key("solosInlineCodeText")
    static let blockquoteDepth = NSAttributedString.Key("solosBlockquoteDepth")
}
...
final class NativeMediaImageCache {
    ...
    private static let defaultsKey = "SolosImageSizes_v1"
    ...
    // 28 SolosImage + 19 AttachHotPath events on a single 115ms landing frame,
...
private let solosInlineCodeBackgroundColor = UIColor { traits in
    traits.userInterfaceStyle == .dark
        ? UIColor(red: 0x3A / 255.0, green: 0x3A / 255.0, blue: 0x3C / 255.0, alpha: 1)
        : .systemGray6
}

/// Mirrors the `.solosChat` MarkdownUI theme using UIKit types.
struct SelectableMarkdownTheme {
    ...
    var inlineCodeBackground: UIColor { solosInlineCodeBackgroundColor }
...
fileprivate final class MarkdownNSRenderer {
    ...
    /// solosChat list item: .em(0.25) top margin
    private var listItemTopMargin: CGFloat { theme.baseFontSize * 0.25 }
    /// solosChat headings: .em(1) top and bottom
    ...
    /// was 27 s of main-thread typesetter pin (ips Solos-2026-05-14-094312).
    ...
    private func renderHeading(level: Int, content: [InlineNode], quoteDepth: Int) -> RenderedBlock {
        ...
        let style = NSMutableParagraphStyle()
        // solosChat doesn't set relativeLineSpacing for headings
        style.lineSpacing = 0
        if quoteDepth > 0 {
    ...
    private func renderBlockquote(children: [BlockNode], listDepth: Int, quoteDepth: Int) -> RenderedBlock {
        ...
        if fullRange.length > 0 {
            ...
            // solosChat blockquote uses secondaryText color
            result.addAttribute(.foregroundColor, value: theme.secondaryLabelColor, range: fullRange)
        }
        // Blockquote inherits margins from children (no explicit markdownMargin in solosChat)
        return RenderedBlock(
            attributed: result,
    ...
    private func renderBulletedList(isTight: Bool, items: [RawListItem], listDepth: Int, quoteDepth: Int) -> RenderedBlock {
        ...
        // device hit a fresh 0x8BADF00D watchdog (ips Solos-2026-05-14-
    ...
    private func renderImageAttachment(source: String) -> NSAttributedString {
        ...
        if let cached = imageAttachmentCache[source] {
            ...
            imgLogger.info("[SolosImage][RenderAttach] REUSE cached attachment src=\(source) loaded=\(cached.loadedImage != nil) imgSize=\(cached.loadedImage.map { "\($0.size.width)x\($0.size.height)" } ?? "nil")")
            ...
            let currentFp = solosMediaCacheKey(for: source)
            if let loadedFp = cached.loadedFingerprint, loadedFp != currentFp {
                ...
                imgLogger.info("[SolosImage][RenderAttach] FINGERPRINT CHANGED src=\(source) old=\(loadedFp) new=\(currentFp) — invalidating cached image")
            ...
            imgLogger.info("[SolosImage][RenderAttach] CREATE new ImageAttachment src=\(source) cacheCount=\(self.imageAttachmentCache.count)")
    ...
    /// solos_url (e.g. `solos://workspace/GitHub%E7%83%AD….md`). When the model
    ...
    private func normalizedLinkURL(from destination: String) -> URL? {
        ...
        // Only solos:// links are affected; everything else is untouched.
        guard destination.hasPrefix("solos://") else {
            return fallback
        }
        // [T-solos-url-fullwidth-pipe-ios] URL(string:) returned nil — on iOS
        ...
        // emitted unencoded inside a solos:// link). The link would otherwise be
        ...
        guard let base = fallback else {
            return encodeRawSolosURL(destination)
        }
        // A correctly singly-encoded solos_url decodes cleanly: `base.path`
    ...
    private func renderInline(_ node: InlineNode, attributes attrs: [NSAttributedString.Key: Any]) -> NSAttributedString {
        ...
        case .code(let code):
            ...
            // Set .backgroundColor to trigger fillBackgroundRectArray in SolosLayoutManager
        ...
        case .image(let source, let children):
            ...
            imgLogger.info("[SolosImage][InlineParse] .image node src=\(source) ext=\(ext) alt=\(altText)")
            if nativeAudioExts.contains(ext) {
                ...
                imgLogger.info("[SolosImage][InlineParse] routing to IMAGE attachment src=\(source)")
                return renderImageAttachment(source: source)
            } else {
                // Unknown extension — render as image attachment (best guess)
                imgLogger.info("[SolosImage][InlineParse] unknown ext=\(ext), routing to IMAGE attachment (best guess) src=\(source)")
                return renderImageAttachment(source: source)
            }
    ...
    /// Per-depth indent: 3pt bar + 10pt leading padding (matches solosChat blockquote padding)
...
final class TableAttachment: NSTextAttachment {
    ...
    private func renderCellInline(_ node: InlineNode, font: UIFont) -> NSAttributedString {
        ...
        case .code(let code):
            ...
            //     SolosLayoutManager whose fillBackgroundRectArray draws the
            ...
            // Both resolve solosInlineCodeBackgroundColor at draw time and
            // repaint on trait changes, so light/dark stay correct.
            codeAttrs[.inlineCodeBackground] = true
            codeAttrs[.backgroundColor] = solosInlineCodeBackgroundColor
    ...
    func makeView(width: CGFloat) -> UIView {
        ...
        let copyTable: () -> Void = { [weak self] in
            ...
            SolosToast.show(AppLocalized("Table copied"))
        }
        ...
        let copyTableImage: () -> Void = { [weak stack] in
            ...
            SolosToast.show(AppLocalized("Table image copied"))
        }
...
private final class TableCellTextView: UITextView, UITextViewDelegate {
    ...
    /// SolosLayoutManager so inline-code spans (marked with
    ...
    convenience init() {
        ...
        let layoutManager = SolosLayoutManager()
...
private final class TableCellLabel: UILabel, UIGestureRecognizerDelegate, UIContextMenuInteractionDelegate {
    ...
    // keeps `.backgroundColor` so its SolosLayoutManager painter still fires.
    ...
    private func paintInlineCodeBackgrounds() {
        ...
        let bgColor = solosInlineCodeBackgroundColor.resolvedColor(with: traitCollection)
        ctx.saveGState()
...
final class ImageAttachment: NSTextAttachment {
    ...
    /// Set when a solos:// file was not found; allows retry on next render.
    ...
    /// a URL scheme (`http`, `https`, `solos`, `file`, …) are returned
    /// unchanged. Bare filenames and relative paths (e.g. `foo.png`,
    /// `./foo.png`, `subdir/bar.jpg`) become `solos://workspace/<path>`
    ...
    static func canonicalizeMarkdownImageSource(_ src: String) -> String {
        ...
        if let colon = src.firstIndex(of: ":") {
            ...
            if !maybeScheme.isEmpty && maybeScheme.allSatisfy({ $0.isLetter }) {
                // [T-solos-url-fullwidth-pipe-ios] Exception: a solos:// source the
                ...
                // encoded URLs (encodeRawSolosURL decodes-then-encodes per segment).
                if src.hasPrefix("solos://"), URL(string: src) == nil,
                   let fixed = encodeRawSolosURL(src) {
        ...
        return "solos://workspace/\(encoded)"
    }
    ...
    private static func probeAndPublishSize(at url: URL, canonicalSrc: String) {
        ...
        imgLogger.info("[SolosImage][Probe] header size=\(Int(size.width))x\(Int(size.height)) src=\(canonicalSrc) — publishing before full decode")
        DispatchQueue.main.async {
            NotificationCenter.default.post(name: .solosAttachmentSizeChanged, object: canonicalSrc)
        }
    }
    ...
    private func adoptLoaded(_ image: UIImage, fingerprint: String, canonicalSrc: String, via: String) {
        ...
        isLoading = false
        fileNotFound = false
        imgLogger.info("[SolosImage][Adopt] via=\(via) src=\(canonicalSrc) size=\(Int(image.size.width))x\(Int(image.size.height))")
        onLoad?()
    ...
    func beginLoadingIfNeeded() {
        // Canonicalize scheme-less / relative sources to solos://workspace/…
        ...
        if canonicalSrc != source {
            imgLogger.info("[SolosImage][Load] canonicalized src=\(self.source) → \(canonicalSrc)")
        }
        ...
        // of the same solos:// path misses and falls through to a fresh decode.
        let fpKey = solosMediaCacheKey(for: canonicalSrc)
        ...
        guard loadedImage == nil, !isLoading else {
            imgLogger.info("[SolosImage][Load] skip src=\(self.source) alreadyLoaded=\(self.loadedImage != nil) isLoading=\(self.isLoading) — the in-flight completion or the next render's cache check will adopt")
            return
        }
        // Reset fileNotFound so we don't spin; it will be set again if still missing.
        fileNotFound = false
        guard retriesRemaining > 0 else {
            imgLogger.warning("[SolosImage][Load] no retries left src=\(self.source)")
            return
        }
        retriesRemaining -= 1
        isLoading = true
        ...
        imgLogger.info("[SolosImage][Load] START gen=\(gen) src=\(canonicalSrc) scheme=\(parsedURL?.scheme ?? "nil") host=\(parsedURL?.host ?? "nil") path=\(parsedURL?.path ?? "nil") ext=\(parsedURL?.pathExtension ?? "nil") retriesRemaining=\(self.retriesRemaining)")

        let src = canonicalSrc
        let isSolosURL = URL(string: src).map { $0.scheme == "solos" } ?? false
        // [T-ios-failed-image-refetch-storm] For remote (non-solos) URLs that
        // failed recently, don't re-dispatch a network fetch on every cell
        // recycle — render the placeholder and bail. solos:// is exempt: those
        ...
        if !isSolosURL, NativeMediaImageCache.shared.isRecentlyFailed(src) {
            imgLogger.info("[SolosImage][Load] SUPPRESSED (recent failure within TTL) src=\(src)")
            isLoading = false
            fileNotFound = true
            return
        }
        imgLogger.info("[SolosImage][Load] dispatching async load src=\(src) isSolosURL=\(isSolosURL)")
        ...
        Task.detached(priority: .userInitiated) {
            var fileURL: URL?
            let img: UIImage?
            if let url = URL(string: src), url.scheme == "solos" {
                imgLogger.info("[SolosImage][Load] resolving solos:// URL src=\(src) host=\(url.host ?? "nil") path=\(url.path)")
                if let resolved = resolveSolosFileURLForNativeText(url: url) {
                    imgLogger.info("[SolosImage][Load] solos:// resolved to localPath=\(resolved.path)")
                    fileURL = resolved
                    ...
                    if let data = try? Data(contentsOf: resolved) {
                        imgLogger.info("[SolosImage][Load] read \(data.count) bytes from localPath=\(resolved.path)")
                        ...
                        if let downsampled {
                            imgLogger.info("[SolosImage][Load] downsample OK src=\(src) resultSize=\(downsampled.size.width)x\(downsampled.size.height)")
                        } else {
                            imgLogger.error("[SolosImage][Load] downsample FAILED src=\(src) dataSize=\(data.count)")
                        }
                        img = downsampled
                    } else {
                        imgLogger.error("[SolosImage][Load] Data(contentsOf:) FAILED localPath=\(resolved.path)")
                        img = nil
                    }
                } else {
                    imgLogger.warning("[SolosImage][Load] resolveSolosFileURLForNativeText returned nil src=\(src)")
                    img = nil
                }
            } else if let url = URL(string: src), url.scheme == "http" || url.scheme == "https" {
                imgLogger.info("[SolosImage][Load] fetching HTTP(S) url=\(src)")
                if let data = try? Data(contentsOf: url) {
                    imgLogger.info("[SolosImage][Load] HTTP fetched \(data.count) bytes src=\(src)")
                    img = downsampleImageData(data, maxPixelSize: displayTargetPixels(for: data, screenScale: screenScale))
                } else {
                    imgLogger.warning("[SolosImage][Load] HTTP fetch FAILED src=\(src)")
                    img = nil
                }
            } else if let url = URL(string: src) {
                // file URL or relative
                imgLogger.info("[SolosImage][Load] trying file/relative URL scheme=\(url.scheme ?? "nil") path=\(url.path)")
                fileURL = url
                // [T-ios-image-squish-probe] Same early header probe as the
                // solos:// branch. HTTP(S) is deliberately NOT probed — a
                ...
                if let data = try? Data(contentsOf: url) {
                    imgLogger.info("[SolosImage][Load] file loaded \(data.count) bytes src=\(src)")
                    img = downsampleImageData(data, maxPixelSize: displayTargetPixels(for: data, screenScale: screenScale))
                } else {
                    imgLogger.warning("[SolosImage][Load] file load FAILED src=\(src)")
                    img = nil
                }
            } else {
                imgLogger.error("[SolosImage][Load] cannot parse URL src=\(src)")
                img = nil
            }

            // Recompute the fingerprint post-load in case the file was
            // rewritten between the pre-check at entry and this point.
            let postLoadKey = solosMediaCacheKey(for: src)
            if let img {
                imgLogger.info("[SolosImage][Load] SUCCESS src=\(src) finalSize=\(img.size.width)x\(img.size.height) — caching in memory")
                ...
                if !isSolosURL { NativeMediaImageCache.shared.clearFailure(src) }
            } else {
                imgLogger.warning("[SolosImage][Load] FAILED src=\(src) — no image produced")
                ...
                // solos:// is exempt — its not-yet-written-file case is handled
                // by the retriesRemaining-scheduled retry below.
                if !isSolosURL { NativeMediaImageCache.shared.recordFailure(src) }
            }
            ...
            await MainActor.run {
                ...
                guard gen == self.loadGeneration else {
                    imgLogger.info("[SolosImage][Load] DROP STALE completion gen=\(gen) current=\(self.loadGeneration) src=\(src)")
                    return
                }
                ...
                if let img {
                    ...
                    if isSolosURL && self.retriesRemaining > 0 {
                        imgLogger.info("[SolosImage][Load] RETRY scheduled src=\(src) retriesRemaining=\(self.retriesRemaining)")
                        ...
                        if isSolosURL {
                            imgLogger.error("[SolosImage][Load] GAVE UP src=\(src) — all retries exhausted")
                        }
    ...
    func makeView(width: CGFloat) -> UIView {
        ...
        if let img = loadedImage {
            imgLogger.info("[SolosImage][MakeView] rendering LOADED image src=\(self.source) imgSize=\(img.size.width)x\(img.size.height) containerWidth=\(width)")
            return makeImageView(img, width: width)
        } else {
            imgLogger.info("[SolosImage][MakeView] rendering PLACEHOLDER src=\(self.source) containerWidth=\(width) fileNotFound=\(self.fileNotFound) retriesRemaining=\(self.retriesRemaining)")
    ...
    private func makeImageView(_ img: UIImage, width: CGFloat) -> UIView {
        ...
        let maxH = UIScreen.main.bounds.height / 2
        let h = min(imgWidth * aspect, maxH)
        imgLogger.info("[SolosImage][MakeView] layout src=\(self.source) displayWidth=\(imgWidth) displayHeight=\(h) aspect=\(aspect) maxImageWidth=\(Self.maxImageWidth)")
...
final class VideoAttachment: NSTextAttachment {
    ...
    func beginLoadingIfNeeded() {
        ...
        Task.detached(priority: .userInitiated) {
            var fileURL: URL?
            if let url = URL(string: src), url.scheme == "solos" {
                fileURL = resolveSolosFileURLForNativeText(url: url)
            } else if let url = URL(string: src) {
                fileURL = url
            }
            ...
            if let cached = NativeMediaImageCache.shared.image(for: "thumb:" + cacheKey) {
                await MainActor.run {
                    ...
                    // 10s later" bug).
                    NotificationCenter.default.post(name: .solosAttachmentSizeChanged, object: src)
                }
                return
            }
            ...
            if NativeMediaImageCache.shared.size(forSource: src) == nil,
               ...
               let (natural, transform) = try? await track.load(.naturalSize, .preferredTransform) {
                ...
                if displaySize.width > 0, displaySize.height > 0 {
                    ...
                    imgLogger.info("[SolosVideo][Probe] track size=\(Int(displaySize.width))x\(Int(displaySize.height)) src=\(src) — publishing before thumbnail generation")
                    await MainActor.run {
                        NotificationCenter.default.post(name: .solosAttachmentSizeChanged, object: src)
                    }
                }
...
private final class VideoTapGesture: UITapGestureRecognizer {
    ...
    @objc func handleTap() {
        ...
        let scrollView = self.view?.enclosingScrollView()
        let savedOffset = scrollView?.contentOffset
        // Reuse the shared SolosVideoFullscreenPlayer from AIChatView.
        let playerView = ScrollRestoringDismissWrapper(scrollView: scrollView, savedOffset: savedOffset) {
            SolosVideoFullscreenPlayer(fileURL: url)
        }
...
final class AudioAttachment: NSTextAttachment {
    ...
    private func resolveURL() {
        ...
        if let url = URL(string: cleanSource), url.scheme == "solos" {
            resolvedURL = resolveSolosFileURLForNativeText(url: url)
...
private final class AudioTapGesture: UITapGestureRecognizer {
    ...
    @objc func handleTap() {
        ...
        let preview = SolosAudioPreviewView(fileURL: url)
        let hosting = UIHostingController(rootView: preview)
...
final class SolosLayoutManager: NSLayoutManager {
    ...
    override func fillBackgroundRectArray(_ rectArray: UnsafePointer<CGRect>, count rectCount: Int, forCharacterRange charRange: NSRange, color: UIColor) {
        ...
        // TextKit callback.
        let bgColor = solosInlineCodeBackgroundColor.resolvedColor(with: UITraitCollection.current)
        context.saveGState()
...
final class SelectableMarkdownTextView: UITextView, UIGestureRecognizerDelegate {
    ...
    // MARK: - [T-selection-menu-solos-tts] Solos TTS menu actions

    /// Read the WHOLE reply from the start via the Solos TTS stack
    ...
    /// Speak an arbitrary text snippet via the Solos TTS stack (vm.speakText).
    ...
    override func buildMenu(with builder: UIMenuBuilder) {
        super.buildMenu(with: builder)
        // [T-selection-menu-solos-tts] Remove the SYSTEM speech menu ("Speak…"
        // / "Spell") — it reads with the OS voice, bypassing the Solos TTS
        ...
        // [T-selection-menu-solos-tts] Solos-owned read-aloud entries. "Read
    ...
    func updateAttachmentViews() {
        ...
        guard let textStorage = self.textStorage as? NSTextStorage,
              let layoutManager = self.layoutManager as? SolosLayoutManager else {
            for view in attachmentViews { view.removeFromSuperview() }
            attachmentViews.removeAll()
            attachmentViewMap.removeAll()
            lastUAVStorageLen = -1
        ...
        textStorage.enumerateAttribute(.attachment, in: fullRange, options: []) { value, range, _ in
            ...
            if let existingView = previousViewMap[attachId] {
                ...
                // case taps fell through to UIApplication.open and solos://
    ...
    private func performRefreshAttachmentViews() {
        ...
        if let layoutManager = self.layoutManager as? SolosLayoutManager,
    ...
    override func layoutSubviews() {
        ...
        // Solos-2026-05-13-084827.ips: every layout pass re-triggers a full
        ...
        if currentWidth > 0 && widthDelta > 0.5 {
            ...
            if let storage = self.textStorage as? NSTextStorage, storage.length > 0 {
                ...
                if !ranges.isEmpty, let lm = self.layoutManager as? SolosLayoutManager {
                    for r in ranges {
        ...
        guard !attachmentViews.isEmpty,
              let layoutManager = self.layoutManager as? SolosLayoutManager,
    ...
    private func drawBlockquoteBars(in rect: CGRect) {
        guard let context = UIGraphicsGetCurrentContext(),
              let textStorage = self.textStorage as? NSTextStorage,
              let layoutManager = self.layoutManager as? SolosLayoutManager else { return }
        ...
        // through, which is why Solos#123 stayed reproducible after it.
        for span in spans {
...
struct SelectableMarkdownView: UIViewRepresentable {
    ...
    var onCopyScreenshot: (() -> Void)?
    /// [T-selection-menu-solos-tts] "Read from Start" (whole reply) via Solos TTS.
    var onReadAloud: (() -> Void)?
    /// [T-selection-menu-solos-tts] "Read Selection" (selected text) via Solos TTS.
    var onSpeakText: ((String) -> Void)?
    ...
    func updateUIView(_ textView: SelectableMarkdownTextView, context: Context) {
        ...
        if _widthChanged && markdown == context.coordinator.lastMarkdown && !fontChanged {
            ...
            if let storage = textView.textStorage as? NSTextStorage, storage.length > 0 {
                ...
                if !ranges.isEmpty, let lm = textView.layoutManager as? SolosLayoutManager {
                    for r in ranges {
                        lm.invalidateLayout(forCharacterRange: r, actualCharacterRange: nil)
                    }
                }
        ...
        if !imageMatches.isEmpty {
            for match in imageMatches {
                ...
                imgLogger.info("[SolosImage][StreamParse] image markdown found: \(matchStr)")
            }
            let imageBlockCount = content.blocks.flatMap { Self.collectImageNodes(from: $0) }.count
            if imageBlockCount > 0 {
                imgLogger.info("[SolosImage][StreamParse] parsed \(imageBlockCount) image node(s) from \(content.blocks.count) block(s), markdownLen=\(markdown.count)")
            }
        }
        ...
        attributed.enumerateAttribute(.attachment,
            ...
            if let loadedFp = att.loadedFingerprint {
                let currentFp = solosMediaCacheKey(for: att.source)
                if loadedFp != currentFp {
                    imgLogger.info("[SolosImage][CachedAttr] FINGERPRINT CHANGED src=\(att.source) old=\(loadedFp) new=\(currentFp) — invalidating cached image")
    ...
    func sizeThatFits(_ proposal: ProposedViewSize, uiView: SelectableMarkdownTextView, context: Context) -> CGSize? {
        ...
        // message (user report, macOS, "Solos Feedback Review" — the shell
        ...
        // recorded in `Solos-2026-05-13-084827.ips` (depth=9 recursion at
    ...
    final class Coordinator: NSObject, UITextViewDelegate {
        ...
        /// `Solos-2026-05-13-084827.ips`: it rebuilds attachment subviews
...
// MARK: - Solos URL Resolution (bridged from AIChatView)

/// Resolves a `solos://` URL to a local file URL.
/// Keeps resolution behavior aligned with AIChatView's solos resolver.
private func resolveSolosFileURLForNativeText(url: URL) -> URL? {
    guard url.scheme == "solos" else { return nil }
    guard let host = url.host else {
        imgLogger.warning("[SolosImage][Resolve] no host in URL: \(url.absoluteString)")
        return nil
    }
    ...
    // same tolerant resolution the other 4 solos:// resolvers already use; this
    ...
    let subPaths = SolosURLPathDecoding.subPathCandidates(for: url)
    imgLogger.info("[SolosImage][Resolve] BEGIN url=\(url.absoluteString) host=\(host) subPaths=\(subPaths) activeSession=\(AIChatViewModel.activeSessionId ?? "nil")")
    let fm = FileManager.default

    // Primary: active session persistent storage
    if let sid = AIChatViewModel.activeSessionId {
        for subPath in subPaths {
            let persistURL = AIChatViewModel.solosPersistentBase
                .appendingPathComponent(sid, isDirectory: true)
                .appendingPathComponent(host, isDirectory: true)
                .appendingPathComponent(subPath)
            imgLogger.info("[SolosImage][Resolve] checking session path=\(persistURL.path)")
            if fm.fileExists(atPath: persistURL.path) {
                let size = (try? fm.attributesOfItem(atPath: persistURL.path)[.size] as? Int64) ?? -1
                imgLogger.info("[SolosImage][Resolve] FOUND in active session=\(sid) path=\(persistURL.path) size=\(size)")
                return persistURL
            }
        }
        imgLogger.info("[SolosImage][Resolve] NOT in active session=\(sid)")
    } else {
        imgLogger.info("[SolosImage][Resolve] no active session — skipping session lookup")
    }

    // Global namespaces (not session-scoped)
    if host == "skills" {
        for subPath in subPaths {
            let candidate = AIChatViewModel.solosSkillsPersistentDir.appendingPathComponent(subPath)
            imgLogger.info("[SolosImage][Resolve] checking global skills path=\(candidate.path)")
            if fm.fileExists(atPath: candidate.path) {
                let size = (try? fm.attributesOfItem(atPath: candidate.path)[.size] as? Int64) ?? -1
                imgLogger.info("[SolosImage][Resolve] FOUND global skills path=\(candidate.path) size=\(size)")
                return candidate
            }
        }
    } else if host == "memory" {
        for subPath in subPaths {
            let candidate = AIChatViewModel.solosMemoryPersistentDir.appendingPathComponent(subPath)
            imgLogger.info("[SolosImage][Resolve] checking global memory path=\(candidate.path)")
            if fm.fileExists(atPath: candidate.path) {
                let size = (try? fm.attributesOfItem(atPath: candidate.path)[.size] as? Int64) ?? -1
                imgLogger.info("[SolosImage][Resolve] FOUND global memory path=\(candidate.path) size=\(size)")
                return candidate
            }
        }
    }

    // [T-ios-solosurl-cross-session-isolation] DO NOT scan all session dirs.
    ...
    // leak fixed in resolveSolosFileURL — session B rendering
    // model-use-zimage-0.jpg would resolve to session A's image. solos:// is
    // session-scoped; not-found is the correct result for a cross-session ref.
    //
    // The rootfs `/var/solos/<host>` fallback is kept ONLY for the global,
    ...
    if globalHosts.contains(host) {
        for subPath in subPaths {
            let rootfsURL = RootfsManager.shared.dataPath
                .appendingPathComponent("var/solos", isDirectory: true)
                .appendingPathComponent(host, isDirectory: true)
                .appendingPathComponent(subPath)
            if fm.fileExists(atPath: rootfsURL.path) {
                let size = (try? fm.attributesOfItem(atPath: rootfsURL.path)[.size] as? Int64) ?? -1
                imgLogger.info("[SolosImage][Resolve] FOUND in global rootfs path=\(rootfsURL.path) size=\(size)")
                return rootfsURL
            }
        }
    }

    imgLogger.warning("[SolosImage][Resolve] NOT FOUND in active session / global dirs url=\(url.absoluteString) host=\(host) subPaths=\(subPaths) (cross-session scan disabled for isolation)")
    return nil
}
