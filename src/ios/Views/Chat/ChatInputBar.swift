...
private struct AttachmentChip: View {
    ...
    private func loadThumbnailIfNeeded() {
        ...
        if let cached = SolosMediaCache.shared.thumbnail(for: key) {
            cachedImage = cached
            return
        }
        Task.detached(priority: .userInitiated) {
            ...
            if let image {
                SolosMediaCache.shared.setThumbnail(image, for: key)
...
struct CameraPicker: UIViewControllerRepresentable {
    ...
    func makeUIViewController(context: Context) -> UIImagePickerController {
        ...
        solosLogger.info("[QuickAction] CameraPicker.makeUIViewController cameraAvailable=\(cameraAvailable) authStatus=\(authStatus.rawValue)")
        let picker = UIImagePickerController()
        if cameraAvailable {
            picker.sourceType = .camera
        } else {
            solosLogger.warning("[QuickAction] CameraPicker — .camera source unavailable, falling back to .photoLibrary")
...
struct UserAttachmentList: View {
    ...
    private func openGallery(startingAt tapped: AttachmentMeta) {
        ...
        guard let start = all.firstIndex(where: { $0.solosURL == tapped.solosURL }) else {
            return
        }
        let items: [GalleryItem] = all.map { meta in
            ...
            let fpKey = solosMediaCacheKey(for: meta.solosURL)
            return GalleryItem(
                id: fpKey,
                title: meta.fileName,
                load: {
                    ...
                    guard let url = URL(string: meta.solosURL),
                          let fileURL = resolveSolosFileURLCached(url: url),
    ...
    private func fileTile(_ meta: AttachmentMeta) -> some View {
        ...
        // menu → "Add to Home Screen". The sheet resolves the solos://
        // URL to a host URL via resolveSolosFileURLCached and hands off
...
/// solos:// URL to a host URL on tap and presents `WebAppAddToHomeSheet`.
        ...
        if isHTML {
            content
                .contextMenu {
                    Button {
                        guard let url = URL(string: meta.solosURL),
                              let host = resolveSolosFileURLCached(url: url) else {
                            return
                        }
