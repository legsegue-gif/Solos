...
final class WebViewHolder: NSObject, ObservableObject {
    ...
    init(url: URL, localFile: Bool) {
        ...
        // BrowserUseManager so solos:// HTML opened from chat (which routes
    ...
    /// inside a `.fullScreenCover` (the iSH terminal → `solos-open` flow
...
struct WebPreviewMoreMenu: View {
    ...
    /// Only the solos:// HTML preview wires this up — it triggers the
...
struct SolosLinkPreviewView: View {
    let url: URL
