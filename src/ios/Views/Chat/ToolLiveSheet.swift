...
/// environment — callers override that to present SolosLinkPreviewView.
///
...
fileprivate func attributedShellLine(_ text: String) -> AttributedString {
    ...
    for m in matches {
        ...
        // which matches path fragments in shell output (e.g. "/Users/.../github.com/legsegue-gif/...").
...
struct ToolLiveSheet: View {
    ...
    private func durationLabel(started: Date) -> String {
        ...
        return SolosStepTimestampFormatter.duration(seconds: secs, stillRunning: stillRunning)
    }
    ...
    var body: some View {
        ...
        .onAppear {
            ...
            SolosOpenURLBroker.shared.toolSheetVisible = true
        }
        .onDisappear {
            stopBrowserTimer()
            resourceMonitor.stop()
            SolosOpenURLBroker.shared.toolSheetVisible = false
        }
        // Auto-present an in-app browser preview when a shell tool emits an
        // OSC SolosOpenURL marker while this sheet is visible.
        //
        // Only intercepts http/https/about: URLs — `solos://` chat-resource
        ...
        .onReceive(SolosOpenURLBroker.shared.$pendingURL.dropFirst().compactMap { $0 }) { url in
            guard SolosOpenURLBroker.isWebScheme(url.scheme) else { return }
        ...
        .sheet(item: $activeSheet, onDismiss: {
            ...
            case .linkPreview(let url):
                SolosLinkPreviewView(url: url, browserPool: browserPool)
            }
        }
    }
    ...
    /// - Parameter toolResult: Optional tool result info (solos_url etc.) shown below the editor.
                ...
                if dur != nil || block.toolStartTime != nil {
                    VStack(alignment: .trailing, spacing: 1) {
                        ...
                        if let started = block.toolStartTime {
                            Text(SolosStepTimestampFormatter.string(from: started))
    ...
    private var toolTitle: String {
        switch block.kind {
        case .shellTool: return "Solos is using Shell"
        case .fileReadTool: return "Solos is reading File"
        case .fileWriteTool: return "Solos is using Editor"
        case .fileEditTool: return "Solos is editing File"
        case .browserTool: return "Solos is using Browser"
        case .readImageTool: return "Solos is reading Image"
        case .memoryTool: return "Solos is using Memory"
        case .info: return "Solos"
        case .text: return "Solos"
        case .thinking: return "Solos"
        }
    }
...
enum SolosStepTimestampFormatter {
