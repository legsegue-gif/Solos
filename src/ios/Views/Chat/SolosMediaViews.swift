...
private let solosLogger = AppLogger(category: "SolosMedia")
// MARK: - solos:// Media Provider

// MARK: Media Cache — avoids re-loading/decoding when LazyVStack recycles cells

final class SolosMediaCache {
    static let shared = SolosMediaCache()

    /// Downsampled UIImage cache keyed by absolute solos:// URL string.
    private let imageCache = NSCache<NSString, UIImage>()
    /// Video thumbnail cache keyed by file path.
    private let thumbnailCache = NSCache<NSString, UIImage>()
    /// Resolved host file URL cache keyed by solos:// URL string.
    ...
    /// time each was taken.
    ///
    /// `solosMediaCacheKey` is evaluated inside `body` (it feeds `.task(id:)`),
    /// so it runs on the main actor once per tile per body pass. Its `stat()`
    /// was previously uncached, and for a `solos://mounts/<name>/…` source the
    ...
    /// after we write to a solos:// path ourselves, where waiting out the TTL
...
/// Resolve a solos:// URL with caching to avoid repeated FileManager lookups.
///
/// Cache key is scoped by the active session id because per-session paths
/// (e.g. `solos://attachments/foo.png`) resolve to different files in
/// `Library/SolosChat/solos/<sessionId>/…` depending on which session is
...
func resolveSolosFileURLCached(url: URL) -> URL? {
    let sessionScope = AIChatViewModel.activeSessionId ?? "nil"
    let key = "\(sessionScope)|\(url.absoluteString)"
    if let cached = SolosMediaCache.shared.resolvedURL(for: key) {
        solosLogger.info("[SolosImage][ResolveCache] HIT url=\(url.absoluteString) → \(cached.path)")
        return cached
    }
    if let resolved = resolveSolosFileURL(url: url) {
        // Don't persist a mount resolution taken on the main thread: that path
        // is returned unverified (see the mounts branch of resolveSolosFileURL,
        ...
        if !isUnverifiedMountGuess {
            SolosMediaCache.shared.setResolvedURL(resolved, for: key)
        }
        solosLogger.info("[SolosImage][ResolveCache] MISS→resolved url=\(url.absoluteString) → \(resolved.path)")
        return resolved
    }
    solosLogger.warning("[SolosImage][ResolveCache] MISS→nil url=\(url.absoluteString)")
    return nil
}
...
///
/// For `solos://` sources we resolve to the host file URL and stat it.
...
func solosMediaCacheKey(for source: String) -> String {
    guard let url = URL(string: source) else { return source }
    return solosMediaCacheKey(for: url)
}

func solosMediaCacheKey(for url: URL) -> String {
    let base = url.absoluteString
    // Non-solos schemes: return as-is, we don't own that file's lifecycle.
    guard url.scheme == "solos" else { return base }
    guard let fileURL = resolveSolosFileURLCached(url: url) else {
    ...
    // fingerprint when we have one; see SolosMediaCache.fingerprint(for:).
    if let cached = SolosMediaCache.shared.fingerprint(for: path) {
        return cached
    }
    ...
    SolosMediaCache.shared.setFingerprint(key, for: path)
    return key
}
...
let solosAudioExtensions: Set<String> = ["mp3", "m4a", "wav", "aac", "ogg", "flac"]
let solosVideoExtensions: Set<String> = ["mp4", "mov", "m4v", "avi", "mkv", "webm"]
let solosImageExtensions: Set<String> = ["png", "jpg", "jpeg", "gif", "webp", "bmp", "tiff"]
let solosTextExtensions: Set<String> = [
...
let solosMarkdownExtensions: Set<String> = ["md", "markdown"]
let solosHTMLExtensions: Set<String> = ["html", "htm", "svg"]
let solosDocumentExtensions: Set<String> = [
    "pdf",
    // Microsoft Office
...
/// Resolve a solos:// URL to a host filesystem URL.
/// Maps directly to persistent storage: Library/SolosChat/solos/<sessionId>/<subdir>/<path>
/// No dependency on iSH boot or bind mounts.
func resolveSolosFileURL(url: URL) -> URL? {
    guard let host = url.host else {
        solosLogger.warning("[ResolveSolosURL] no host in URL: \(url.absoluteString)")
        return nil
    }
    ...
    if let sid = AIChatViewModel.activeSessionId {
        for subPath in subPaths {
            ...
            solosLogger.info("[SolosImage][Resolve] \(url.absoluteString) → session=\(sid) → \(persistURL.path) exists=\(exists) size=\(fileSize)")
    ...
    let globalDirs: [(subdir: String, url: URL)] = [
        ("skills", AIChatViewModel.solosSkillsPersistentDir),
        ("memory", AIChatViewModel.solosMemoryPersistentDir),
        ("shared", AIChatViewModel.solosSharedPersistentDir),
    ]
    for (subdir, globalDir) in globalDirs where host == subdir {
        for subPath in subPaths {
            let candidate = globalDir.appendingPathComponent(subPath)
            if fm.fileExists(atPath: candidate.path) {
                solosLogger.info("[SolosImage][Resolve] \(url.absoluteString) → global \(subdir) → \(candidate.path)")
                return candidate
            }
        }
    }

    // User-mounted external folders: solos://mounts/<name>/<path>
    ...
    if host == "mounts" {
        for subPath in subPaths {
            ...
            if let mountURL {
                ...
                // `solosMediaCacheKey`, so on the main thread we accept the
                ...
                // the precise check.
                if Thread.isMainThread {
                    solosLogger.info("[SolosImage][Resolve] \(url.absoluteString) → mount '\(mountName)' → \(candidate.path) (unverified, main thread)")
                    return candidate
                }
                if fm.fileExists(atPath: candidate.path) {
                    solosLogger.info("[SolosImage][Resolve] \(url.absoluteString) → mount '\(mountName)' → \(candidate.path)")
                    return candidate
                }
            }
        }
    ...
    // [T-ios-solosurl-cross-session-isolation] DO NOT scan other sessions.
    // solos://attachments/<file> is session-scoped: it must only resolve
    ...
    // fallback was a P0 isolation leak — solos-model-use emits fixed names like
    ...
    solosLogger.warning("[SolosImage][Resolve] \(url.absoluteString) → not found in active session, global dirs, or mounts (cross-session scan disabled for isolation)")
    return nil
}

// MARK: - solos:// Link Handling

/// SF Symbol icon for a given file extension.
private func solosFileIcon(for ext: String) -> String {
...
/// Tappable file chip for solos:// links in Markdown.
private struct SolosFileChipView: View {
    let url: URL
    ...
    var body: some View {
        ...
        Button {
            ...
            HStack(spacing: 6) {
                Image(systemName: solosFileIcon(for: ext))
                    .font(.system(size: 13))
...
struct AsyncImageTile: View {
    ...
    var body: some View {
        ...
        // the underlying solos:// file invalidates the displayed
        // thumbnail (T-image-cache-mtime-35133). URL string alone would
        // keep the stale image until the cell recycles.
        .task(id: solosMediaCacheKey(for: meta.solosURL)) { loadThumbnail() }
    }

    private func loadThumbnail() {
        // Fingerprint key captures file size + mtime so a rewrite of the
        // same solos:// path invalidates the cache automatically.
        let cacheKey = "attach:\(solosMediaCacheKey(for: meta.solosURL))"
        if let cached = NativeMediaImageCache.shared.image(for: cacheKey) {
            thumbnail = cached
            return
        }
        guard let url = URL(string: meta.solosURL),
              let fileURL = resolveSolosFileURLCached(url: url) else { return }
...
struct AsyncVideoTile: View {
    ...
    var body: some View {
        Button {
            if let url = URL(string: meta.solosURL) { openURL(url) }
        } label: {
...
/// Environment action for handling solos:// and http(s):// URL taps.
///
...
struct SolosURLAction {
    var handler: (URL) -> OpenURLAction.Result
    func callAsFunction(_ url: URL) -> OpenURLAction.Result { handler(url) }
}

private struct SolosURLKey: EnvironmentKey {
    static let defaultValue = SolosURLAction { _ in .systemAction }
}

extension EnvironmentValues {
    var solosURL: SolosURLAction {
        get { self[SolosURLKey.self] }
        set { self[SolosURLKey.self] = newValue }
    }
}

/// Thin modifier that overrides `\.openURL` for a subtree and forwards to the
/// top-level `\.solosURL` action. Keeps cell-internal views from holding their
/// own sheet @State (which would break presentation animations).
struct SolosOpenURLHandler: ViewModifier {
    @Environment(\.solosURL) private var solosURL
...
private struct SolosImageView: View {
    let url: URL?
    ...
    var body: some View {
        if let url, url.scheme == "solos" {
            let ext = url.pathExtension.lowercased()
            let _ = solosLogger.info("[SolosImage][View] render url=\(url.absoluteString) ext=\(ext) hasLoadedImage=\(self.loadedImage != nil) attempt=\(self.loadAttempt)")
            if solosAudioExtensions.contains(ext) {
                if let fileURL = resolveSolosFileURLCached(url: url) {
                    SolosAudioPlayerView(url: url, fileURL: fileURL)
                }
            } else if solosVideoExtensions.contains(ext) {
                if let fileURL = resolveSolosFileURLCached(url: url) {
                    SolosVideoPlayerView(url: url, fileURL: fileURL)
                }
            } else {
                solosImageContent(url: url)
                    ...
                    // already fingerprint-keyed via solosMediaCacheKey.
                    .task(id: solosMediaCacheKey(for: url)) {
                        await loadWithRetry(url: url)
                    }
            }
        } else if let url {
            // Fall through to default async loading for non-solos URLs
    ...
    @ViewBuilder
    private func solosImageContent(url: URL) -> some View {
        let maxH = UIScreen.main.bounds.height / 2
    ...
    private func loadWithRetry(url: URL) async {
        let cacheKey = solosMediaCacheKey(for: url)
        solosLogger.info("[SolosImage][MarkdownUI] loadWithRetry START url=\(url.absoluteString)")
        // Check memory cache first — instant on cell recycle
        if let cached = SolosMediaCache.shared.image(for: cacheKey) {
            solosLogger.info("[SolosImage][MarkdownUI] CACHE HIT url=\(url.absoluteString) size=\(cached.size.width)x\(cached.size.height)")
            loadedImage = cached
        ...
        for attempt in 0..<6 {
            ...
            loadAttempt = attempt
            if let fileURL = resolveSolosFileURLCached(url: url) {
                solosLogger.info("[SolosImage][MarkdownUI] attempt=\(attempt) url=\(url.absoluteString) resolved=\(fileURL.path) exists=\(FileManager.default.fileExists(atPath: fileURL.path))")
                // Load and downsample on background thread to avoid blocking main thread
                let image = await Task.detached(priority: .userInitiated) {
                    guard let data = try? Data(contentsOf: fileURL) else {
                        solosLogger.warning("[SolosImage][MarkdownUI] Data(contentsOf:) FAILED url=\(url.absoluteString) path=\(fileURL.path)")
                        return nil as UIImage?
                    }
                    let fileSize = data.count
                    let img = downsampleImage(data: data, maxPixelSize: 2048)
                    if let img {
                        solosLogger.info("[SolosImage][MarkdownUI] decoded url=\(url.absoluteString) dataSize=\(fileSize) imgSize=\(img.size.width)x\(img.size.height)")
                    } else {
                        solosLogger.warning("[SolosImage][MarkdownUI] downsample FAILED url=\(url.absoluteString) dataSize=\(fileSize)")
                    }
                    return img
                }.value
                if let image {
                    SolosMediaCache.shared.setImage(image, for: cacheKey)
                    loadedImage = image
                    solosLogger.info("[SolosImage][MarkdownUI] SUCCESS url=\(url.absoluteString) attempt=\(attempt)")
                    // [T-attachment-size-invalidate] Placeholder (~fixed) → fit (variable, up to half-screen).
                    notifySizeChangeOnce()
                    return
                }
            } else {
                solosLogger.warning("[SolosImage][MarkdownUI] resolve FAILED attempt=\(attempt) url=\(url.absoluteString)")
            }
        }
        solosLogger.error("[SolosImage][MarkdownUI] GAVE UP after 6 attempts url=\(url.absoluteString)")
        loadAttempt = 0
    }

    /// Post .solosAttachmentSizeChanged at most once per view lifetime.
    ...
    private func notifySizeChangeOnce() {
        ...
        NotificationCenter.default.post(name: .solosAttachmentSizeChanged, object: urlStr)
    }
}
...
// MARK: - solos:// Image File Preview (from URL handler)

/// Fullscreen image preview loading from a local file URL.
struct SolosImageFilePreviewView: View {
    let fileURL: URL
...
/// `.task(id: solosMediaCacheKey(...))`; mirror it here with a cheap stat so a
/// same-path rewrite re-reads the file. Unlike solosMediaCacheKey this takes the
/// already-resolved disk URL directly (no solos:// re-resolve).
...
struct SolosTextPreviewView: View {
    let fileURL: URL
    @Environment(\.dismiss) private var dismiss
...
private struct SolosTextView: UIViewRepresentable {
    let text: String
...
// Non-private so cross-file preview sheets (e.g. SolosLinkPreviewView in
...
struct SolosMarkdownPreviewView: View {
    let fileURL: URL
    @Environment(\.dismiss) private var dismiss
...
// MARK: - solos:// HTML Preview

struct SolosHTMLPreviewView: View {
    let fileURL: URL
    ...
    var body: some View {
        ...
        // iPad/Mac, matching SolosMarkdownPreviewView. iPhone unaffected.
        .modifier(WideSheetSizingModifier())
...
// MARK: - solos:// Document Preview

import QuickLook

struct SolosDocumentPreviewView: UIViewControllerRepresentable {
    let fileURL: URL
    ...
    class Coordinator: NSObject, QLPreviewControllerDataSource {
        ...
        /// solosMediaCacheKey's stat-based approach.
