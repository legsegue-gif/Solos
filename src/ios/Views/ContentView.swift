...
struct ContentView: View {
    ...
    /// back to "Solos". Refreshed whenever SoulStore posts .soulMdChanged.
    @State private var soulName: String = SoulStore.cachedMetadata.name.isEmpty
        ? "Solos" : SoulStore.cachedMetadata.name
    /// Subtitle state shown under the "Solos" sidebar title. nil hides the
    ...
    var body: some View {
        ...
        // WebApp deep link, a `.solosbak` opened from Files). iOS will not
        ...
        .onChange(of: selectedSessionId) { newValue in
            ...
            // Solos-2026-06-01-134710.ips). Suspend the outgoing vm here,
        ...
        .onChange(of: navigationPath) { _ in
            ...
            //
            // Crash 2026-08-10 19:23 (Solos 1.12(1), iOS 26.5.2, iPhone18,1 —
    ...
    private func refreshMigrationSubtitle() {
        ...
        Task { @MainActor in
            ...
            if !byType.isEmpty {
                ...
                // "Solos" title sits at its normal size.
                if migrationSubtitle != nil { migrationSubtitle = nil }
            }
        }
    }
    ...
    /// Tiny indicator next to the "Solos" title showing the sync state.
    @ViewBuilder
    ...
    private var sidebarToolbarContent: some ToolbarContent {
        ToolbarItem(placement: .principal) {
            if isSelecting {
                ...
                // Title text comes from SOUL.md (falls back to "Solos"). The
    ...
    private nonisolated static func computeDeleteInfo(for ids: Set<String>, totalSessions: Int) -> DeleteInfo {
        ...
        let solosBase = libBase.appendingPathComponent("SolosChat/solos", isDirectory: true)
        let dbPath = libBase.appendingPathComponent("SolosChat/solos.db")
        ...
        for id in ids {
            let sessionDir = solosBase.appendingPathComponent(id, isDirectory: true)
    ...
    /// Remove persistent solos files for a session (Library/SolosChat/solos/<sessionId>/).
    private func deleteSessionFiles(_ sessionId: String) {
        let fm = FileManager.default
        let base = fm.urls(for: .libraryDirectory, in: .userDomainMask).first!
            .appendingPathComponent("SolosChat/solos", isDirectory: true)
    ...
    private func exportSessions(ids: Set<String>, format: ExportFormat) {
        ...
        Task.detached(priority: .userInitiated) {
            ...
            let tmpRoot = FileManager.default.temporaryDirectory
                .appendingPathComponent("solos-export-\(UUID().uuidString)", isDirectory: true)
            ...
            if ids.count == 1, let title = targetSessions.first?.title {
                ...
                baseName = "solos-sessions-\(ids.count)"
            }
    ...
    private static func streamExportAsPlainText(
        ...
        for (i, session) in targetSessions.enumerated() {
            ...
            for msg in messages {
                ...
                let role = msg.role == .user ? "User" : "Solos"
...
private struct ShareSheet: UIViewControllerRepresentable {
    ...
    func makeUIViewController(context: Context) -> UIActivityViewController {
        ...
        // SolosShareSheet.sanitizedShareURL for context.
        let safeURL = SolosShareSheet.sanitizedShareURL(url) ?? url
...
private struct SessionContextMenu: View, Equatable {
    ...
    var body: some View {
        ...
        Button {
            ...
            if let url = URL(string: "mailto:dev@solos.app?subject=\(subject)&body=\(msgBody)") {
            ...
            Section {
                ForEach(supportedLanguages) { lang in
                    Button {
                        ...
                        // `.id(appLanguage)` rebuild in SolosApp.swift, which
...
private struct SettingsSheet: View {
        ...
        NavigationStack(path: $navPath) {
            List {
                ...
                Section("About") {
                    ...
                    Link(destination: URL(string: "https://legsegue-gif.github.io/solos-privacy.html")!) {
                        Label {
                            Text("Privacy Policy")
                        } icon: {
                    ...
                    .confirmationDialog("Feedback", isPresented: $showFeedbackDialog, titleVisibility: .visible) {
                        ...
                        Button("Feedback (Telegram)") {
                            if let url = URL(string: "") { UIApplication.shared.open(url) }
                        }
    ...
    fileprivate static func makeFeedbackEmailURL() -> URL? {
        ...
        components.path = "dev@solos.app"
        components.queryItems = [
            URLQueryItem(name: "subject", value: "Solos Feedback"),
    ...
    fileprivate static func makeBugReportURL() -> URL? {
        ...
        | Solos Version | \(appVersion) (build \(build)) |
        | Device Model | \(device) |
        ...
        var components = URLComponents(string: "https://github.com/legsegue-gif/Solos/issues/new")
        components?.queryItems = [
