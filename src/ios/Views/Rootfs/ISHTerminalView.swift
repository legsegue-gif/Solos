...
struct ISHTerminalView: View {
    ...
    /// URL captured from an OSC `SolosOpenURL` marker emitted by
    /// /usr/local/bin/solos-open — presented in an in-app WKWebView sheet.
    ...
    var body: some View {
        ...
        .onAppear {
            ...
            SolosOpenURLBroker.shared.terminalVisible = true
        }
        .onDisappear {
            SolosOpenURLBroker.shared.terminalVisible = false
        }
        ...
        // In-app WKWebView preview for URLs emitted by `solos-open` via
        // the OSC 1337 SolosOpenURL marker. `TerminalEmulator` parses the
        // marker and forwards the URL through `SolosOpenURLBroker`.
        ...
        .onReceive(SolosOpenURLBroker.shared.$pendingURL.dropFirst().compactMap { $0 }) { url in
            // Only web schemes are routed here — solos:// resource
            // previews need AIChatView's `handleSolosURLTap` and aren't
        ...
        .sheet(item: $linkPreviewURL) { url in
            ...
            // uses — `SolosLinkPreviewView` with its toolbar (reload /
            ...
            SolosLinkPreviewView(url: url, browserPool: nil)
        }
    }
}
...
class ISHTerminalViewModel: ObservableObject {
    ...
    func startShell(sessionId: String? = nil, initCommand: String? = nil) {
        ...
        // Mount /var/solos/* for this session BEFORE starting the shell — the
    ...
    private func finishStartShell(totalStart: CFAbsoluteTime,
        ...
        if sessionId != nil {
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.3) { [weak self] in
                if let data = "cd /var/solos && clear\n".data(using: .utf8) {
                    self?.sendInput(data)
                }
    ...
    private let inputQueue = DispatchQueue(label: "com.solos.terminal.input")
