...
struct FileBrowserView: View {
    ...
    var body: some View {
        ...
        .sheet(item: $moveOrCopyItem) { item in
            let solosPath = viewModel.rootPath.appendingPathComponent("var/solos")
            let initial = FileManager.default.fileExists(atPath: solosPath.path) ? solosPath : nil
            NavigationStack {
                DirectoryPickerView(
...
private struct HTMLFilePreview: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> WKWebView {
        ...
        config.setURLSchemeHandler(BrowserUseManager.sharedSolosSchemeHandler, forURLScheme: "solos")
        ...
        FileItemRow(item: item, onTap: onTap, onExport: onExport)
            .contextMenu {
                Button {
                    ...
                    // absolute path (e.g. /var/solos/workspace/.../L3_0001.png)
            ...
            .sheet(isPresented: $showAddWebApp) {
                ...
                // without going through solos:// resolution.
...
/// `SolosShareSheet.sanitizedShareURL` for the ShareKit UTI crash mitigation).
struct DocumentExportView: UIViewControllerRepresentable {
    ...
    func makeUIViewController(context: Context) -> UIActivityViewController {
        ...
        let shareURL = SolosShareSheet.sanitizedShareURL(staged) ?? staged
...
class FileBrowserViewModel: ObservableObject {
    ...
    func loadItems() {
        ...
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            ...
            do {
                ...
                // (e.g. /var/solos/attachments -> Library/SolosChat/...)
    ...
    func deleteItem(_ item: FileItem) {
        ...
        do {
            ...
            if let linux = linuxPath(for: removedURL) {
                ...
                // trees (/var/solos/{shared,skills,memory}/...), the Files app has
            ...
            // workspace tree (Library/SolosChat/solos/<sid>/...), queue
    ...
    /// Maps a Linux path under `/var/solos/{shared,skills,memory}/...` to the
    ...
    private static let fileProviderExposedRoots: [String: NSFileProviderItemIdentifier] = [
        "/var/solos/shared":  NSFileProviderItemIdentifier("shared"),
        "/var/solos/skills":  NSFileProviderItemIdentifier("skills"),
        "/var/solos/memory":  NSFileProviderItemIdentifier("memory"),
    ]
    ...
    private static func signalFileProviderParent(forLinuxPath linux: String) {
        ...
        let domainIdentifier = NSFileProviderDomainIdentifier("com.solos.app.files")
    ...
    /// If `removedURL` lives inside a session's per-session solos directory
    /// (`Library/SolosChat/solos/<sid>/<subdir>/<rel>`), queue a SessionFile
    ...
    private static func queueSessionFileCloudDelete(forRemovedURL removedURL: URL) {
        let baseURL = ChatStore.shared.solosBaseURL
    ...
    func displayPath(for url: URL) -> String {
        // [T-ios-copy-abs-path-varsolos-rooted] Primary path: reconstruct the
        // guest path from `currentPath` (the LOGICAL, un-symlink-resolved
        // directory we're browsing, e.g. `.../data/var/solos/browser`) + the
        ...
        // symlink-RESOLVED. For a bind-mounted subdir like `/var/solos/browser`
        // (an APFS symlink → `.../Library/SolosChat/solos/<sid>/browser`) the
        ...
        // (`0419….jpg`) instead of `/var/solos/browser/0419….jpg`.
        ...
        if curStd == rawRootEarly || curStd.hasPrefix(rawRootEarly + "/") {
            let relDir = String(curStd.dropFirst(rawRootEarly.count))   // "" or "/var/solos/browser"
        ...
        // SolosChat container) resolve OUTSIDE the rootfs `data/` tree, so the
        // resolved prefix check above won't match them. Fall back to the
        // UN-resolved namespace: the listing keeps the logical `/var/solos/…`
    ...
    /// plus a rel of "/var/solos/x" yields "/var/solos/x" (not "//var/solos/x")
