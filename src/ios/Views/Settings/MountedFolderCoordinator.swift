...
enum MountedFolderCoordinator {
    ...
    /// Checks both the fakefs symlink path (/var/solos/mounts/<name>/...) and
    ...
    /// Returns true if `linuxPath` (a `/var/solos/mounts/<name>/...` path from
    /// the iSH namespace) is under a mount entry that is effectively read-only.
    /// Compares by mount name under `/var/solos/mounts/`, so it's immune to the
