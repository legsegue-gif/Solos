...
//  mounted into /var/solos/mounts/<name> via security-scoped bookmarks.
//
//  Lifecycle:
...
//  - ensureSolosSymlinks() creates /var/solos/mounts/<name> symlinks in fakefs
//
...
struct MountedFolderEntry: Codable, Identifiable, Equatable {
    ...
    let id: UUID
    /// User-chosen name; becomes the folder name under /var/solos/mounts/<name>.
    ...
    /// User intent: does the user *want* Solos (shell + AI + Browse Files) to
...
final class MountedFoldersManager {
    ...
    /// Mounts metadata lives in the App Group's private SolosConfig subdir
    /// so it does NOT leak into iOS Files' "On My iPhone → Solos" view.
    /// Historically stored in `solosAppGroupRoot/mounted-folders.json`, but
    /// that path sits inside the FileProvider's exposed root.
    private static var storeURL: URL {
        return AIChatViewModel.solosConfigRoot.appendingPathComponent("mounted-folders.json")
    }
    ...
    private static var legacyStoreURL: URL {
        AIChatViewModel.solosAppGroupRoot.appendingPathComponent("mounted-folders.json")
    }

    /// Move `mounted-folders.json` out of providerRoot into SolosConfig the
    ...
    private func migrateStoreFileIfNeeded() {
        ...
        do {
            ...
            mountLog.info("migrateStoreFile: moved \(legacy.lastPathComponent) to SolosConfig/")
        } catch {
            // Fallback: copy + delete, in case move-across-bind fails.
            do {
                try fm.copyItem(at: legacy, to: current)
                try fm.removeItem(at: legacy)
                mountLog.info("migrateStoreFile: copied \(legacy.lastPathComponent) to SolosConfig/ (move failed, copy+delete succeeded)")
            } catch {
    ...
    /// Solos-internal soft lock on top of the OS-level `isWritable`. Calling
    ...
    func pushExternalMountSnapshot() {
        ...
        let snapshot: [ISHExecutionCoordinator.ExternalMountSpec] = entries.compactMap { entry in
            ...
            let linuxDir = "\(AIChatViewModel.solosMountsLinuxDir)/\(entry.name)"
    ...
    /// Returns the entry matching a fakefs symlink path (`/var/solos/mounts/<name>/...`).
    ...
    func entryForLinuxPath(_ linuxPath: String) -> MountedFolderEntry? {
        let prefix = AIChatViewModel.solosMountsLinuxDir + "/"
    ...
    nonisolated static func probeWritable(at url: URL) -> Bool {
        let probe = url.appendingPathComponent(".solos-probe-\(UUID().uuidString)")
    ...
    private func removeMountSymlink(name: String) {
        ...
        let linkPath = dataPath
            .appendingPathComponent("var/solos/mounts", isDirectory: true)
