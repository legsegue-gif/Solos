...
struct CloudSyncSettingsView: View {
    ...
    var body: some View {
        ...
        .alert("Are you absolutely sure?", isPresented: $showDeleteCloudStep2) {
            ...
            Text("This action cannot be undone. All Solos data stored in iCloud will be erased immediately. Other devices signed into the same iCloud account will lose their synced copies on the next sync.\n\nLocal data on this device is safe.")
        }
        .alert("iCloud Data Deleted", isPresented: $showDeleteCloudDone) {
            Button("OK", role: .cancel) {}
        } message: {
            Text("All Solos data has been removed from iCloud. This device is now re-uploading its local content to a fresh iCloud zone.")
        }
    ...
    private var dangerZoneSection: some View {
        Section {
            ...
            Text("Permanently erase all Solos data from iCloud. Local data on this device is not affected.")
        }
    }
            ...
            ToolbarItemGroup(placement: .topBarTrailing) {
                Button {
                    ...
                    let tempURL = FileManager.default.temporaryDirectory
                        .appendingPathComponent("solos-sync-diagnostic.txt")
...
private struct SyncLogShareSheet: UIViewControllerRepresentable {
    ...
    func makeUIViewController(context: Context) -> UIActivityViewController {
        // [T-share-sheet-uti] See SolosShareSheet.sanitizedShareURL.
