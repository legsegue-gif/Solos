...
struct BackupRestoreView: View {
    ...
    /// opening a `.solosbak` file from Files rather than from this screen.
    ...
    private var otherSourcesSection: some View {
        Section {
            ...
            Text("Pick a .solosbak file from Files, iCloud Drive, or any connected storage — or add a server your backups were saved to.")
        }
    }
...
/// The `.solosbak` packages inside a single mounted destination folder.
///
    ...
    var body: some View {
        List {
            ...
            if loading {
                ...
                VStack(spacing: 10) {
                    Image(systemName: "externaldrive.badge.questionmark")
                        .font(.system(size: 40))
                        .foregroundStyle(.secondary)
                    Text("No .solosbak files in this folder.")
                        .font(.headline)
            ...
            ForEach(packages) { pkg in
                ...
                if !packages.isEmpty {
                    Text("Showing .solosbak files in this folder.")
                }
            }
        }
        ...
        VStack(spacing: 0) {
            ...
            List {
                Section {
                    ...
                    Text("Open folders to browse. Only .solosbak files are shown.")
                }
            }
            .refreshable { await reload() }
        }
