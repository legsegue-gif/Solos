            ...
            ToolbarItem(placement: .topBarTrailing) {
                Menu {
                    ...
                    Button {
                        ...
                        Label(AppLocalized("Solos Skills"), systemImage: "globe")
                    }
...
// MARK: - Solos Skills Browser

import WebKit

struct SolosSkillsBrowserView: View {
    @Environment(\.dismiss) private var dismiss
...
private struct SkillBrowserWebView: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> WKWebView {
        ...
        config.applicationNameForUserAgent = "SolosApp Mobile Safari"
        let webView = WKWebView(frame: .zero, configuration: config)
        webView.customUserAgent = "Mozilla/5.0 (iPhone; CPU iPhone OS 18_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/18.0 Mobile/15E148 Safari/604.1"
        webView.allowsBackForwardNavigationGestures = true
        // KVO to track URL changes (GitHub SPA uses pushState)
        context.coordinator.observe(webView)
        if let url = URL(string: "https://github.com/legsegue-gif/SolosSkills") {
