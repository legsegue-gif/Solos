...
struct ThinkingRulesSection: View {
    ...
    /// release (Solos#86 Venice, #87 Mistral).
    ///
    ...
    /// explains that thinking parameters here are maintained by Solos by design.
    private var unsupportedNoticeSection: some View {
        Section {
            HStack(alignment: .firstTextBaseline, spacing: 8) {
                Image(systemName: "info.circle")
                    .foregroundStyle(.secondary)
                Text("This provider uses an official protocol, so its thinking parameters are maintained by Solos. Custom rules apply to OpenAI-compatible providers on the Chat Completions API.")
                    .font(.footnote)
                    .foregroundStyle(.secondary)
            }
        } header: {
