...
struct ProviderInstanceDetailView: View {
    ...
    private func customUserAgentSection(_ instance: ProviderInstance) -> some View {
        Section {
            ...
            Text(AppLocalized("Override the User-Agent header sent to this endpoint. Leave empty to use the Solos default. Useful for relays that only accept specific clients (e.g. \"claude-cli/1.0\")."))
        }
    }
    ...
    private func modelEntryRow(_ entry: ModelEntry, isHidden: Bool) -> some View {
        ...
        .contextMenu {
            Button {
                ...
                SolosToast.show(AppLocalized("Copied: \(entry.model.displayName)"))
            } label: {
