...
/// Inline section for ModelGroupsView — manages models available via solos-model-use.
...
struct AgentLoopModelsSection: View {
    ...
    var body: some View {
        Section {
            ...
            HStack {
                Text("Models Solos Can Call at Runtime")
                Spacer()
            ...
            Text("During a Solos task, the agent calls these models for sub-tasks such as generating an image or summarizing text — work its own model can't do. Also callable from the terminal via solos-model-use. Only these are visible to the agent.")
        }
    }
