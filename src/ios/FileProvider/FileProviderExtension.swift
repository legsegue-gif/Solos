...
/// Replicated File Provider extension that exposes SolosFileProvider/ to the system Files app.
/// Structure: Solos → { memory, skills, shared }
/// Uses the modern NSFileProviderReplicatedExtension protocol (iOS 16+).
final class FileProviderExtension: NSObject, NSFileProviderReplicatedExtension {

    let domain: NSFileProviderDomain

    private static let log = OSLog(subsystem: "com.solos.app.FileProvider", category: "Extension")

    /// Root directory for all FileProvider-visible files in the App Group container.
    static var providerRoot: URL {
        let container = FileManager.default.containerURL(
            forSecurityApplicationGroupIdentifier: "group.com.solos.app"
        )!
        let url = container.appendingPathComponent("SolosFileProvider", isDirectory: true)
    ...
    ///      into iOS Files).
    ///   2. `<App Group>/SolosConfig/logs/` — second iteration; private
    ...
    private static func cleanupLegacyLogsDirIfNeeded(root: URL) {
        ...
        // Location 2: under SolosConfig (private but still pure cruft).
        if let container = fm.containerURL(forSecurityApplicationGroupIdentifier: "group.com.solos.app") {
            let inConfig = container.appendingPathComponent("SolosConfig/logs", isDirectory: true)
    ...
    /// already migrated to `SolosConfig/`. If both copies exist the main
    ...
    private static func cleanupLegacyMountedFoldersIfNeeded(root: URL) {
        ...
        // Only delete if the canonical copy already exists under SolosConfig —
        ...
        let canonical = container.appendingPathComponent("SolosConfig/mounted-folders.json")
