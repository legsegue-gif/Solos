...
final class AppDelegate: NSObject, UIApplicationDelegate {
    ...
    func application(
        ...
        // the wrong (new) session. The .onAppear registration in SolosApp
...
final class SceneDelegate: NSObject, UIWindowSceneDelegate {
    ...
    func scene(
        ...
        // (or "Open in Solos") a file, the file:// URL arrives here in
    ...
    // app is ALREADY running, AirDrop / "Open in Solos" delivers the file URL
    ...
    // cold-launch path above and as SolosApp's `.onOpenURL`.
    ...
    /// SolosApp's `.onOpenURL`: a local file goes to `ExternalFileImporter`
    /// (which detects Provider-export JSON and prompts import-vs-attach), and
    /// anything else falls through to `DeepLinkRouter`. Uses
    /// `ShareCoordinator.shared` — the exact instance SolosApp injects into the
    ...
    private static func handleURLContexts(_ contexts: Set<UIOpenURLContext>, phase: String) {
        for context in contexts {
            ...
            Task { @MainActor in
                ...
                // A .solosbak must reach the restore flow, not the attachment
