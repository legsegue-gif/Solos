...
class BackupPackageReaderTest {
    ...
    @Before
    fun setUp() {
        root = File.createTempFile("solosbak-reader", "").apply { delete(); mkdirs() }
    }
    ...
    fun `refuses a future format major version instead of guessing`() {
        writeManifest("""{"format":"solosbak/2","backup_id":"future"}""")
    ...
    fun `accepts a same-major minor bump`() {
        writeManifest("""{"format":"solosbak/1.4","backup_id":"newer-minor"}""")
    ...
    fun `an unsupported encryption scheme is refused`() {
        writeManifest(
            """
            {"format":"solosbak/1","encryption":{"scheme":"solosbak-enc/99",
    ...
    fun `an edited manifest fails the sidecar MAC`() {
        ...
        writeManifest(body.replace("solosbak/1\"", "solosbak/1\",\"device_name\":\"forged\""))
    ...
    fun `a stripped encryption block is caught when encrypted members remain`() {
        writeManifest("""{"format":"solosbak/1"}""")
        File(root, "data").mkdirs()
