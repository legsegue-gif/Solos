package com.solos.app.offload

import com.solos.app.sandbox.offload.OffloadArgs
...
class PhotosImportPathArgTest {
    ...
    fun `positional path is accepted`() {
        assertEquals("/var/solos/offloads/p.jpg", resolvePath(listOf("import", "/var/solos/offloads/p.jpg")))
    }

    @Test
    fun `path flag still works`() {
        assertEquals("/var/solos/offloads/p.jpg", resolvePath(listOf("import", "--path", "/var/solos/offloads/p.jpg")))
    }
