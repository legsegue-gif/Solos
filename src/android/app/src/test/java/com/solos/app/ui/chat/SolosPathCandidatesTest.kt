...
/**
 * [T-android-solos-url-double-encoding] Decode candidates for a `solos://`
 * path.
 *
 * The field report: `solos://workspace/厚升凭证拆分/厚博-2026-03-935.pdf` was
 ...
 * to `ChatLinkAction.Web`, and a web preview of a `solos://` URL renders
 * nothing, so the link simply looks dead.
 *
 * Ported from iOS `SolosURLPathDecoding` (T-fix-double-encoding 2026-06-01),
 * which fixed the same defect there.
 */
class SolosPathCandidatesTest {

    private fun candidates(path: String) = ChatLinkResolver.solosPathCandidates(path)
