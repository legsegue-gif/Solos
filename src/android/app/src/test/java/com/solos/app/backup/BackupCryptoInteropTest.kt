...
/**
 * Cross-platform crypto vectors for `solosbak-enc/1`.
 *
...
class BackupCryptoInteropTest {
    ...
    fun `manifest sidecar MAC matches iOS over identical raw bytes`() {
        val k = keys()
        val raw = """{"format":"solosbak/1","backup_id":"vector"}""".toByteArray(Charsets.UTF_8)
        ...
        val tampered = """{"format":"solosbak/1","backup_id":"forged"}""".toByteArray(Charsets.UTF_8)
        var threw = false
        try {
    ...
    fun `decrypts a member sealed by iOS`() {
        ...
        assertEquals("hello solosbak — 跨平台备份\n", out.toString("UTF-8"))
    }
    ...
    private fun createTempFile() =
        java.io.File.createTempFile("solosbak-test", null).apply { deleteOnExit() }
