package com.solos.app.ui.theme
 ...
 *
 * Solos resolves its own theme from the `theme_mode` preference (0=System,
 * 1=Light, 2=Dark) in MainActivity and hands the answer to `SolosTheme`, which
 ...
 * exactly when the user has overridden the theme in-app — system light + Solos
 * dark being the reported case. A component reading the system API then paints
 * light chrome inside a dark app (GH Solos#187: pop-up menus followed the
 ...
 *     invoke SolosTheme without an explicit value.
 *
...
class InAppThemeSourceGuardTest {
    ...
    fun `ui code reads the in-app theme, not the system theme`() {
        val root = File("src/main/java/com/solos/app")
        assertTrue(
        ...
        assertTrue(
            buildString {
                ...
                append("See GH Solos#187 and commit 8154d9b05.\n\n")
