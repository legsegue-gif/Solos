...
class ExecutionCoordinatorInstrumentedTest {
    ...
    fun executeCreatesSessionBindMounts() = runBlocking {
        ...
        assertTrue(PRootKernel.bindMounts.containsKey("/var/solos/attachments"))
        assertTrue(PRootKernel.bindMounts.containsKey("/var/solos/offloads"))
        assertTrue(PRootKernel.bindMounts.containsKey("/var/solos/workspace"))
        assertTrue(PRootKernel.bindMounts.containsKey("/var/solos/browser"))

        // Verify global mounts
        assertTrue(PRootKernel.bindMounts.containsKey("/var/solos/memory"))
        assertTrue(PRootKernel.bindMounts.containsKey("/var/solos/skills"))
    }
    ...
    fun sessionBindMountsPointToCorrectHostDirs() = runBlocking {
        ...
        assertEquals(
            ...
            PRootKernel.bindMounts["/var/solos/workspace"]
        )

        val globalBase = File(context.filesDir, "solos-global")
        assertEquals(
            File(globalBase, "memory").absolutePath,
            PRootKernel.bindMounts["/var/solos/memory"]
        )
    }
    ...
    fun differentSessionsHaveIsolatedWorkspaces() = runBlocking {
        ...
        ExecutionCoordinator.execute(
            "session-iso-A",
            "echo 'from A' > /var/solos/workspace/test.txt"
        )

        // Check it exists in session A
        val resultA = ExecutionCoordinator.execute(
            "session-iso-A",
            "cat /var/solos/workspace/test.txt"
        )
    ...
    private fun cleanupSessionDirs() {
        listOf("session-mount-test", "session-A", "session-B", "session-X",
            ...
            File(context.filesDir, "solos-sessions/$id").deleteRecursively()
        }
        File(context.filesDir, "solos-global").deleteRecursively()
    }
}
