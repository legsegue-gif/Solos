...
class PRootKernelInstrumentedTest {
    ...
    fun addBindMountStoresMapping() {
        PRootKernel.addBindMount("/var/solos/workspace", "/data/host/workspace")
        assertEquals("/data/host/workspace", PRootKernel.bindMounts["/var/solos/workspace"])
    }

    @Test
    fun removeBindMountRemovesMapping() {
        PRootKernel.addBindMount("/var/solos/workspace", "/data/host/workspace")
        PRootKernel.removeBindMount("/var/solos/workspace")
        assertNull(PRootKernel.bindMounts["/var/solos/workspace"])
    }
    ...
    fun buildProotCommandIncludesBindMounts() = runBlocking {
        ...
        PRootKernel.addBindMount("/var/solos/workspace", "/data/user/0/com.solos.app/workspace")

        val cmd = PRootKernel.buildProotCommand("ls /var/solos/workspace")

        // Should contain -b host:linux format
        val bindStr = "/data/user/0/com.solos.app/workspace:/var/solos/workspace"
    ...
    fun resolveHostPathMatchesExactMount() {
        PRootKernel.addBindMount("/var/solos/workspace", "/host/workspace")

        val result = PRootKernel.resolveHostPath("/var/solos/workspace")
    ...
    fun resolveHostPathMatchesSubpath() {
        ...
        val result = PRootKernel.resolveHostPath("/var/solos/workspace/file.txt")
        assertNotNull(result)
    ...
    fun resolveHostPathUsesLongestPrefixMatch() {
        ...
        val result = PRootKernel.resolveHostPath("/var/solos/workspace/deep/file.txt")
        assertNotNull(result)
    ...
    fun resolveHostPathDoesNotMatchPartialPrefix() {
        PRootKernel.addBindMount("/var/solos/work", "/host/work")

        // "/var/solos/workspace" should NOT match "/var/solos/work" (no / after "work")
        val result = PRootKernel.resolveHostPath("/var/solos/workspace/file.txt")
        // Should fallback to rootfs or null, not match /var/solos/work
