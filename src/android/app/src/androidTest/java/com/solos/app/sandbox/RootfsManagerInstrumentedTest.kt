...
class RootfsManagerInstrumentedTest {
    ...
    fun installIfNeededExtractsRootfsFromAssets() = runBlocking {
        ...
        // Verify /var/solos subdirs created
        val expectedDirs = listOf("attachments", "offloads", "workspace", "skills", "memory")
        for (subdir in expectedDirs) {
            val dir = File(manager.rootfsDir, "var/solos/$subdir")
            assertTrue("var/solos/$subdir should exist", dir.exists())
            assertTrue("var/solos/$subdir should be directory", dir.isDirectory)
        }
