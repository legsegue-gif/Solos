# SolosApp shell configuration
# Loaded by /etc/profile via the profile.d mechanism (login shells only).

# T294: prompt parity with iOS — `root@solos:/var/solos#`. iOS bakes the
# literal "solos" into PS1 (deps/prepare_alpine_rootfs.sh) rather than
...
# boot.
export BROWSER=/usr/local/bin/solos-open
...
# touched as link sources. Reported as solos/solos#7.
