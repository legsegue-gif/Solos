#!/usr/bin/env python3
"""solos-mcp-cli — MCP client CLI for the Solos agent (iSH / PRoot).
...
Diagnostics go to /var/solos/mcp-servers/mcp-cli.log, never to stdout.
...
published in /tmp/solos-mcp-daemon.port. info / add / remove / enable / disable
...
LOG_PATH = "/var/solos/mcp-servers/mcp-cli.log"

PID_FILE = "/tmp/solos-mcp-daemon.pid"
PORT_FILE = "/tmp/solos-mcp-daemon.port"  # daemon publishes its 127.0.0.1 port here
LOCK_FILE = "/tmp/solos-mcp-daemon.lock"  # cold-start fork guard (one winner forks)
...
def maybe_start_daemon():
    ...
    try:
        ...
        import logging
        daemon_log = "/var/solos/mcp-servers/mcp-daemon.log"
        try:
...
def cmd_add(args, pretty):
    ...
    if wants_oauth:
        out["oauth_note"] = (
            ...
            "the CLI — open Solos Settings > MCP Integrations > %s and tap "
...
USAGE = """solos-mcp-cli — MCP (Model Context Protocol) client for the Solos agent.

Usage: solos-mcp-cli <command> [args] [--pretty]
...
Commands:
  ...
  add ... --oauth-client-id <id> --oauth-auth-endpoint <url> --oauth-token-endpoint <url>
      [--oauth-client-secret <secret>] [--oauth-scopes "s1 s2"] [--oauth-redirect-uri <uri>]
                                        ...
                                        Solos Settings > MCP Integrations >
                                        <server> > Authorize.
...
Files:
  Servers:  /var/solos/mcp-servers/servers.json   (mcpServers object, Claude-Desktop compatible)
  Log:      /var/solos/mcp-servers/mcp-cli.log

Examples:
  solos-mcp-cli list --pretty
  solos-mcp-cli tools notion
  solos-mcp-cli call notion search --input '{"q":"x"}'
  solos-mcp-cli add --name notion --url https://mcp.notion.so/mcp --header "Authorization: Bearer $NOTION_TOKEN"
  solos-mcp-cli add --name github --command npx --args "-y @modelcontextprotocol/server-github" --env "GITHUB_TOKEN=$GITHUB_TOKEN"
  solos-mcp-cli add --name atlassian --command uvx --args "mcp-atlassian" --startup-timeout 120
  solos-mcp-cli add --name gworkspace --url https://my-gws-mcp.example.com/mcp \
...
Server startup timeout (Solos config, not part of the MCP protocol):
...
def main():
    ...
    try:
        if cmd == "list":
            ...
            sys.stderr.write("Run 'solos-mcp-cli --help' for usage.\n")
