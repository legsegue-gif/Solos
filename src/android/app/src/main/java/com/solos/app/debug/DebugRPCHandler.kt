...
class DebugRPCHandler(private val context: Context) {
    ...
    private suspend fun dispatch(method: String, params: JSONObject): Any {
        return when (method) {
            ...
            // harnesses can verify solos-sessions-cli (list / search /
            ...
            // [T-solos-config-provider-add] DEBUG-only invocation of the
            // ConfigOffloadHandler — parallels debug.modelUse.exec so test
            // harnesses can exercise solos-config (get / set / set-batch /
            ...
            "debug.solosConfig.exec" -> {
                if (!BuildConfig.DEBUG) {
                    throw RPCException(-32601, "Method not found: $method. Call 'rpc.discover' to list available methods.")
                }
                handleSolosConfigExec(params)
            }
    ...
    private fun handleAppInfo(): JSONObject {
        ...
        return JSONObject().apply {
            ...
            put("diskUsage", JSONObject().apply {
                ...
                put("sessions", dirSize(File(filesDir, "solos-sessions")))
                put("global", dirSize(File(filesDir, "solos-global")))
            })
        }
    }
     ...
     * at arbitrary paths. Use `solos-sessions` (default) to enumerate every
    ...
    private fun handleRawLS(params: JSONObject): Any {
        val subPath = params.optString("path", "solos-sessions")
    ...
    private fun crashSummary(f: File): String = try {
        ...
        val frame = body.firstOrNull { it.trimStart().startsWith("at com.solos") }
     ...
     * `if (BuildConfig.DEBUG)` in SolosApp, so no release build carries this.
     */
    private suspend fun handleSetClipboard(params: JSONObject): JSONObject {
        val text = params.optString("text")
        if (text.isEmpty()) throw RPCException(-32602, "Invalid params: 'text' is required")
        val label = params.optString("label", "solos-debug")
     ...
     * (`solos-browser-use`, `solos-open`, …) without going through the agent.
     *
     * Params:
    ...
    private suspend fun handleShellExecute(params: JSONObject): JSONObject {
        ...
        // OSC SolosOpenURL markers before TerminalSanitizer strips them and
        // hand captured URLs to the broker so test harnesses driving
        // `solos-open` via this RPC trigger the same in-app preview flow as
        ...
        val result = try {
            ExecutionCoordinator.execute(
                ...
                lineCallback = { rawLine ->
                    val (_, urls) = com.solos.app.terminal.SolosUrlMarker.extract(rawLine)
        ...
        for (raw in capturedUrls) {
            com.solos.app.terminal.SolosOpenUrlBroker.offer(raw)
        }
        return JSONObject()
    ...
    private suspend fun handleUpdateCheck(): JSONObject {
        return when (val r = com.solos.app.data.UpdateChecker.check()) {
            is com.solos.app.data.UpdateChecker.CheckResult.UpdateAvailable -> JSONObject()
            ...
            com.solos.app.data.UpdateChecker.CheckResult.UpToDate ->
                JSONObject().put("status", "up_to_date")
            com.solos.app.data.UpdateChecker.CheckResult.NoReleaseAvailable ->
                JSONObject().put("status", "no_release")
            is com.solos.app.data.UpdateChecker.CheckResult.NoApkAsset ->
                JSONObject().put("status", "no_apk_asset").put("tag_name", r.tagName)
            com.solos.app.data.UpdateChecker.CheckResult.Forbidden ->
                JSONObject().put("status", "forbidden")
            com.solos.app.data.UpdateChecker.CheckResult.NetworkUnreachable ->
                JSONObject().put("status", "network_unreachable")
            is com.solos.app.data.UpdateChecker.CheckResult.Error ->
    ...
    private suspend fun handleUpdateDownload(params: JSONObject): JSONObject {
        ...
        return when (val r = com.solos.app.data.UpdateChecker.download(context, url)) {
            is com.solos.app.data.UpdateChecker.DownloadResult.Success -> JSONObject()
                .put("status", "ok")
                .put("path", r.file.absolutePath)
                .put("size", r.file.length())
            is com.solos.app.data.UpdateChecker.DownloadResult.Error -> JSONObject()
                .put("status", "error")
    ...
    private fun handleUpdateInstall(params: JSONObject): JSONObject {
        ...
        val canInstall = com.solos.app.data.UpdateChecker.canInstall(context)
        return try {
            com.solos.app.data.UpdateChecker.installApk(context, file)
     ...
     * iCloud-equivalent built into SolosApp, so we report disabled and an
    ...
    private fun handleWriteFile(params: JSONObject): JSONObject {
        ...
        val hostFile = PRootKernel.resolveHostPath(path) ?: run {
            val rootfsDir = com.solos.app.sandbox.RootfsManager.getInstance(context).rootfsDir
    ...
    private fun handlePermissionsList(): JSONObject {
        ...
        for (tool in com.solos.app.offload.OffloadPermissionManager.toolRegistry) {
            arr.put(JSONObject()
                ...
                .put("currentLevel", com.solos.app.offload.OffloadPermissionManager.getLevel(tool.toolName).name)
    ...
    /**
     * T344: Direct invocation of [com.solos.app.sandbox.offload.ShizukuOffloadHandler]
    ...
    private fun handleShizukuExec(params: JSONObject): JSONObject {
        ...
        val handler = com.solos.app.sandbox.offload.ShizukuOffloadHandler(context)
        ...
        val request = com.solos.app.sandbox.NativeOffloadRequest(
            pid = -1,
    ...
    /**
     * Direct invocation of [com.solos.app.sandbox.offload.ModelUseOffloadHandler]
     * for e2e harnesses. Mirrors [handleShizukuExec]; lets callers exercise the
     * `solos-model-use` CLI without going through a real Alpine shell prompt.
     * DEBUG-only.
     */
    private fun handleModelUseExec(params: JSONObject): JSONObject {
        ...
        val finalArgv: List<String> = if (params.has("input")) {
            ...
            val hostFile = com.solos.app.sandbox.PRootKernel.resolveHostPath(linuxPath)
        ...
        val app = context.applicationContext as com.solos.app.SolosApp
        val handler = com.solos.app.sandbox.offload.ModelUseOffloadHandler(context, app.providerRepository)
        val request = com.solos.app.sandbox.NativeOffloadRequest(
            pid = -1,
            argv = listOf("solos-model-use") + finalArgv,
            env = emptyMap(),
            cwd = "/",
            sessionId = null,
        )
     ...
     * [com.solos.app.sandbox.offload.SessionsOffloadHandler] for e2e
     * harnesses. Mirrors [handleModelUseExec]; lets callers exercise the
     * `solos-sessions-cli` CLI (list / search / messages, incl. --full)
    ...
    private fun handleSessionsExec(params: JSONObject): JSONObject {
        ...
        val app = context.applicationContext as com.solos.app.SolosApp
        val handler = com.solos.app.sandbox.offload.SessionsOffloadHandler(app.chatRepository)
        val request = com.solos.app.sandbox.NativeOffloadRequest(
            pid = -1,
            argv = listOf("solos-sessions-cli") + argvTail,
            env = emptyMap(),
            cwd = "/",
            sessionId = null,
        )
    ...
    /**
     * [T-solos-config-provider-add] DEBUG-only solos-config invocation
     ...
     */
    private fun handleSolosConfigExec(params: JSONObject): JSONObject {
        ...
        return when (sub) {
            "set" -> {
                ...
                AppLogger.info(
                    "DebugRPC",
                    "debug.solosConfig.exec set items=${items.length()} skipConfirmation=$skip",
                )
                // Hop to the main thread because performWriteBatch is a
                // suspend fun that uses Dispatchers.Main internally.
                kotlinx.coroutines.runBlocking {
                    com.solos.app.config.ConfigBridge.performWriteBatch(
                        items = items,
                        caption = "debug.solosConfig.exec",
                        actorRaw = "debug-rpc",
                        sessionId = null,
            ...
            "get" -> {
                ...
                AppLogger.info("DebugRPC", "debug.solosConfig.exec get path=$path")
                com.solos.app.config.ConfigBridge.readField(
                    path = path,
                    filter = filter,
                    page = page,
                    pageSize = pageSize,
            ...
            // writable paths in advance; `topics` is `solos-config --help`'s
            // index and `topic-help` is `solos-config <topic> --help`.
            "topics" -> JSONObject().apply {
                put("ok", true)
                put("topics", com.solos.app.config.ConfigBridge.allTopics())
            }
            "topic-help" -> {
            ...
            "audit-list" -> {
                ...
                com.solos.app.config.ConfigBridge.auditList(limit, scope)
            }
