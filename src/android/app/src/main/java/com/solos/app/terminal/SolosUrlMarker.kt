package com.solos.app.terminal

/**
 * Parses the OSC 1337 `SolosOpenURL` escape sequence emitted by the rootfs
 * shim at `/usr/local/bin/solos-open`. The shim stands in for `xdg-open`,
 ...
 * http(s)/about, file preview for solos://).
 *
 * Mirrors iOS `SolosURLMarker` in `AIChatViewModel.swift`.
 */
object SolosUrlMarker {
    /**
     * ESC ] 1337 ; SolosOpenURL = <url> (BEL | ESC \)
     *
     ...
     */
    private val PATTERN = Regex("\u001B]1337;SolosOpenURL=([^\u0007\u001B]*)(?:\u0007|\u001B\\\\)")
    ...
    fun extract(text: String): Pair<String, List<String>> {
        if (!text.contains("SolosOpenURL=")) return text to emptyList()
    ...
    fun stripMarkers(text: String): String {
        if (!text.contains("SolosOpenURL=")) return text
    ...
    fun extractUrl(text: String): String? {
        if (!text.contains("SolosOpenURL=")) return null
