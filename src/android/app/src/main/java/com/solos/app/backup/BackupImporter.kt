...
/**
 * Restores a `.solosbak` package on Android (docs/backup-restore-design.md §8),
...
class BackupImporter(
    ...
    private suspend fun importChats(
        ...
        val sessionsRoot = File(context.filesDir, "solos-sessions")
    ...
    private fun importSharedFiles(
        ...
        val dest = File(context.filesDir, "solos-global/shared")
        val files = BackupRestoreFiles.restore(
    ...
    private fun importSkills(root: File, fileIndex: List<BackupFileIndexEntry>): CategoryReport {
        ...
        val dest = File(context.filesDir, "solos-global/skills")
    ...
    private fun importMemory(root: File): CategoryReport {
        ...
        val dest = File(context.filesDir, "solos-global/memory").apply { mkdirs() }
    ...
    private fun importMcpServers(root: File): CategoryReport {
        ...
        val dest = File(context.filesDir, "solos-global/mcp-servers/servers.json")
    ...
    companion object {
        ...
        internal data class LenientProviderConfig(
            val config: com.solos.app.data.model.ProviderConfig,
        ...
        internal fun parseProviderConfigLeniently(text: String): LenientProviderConfig {
            ...
            if (rawInstances == null) {
                return LenientProviderConfig(
                    BackupFormat.json.decodeFromJsonElement(
                        com.solos.app.data.model.ProviderConfig.serializer(), root,
                    ),
                    droppedInstances = 0,
                )
            }

            val knownTypes = com.solos.app.data.model.ProviderType.entries.map { it.name }.toSet()
            ...
            for (element in rawInstances) {
                ...
                val normalized = if (rawType != null && rawType !in knownTypes) {
                    ...
                    JsonObject(
                        obj + ("providerType" to kotlinx.serialization.json.JsonPrimitive(
                            com.solos.app.data.model.ProviderType.unsupported.name,
                        )),
                    )
                } else obj
                ...
                val decodes = runCatching {
                    BackupFormat.json.decodeFromJsonElement(
                        com.solos.app.data.model.ProviderInstance.serializer(), normalized,
                    )
                }
            ...
            return LenientProviderConfig(
                BackupFormat.json.decodeFromJsonElement(
                    com.solos.app.data.model.ProviderConfig.serializer(), patched,
                ),
