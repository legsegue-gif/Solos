...
/**
 * Passphrase-based encryption for `.solosbak` packages
 * (docs/backup-restore-design.md §5, scheme `solosbak-enc/1`).
 *
 ...
 *     block) with infos `solosbak/{data,secrets,mac,verify}`.
...
object BackupCrypto {
    ...
    const val SCHEME = "solosbak-enc/1"
    ...
    class Keys internal constructor(kek: ByteArray) {
        ...
        val dataKey: ByteArray = hkdfSha256(kek, "solosbak/data")
        /**
         ...
         */
        val secretsKey: ByteArray = hkdfSha256(kek, "solosbak/secrets")
        /** Authenticates manifest.json. */
        val macKey: ByteArray = hkdfSha256(kek, "solosbak/mac")
        /** Answers "is this passphrase right?" without touching any payload. */
        val verifierKey: ByteArray = hkdfSha256(kek, "solosbak/verify")
        ...
        /**
         * `HMAC(verifier_key, "solosbak-v1")` truncated to 16 bytes, base64
        ...
        val verifier: String
            get() {
                val mac = hmacSha256(verifierKey, "solosbak-v1".toByteArray(Charsets.UTF_8))
