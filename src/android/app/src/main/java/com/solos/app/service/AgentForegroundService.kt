...
class AgentForegroundService : Service() {
    ...
    companion object {
        ...
        private const val ACTION_STOP = "com.solos.app.STOP_AGENT_SERVICE"
    ...
    override fun onCreate() {
        ...
        // SolosApp.onCreate, the Application skipped its lateinit init
        // for repositories — but a sticky FG service that was running
        // pre-crash will still be re-created by the system on the next
        // process spawn. Reading SolosApp.backgroundSettingsRepository
    ...
    private fun startOverlayObserver() {
        val app = applicationContext as? SolosApp ?: return
    ...
    private fun buildNotification(sessionCount: Int, toolStatus: String): Notification {
        ...
        val mainIntent = Intent(this, Class.forName("com.solos.app.MainActivity")).apply {
        ...
        // Title:   "Solos is using <Tool>"  (or session-count summary when idle/between turns)
        ...
        // through to the generic "Solos is running" title while the icon fell
        ...
        // by the system with no Activity, so it can observe a SolosApp whose
        ...
        val solosApp = (applicationContext as? SolosApp)?.takeIf { it.subsystemsReady() }
        val dynamicIslandUserEnabled =
            solosApp?.backgroundSettingsRepository?.dynamicIslandEnabled?.value == true
    ...
    private fun toolDisplayLabel(toolName: String): String = when (toolName) {
        "shell_execute" -> "Solos is using Shell"
        "file_read" -> "Solos is reading File"
        "file_write" -> "Solos is using Editor"
        "file_edit" -> "Solos is editing File"
        "browser_use" -> "Solos is using Browser"
        "read_image" -> "Solos is reading Image"
        "memory_write", "memory_get" -> "Solos is using Memory"
        "web_search" -> "Solos is using Search"
        else -> "Solos is using $toolName"
    }
