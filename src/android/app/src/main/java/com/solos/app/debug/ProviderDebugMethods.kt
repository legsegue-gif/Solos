...
internal object ProviderDebugMethods {
    ...
    private fun repo(context: Context): ProviderRepository {
        val app = context.applicationContext as? SolosApp
            ?: throw RPCException(-32000, "SolosApp not initialized")
            ...
            if (includeMembers) {
                ...
                for (memberId in group.memberEntryIds) {
                    ...
                    val entry = cfg.modelEntries.find { it.id == memberId }
                        ?: com.solos.app.data.model.SystemVoiceEntries.resolve(memberId)
                        ?: continue
    ...
    suspend fun quickTest(context: Context, params: JSONObject): JSONObject {
        ...
        val entry = repo.config.value.modelEntries.find { it.id == entryId }
            ?: com.solos.app.data.model.SystemVoiceEntries.resolve(entryId)
            ?: throw RPCException(-32602, "Model entry not found: $entryId")

        val requested = params.optJSONArray("kinds")
        val kinds: List<com.solos.app.ui.components.QuickTestKind> = if (requested == null) {
            com.solos.app.ui.components.applicableKinds(entry)
        } else {
            (0 until requested.length()).mapNotNull { i ->
                when (requested.optString(i).lowercase()) {
                    "text" -> com.solos.app.ui.components.QuickTestKind.TEXT
                    "speechout", "speech_out", "tts" -> com.solos.app.ui.components.QuickTestKind.SPEECH_OUT
                    "transcription", "asr" -> com.solos.app.ui.components.QuickTestKind.TRANSCRIPTION
                    "imagegen", "image_gen", "image" -> com.solos.app.ui.components.QuickTestKind.IMAGE_GEN
                    else -> null
                }
            }
        }
        ...
        for (kind in kinds) {
            ...
            val state = try {
                com.solos.app.ui.components.performTest(kind, entry, repo, context)
            } catch (e: Exception) {
                com.solos.app.ui.components.QuickTestState.Failure(
            ...
            results.put(JSONObject().apply {
                ...
                when (state) {
                    is com.solos.app.ui.components.QuickTestState.TextReply -> {
                        put("status", "ok"); put("detail", state.text.take(400))
                    }
                    is com.solos.app.ui.components.QuickTestState.AudioReply -> {
                        put("status", "ok")
                        put("bytes", state.data.size)
                        put("detail", "audio ${state.data.size} bytes")
                    }
                    is com.solos.app.ui.components.QuickTestState.ImageReply -> {
                        put("status", "ok")
                        put("bytes", state.data.size)
                        put("detail", "image ${state.data.size} bytes")
                    }
                    is com.solos.app.ui.components.QuickTestState.Failure -> {
