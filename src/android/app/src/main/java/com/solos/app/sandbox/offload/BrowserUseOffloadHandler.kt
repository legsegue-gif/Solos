...
import com.solos.app.SolosApp
import com.solos.app.browser.BrowserAction
import com.solos.app.browser.BrowserActionInput
import com.solos.app.browser.BrowserActionResult
...
/**
 * solos-browser-use — expose the agent's browser_use tool as a CLI inside the
 * PRoot sandbox.
 *
 ...
 * Screenshots are persisted under `/var/solos/browser/` and referenced via
 * `image_path` + `solos_url` — raw base64 is only included when the caller
 ...
 */
class BrowserUseOffloadHandler(private val app: SolosApp) : NativeOffloadHandler {
    ...
    override fun handle(request: NativeOffloadRequest): NativeOffloadResult {
        ...
        com.solos.app.service.SessionActivityTracker.updateToolStatus(
        ...
        val result: BrowserActionResult = try {
            runBlocking {
                withTimeout(EXECUTE_TIMEOUT_MS) {
                    ...
                    // (`solos-browser-use`) is a serial human/script driver:
    ...
    private fun buildInputJson(args: OffloadArgs): JSONObject {
        ...
        // (mirrors `solos-config set --file`). Parse failures throw so the
    ...
    private fun encodeData(r: BrowserActionResult, withBase64: Boolean): JSONObject {
        ...
        // Persist screenshot bytes under /var/solos/browser/ so shells can
        // reference the JPEG via image_path + solos_url instead of piping
        // base64 through stdout.
        val browserHostDir: File? = PRootKernel.resolveHostPath(VAR_SOLOS_BROWSER)?.also {
        ...
        if (!base64.isNullOrEmpty() && browserHostDir != null) {
            ...
            if (bytes != null) {
                ...
                if (runCatching { dest.writeBytes(bytes) }.isSuccess) {
                    val linuxPath = "$VAR_SOLOS_BROWSER/$filename"
                    persistedImagePath = linuxPath
                    out.put("image_path", linuxPath)
                    out.put("solos_url", "solos://browser/$filename")
                } else {
                    Log.w(TAG, "Failed to persist screenshot to ${dest.absolutePath}")
                }
            }
        } else if (!base64.isNullOrEmpty()) {
            Log.w(TAG, "No /var/solos/browser mount — falling back to base64-only output")
        }
            ...
            if (data != null) {
                ...
                if (browserHostDir != null) {
                    ...
                    if (runCatching { dest.writeBytes(data) }.isSuccess) {
                        out.put("fetched_path", "$VAR_SOLOS_BROWSER/$fname")
                        out.put("fetched_solos_url", "solos://browser/$fname")
                    } else {
    ...
    companion object {
        ...
        private const val TOOL_NAME = "solos-browser-use"
        private const val VAR_SOLOS_BROWSER = "/var/solos/browser"
        ...
        private val HELP = """
solos-browser-use - Drive the in-app WebView from the shell

USAGE:
  solos-browser-use <action> [options]
  solos-browser-use --json '<json>'
  solos-browser-use --help
...
COMMON OPTIONS:
  ...
  --with-base64    Also include image_base64 in the screenshot output. Off by
                   default — screenshots are persisted to /var/solos/browser/
                   and referenced via image_path + solos_url.
...
OUTPUT:
  JSON envelope with a `data` object containing:
    ...
    image_path        Linux path of the persisted JPEG under /var/solos/browser/
    solos_url         solos://browser/<filename> — stable reference for
    ...
    fetched_path      Linux path of the persisted download under /var/solos/browser/
    fetched_solos_url solos://browser/<filename> for the download

EXAMPLES:
  solos-browser-use navigate --url https://example.com
  solos-browser-use screenshot
  solos-browser-use click --selector '.btn-primary'
  solos-browser-use type --selector 'input[name=q]' --text 'hello'
  solos-browser-use execute_js --script 'return document.title'
  solos-browser-use --json '{"action":"navigate","url":"https://x.com"}'
""".trimIndent() + "\n"
    }
}
