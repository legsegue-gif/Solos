...
/**
 * solos-debug — DEBUG-ONLY CLI wrapper for the local DebugServer JSON-RPC
 ...
 * `android-*` / `solos-*` offload.
 *
 * Registration is guarded by `BuildConfig.DEBUG` in SolosApp.kt so the
...
class DebugOffloadHandler(@Suppress("UNUSED_PARAMETER") context: Context) : NativeOffloadHandler {
    ...
    override fun handle(request: NativeOffloadRequest): NativeOffloadResult {
        ...
        // Per-subcommand --help: `solos-debug ls --help` prints the same
        ...
        val rpc: RpcCall = try {
            ...
            return NativeOffloadResult(2, "solos-debug $sub: ${e.message}\n")
        }
    ...
    private fun buildRpcCall(sub: String, args: OffloadArgs): RpcCall {
        ...
        return when (sub) {
            ...
            // `solos-debug call <method> [--params '<json>']`
            "call" -> {
            ...
            else -> throw BadArgsException("unknown subcommand '$sub' (try `solos-debug --help`)")
        }
    }
    ...
    companion object {
        ...
        private const val HELP = """solos-debug — CLI for the in-app DebugServer JSON-RPC (debug builds only)

Usage:
  solos-debug <subcommand> [options]
...
Android-only (DEBUG_ONLY_METHODS in DebugMethodRegistry):
  ...
  model-use <argv...>                Invoke solos-model-use (debug.modelUse.exec)
...
EXAMPLES:
  ...
  solos-debug appInfo
  solos-debug ls /sdcard --recursive --maxDepth 2
  solos-debug read /etc/passwd --offset 0 --limit 100
  solos-debug exec uname -a
  solos-debug call debug.logs.list --params '{}'
