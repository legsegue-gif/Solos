 ...
 * [com.solos.app.provider.openai.OpenAIProvider]). Gemini and Anthropic have their
...
sealed interface ThinkingWireFormat {
     ...
     *
     * Mistral (GH Solos#87): `AssistantMessage` is a closed schema and the request
     * rejects `reasoning` with `422 extra_forbidden`. Venice (GH Solos#86) is the
     ...
     * official endpoint reasons by default and its real switch lives here; Solos never
     * sent it, so the official endpoint always ran its default config (GH Solos#171).
     */
