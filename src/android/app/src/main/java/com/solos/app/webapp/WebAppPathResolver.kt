package com.solos.app.webapp

import android.content.Context
import com.solos.app.data.db.WebAppShortcutEntity
 ...
 *      under /var/solos/<sub>/ are also accepted via resolveSessionHostPath)
...
object WebAppPathResolver {
     ...
     *
     * Mapping rules:
     *  - `/var/solos/shared` bind  → `pathScope = "shared"`,  `scopeContext = null`
     *  - `/var/solos/mounts/<n>`   → `pathScope = "mount"`,   `scopeContext = "<n>"`
    ...
    fun inferScope(hostFile: File): Triple<String, String?, String>? {
        ...
        val sorted = com.solos.app.sandbox.PRootKernel
        ...
        for ((linuxPrefix, hostBase) in sorted) {
            ...
            if (hostAbs == baseNorm || hostAbs.startsWith("$baseNorm/")) {
                ...
                return when {
                    linuxPrefix == "/var/solos/shared" ->
                        Triple(WebAppShortcutRepository.SCOPE_SHARED, null, linuxPath)
                    linuxPrefix.startsWith("/var/solos/mounts/") -> {
                        val mountName = linuxPrefix.removePrefix("/var/solos/mounts/")
                            .substringBefore('/')
    ...
    private fun resolveSession(context: Context, shortcut: WebAppShortcutEntity): File? {
        ...
        // Absolute /var/solos/<perSession>/... — go through PRootKernel which
        // knows how to map per-session subdirs to <filesDir>/solos-sessions/<id>/<sub>.
        if (shortcut.htmlPath.startsWith("/var/solos/")) {
