...
/**
 * Coil Fetcher that resolves `solos://` URIs to local files.
 *
 * Usage: Register with ImageLoader.Builder().components {
 *     add(SolosImageFetcher.Factory())
 * }
 *
 * solos://attachments/foo.jpg → /var/solos/attachments/foo.jpg → host path
 */
class SolosImageFetcher(
    private val uri: String,
    private val options: Options,
) : Fetcher {

    override suspend fun fetch(): FetchResult {
        // Strip solos:// prefix, drop any ?query, and percent-decode so
        // Chinese/emoji/space filenames resolve to the actual on-disk file.
        val stripped = uri.removePrefix("solos://").substringBefore('?')
    ...
    class Factory : Fetcher.Factory<String> {
        override fun create(data: String, options: Options, imageLoader: ImageLoader): Fetcher? {
            if (!data.startsWith("solos://")) return null
            return SolosImageFetcher(data, options)
        }
    }

    /**
     * Uri factory — the path the Markdown renderer hits. Coil's default
     * StringMapper converts an `AsyncImage(model = "solos://…")` String into
    ...
    class UriFactory : Fetcher.Factory<Uri> {
        override fun create(data: Uri, options: Options, imageLoader: ImageLoader): Fetcher? {
            if (data.scheme != "solos") return null
            return SolosImageFetcher(data.toString(), options)
        }
    }

    /**
     * T-image-cache-mtime-35133: Bust Coil's memory + disk cache when a
     * `solos://` file is overwritten in-place (e.g. Grok regenerating an
     ...
     *
     * The key composes `<solos-uri>?mt=<lastModified>`. The fetcher above
     ...
     * to Coil's default keying, which is correct for non-solos data.
     *
