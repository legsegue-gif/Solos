 ...
 *
 * Mirrors iOS `AIChatViewModel.solosOffloadsPersistentDir(for:)`,
 ...
 *
 * Linux-visible mount: `/var/solos/offloads/tools/<file>`. The host base
 * `filesDir/solos-sessions/<sid>/offloads` is bind-mounted into the
 * sandbox by [com.solos.app.sandbox.PRootKernel.perSessionSubdirs]
 * (which already includes the "offloads" subdir — no kernel changes
 * required for this feature).
 */
object ContextOffload {
    /** Linux-side mount point — keep in lock-step with iOS `solosOffloadsLinuxDir`. */
    const val LINUX_OFFLOADS_DIR = "/var/solos/offloads"
    ...
    fun toolsDir(context: Context, sessionId: String): File =
        File(context.filesDir, "solos-sessions/$sessionId/offloads/tools")
