...
class DebugServer(
    ...
    fun start() {
        ...
        acceptJob = scope.launch {
            try {
                ...
                Log.i(TAG, "Remote (non-loopback) clients must send X-Solos-Token: $authToken")
        ...
        socket.use { s ->
            try {
                ...
                while (true) {
                    ...
                    // [T-android-debugserver-auth] Token via X-Solos-Token or
                    // Authorization: Bearer — either spelling accepted.
                    if (lower.startsWith("x-solos-token:")) {
                ...
                if (!isAuthorized(isLoopback, providedToken, authToken)) {
                    ...
                    sendResponse(writer, 401, rpcHandler.errorJSON(-32000, "Unauthorized — send X-Solos-Token (see `adb shell run-as com.solos.app cat files/debug_server_token`)"))
                    return
                }
                ...
                if (method == "GET") {
                    ...
                    when (path) {
                        ...
                        "/skill/examples/python", "/skill/examples/solos_rpc_android.py" -> {
                            sendSkillAsset(writer, "examples/solos_rpc_android.py", "text/x-python; charset=utf-8")
                            return
                        }
    ...
    private fun schemaJSON(): String {
        ...
        return """{"app":"SolosApp","platform":"android","version":"$version",""" +
    ...
    private fun sendSkill(writer: PrintWriter, wantsHuman: Boolean) {
        ...
        val py = readSkillAsset("examples/solos_rpc_android.py") ?: ""
    ...
    private fun sendResponse(
        ...
        writer.print("Access-Control-Allow-Headers: Content-Type, X-Solos-Token, Authorization\r\n")
        writer.print("\r\n")
        writer.print(body)
        writer.flush()
    }
