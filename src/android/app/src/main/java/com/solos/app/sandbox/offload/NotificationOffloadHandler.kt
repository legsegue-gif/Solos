...
import com.solos.app.offload.SolosNotificationListenerService
import com.solos.app.offload.OffloadPermissionManager
import com.solos.app.offload.ScheduledNotificationReceiver
import com.solos.app.offload.ScheduledNotificationStore
...
class NotificationOffloadHandler(private val context: Context) : NativeOffloadHandler {
    ...
    private fun handleSend(args: OffloadArgs): NativeOffloadResult {
                ...
                if (r == OffloadPermissionManager.AndroidPermissionResult.DENIED) {
                    ...
                    r = OffloadPermissionManager.requestSettingsGate(
                        OffloadPermissionManager.SettingsGateRequest(
                            ...
                            message = "Solos needs notification permission to send notifications. Open Settings to allow it.",
        ...
        if (SolosNotificationListenerService.isEnabled(context) &&
            SolosNotificationListenerService.isConnected()
        ) {
            val seen = SolosNotificationListenerService.awaitPosted(context.packageName, notifId)
            if (!seen) {
        ...
        val oemHint = if (OsCompat.isHuawei || OsCompat.isXiaomi) {
            "On ${OsCompat.oemLabel()}, banner notifications may be disabled by default — the user can enable them in Settings → Notifications for Solos."
        } else null
        val data = JSONObject()
            .put("id", id)
            .put("title", title)
    ...
    private fun handleSettings(args: OffloadArgs): NativeOffloadResult {
        ...
        data.put("listener_access", SolosNotificationListenerService.isEnabled(context))
        AppLogger.info(TAG, "settings: can_post=$canPost listener=${SolosNotificationListenerService.isEnabled(context)}")
        return NativeOffloadResult(0, OffloadOutput.formatBody(data.toString(2), args) + "\n")
    }
    ...
    private fun handleList(args: OffloadArgs): NativeOffloadResult {
            ...
            val result = runBlocking {
                OffloadPermissionManager.requestSettingsGate(
                    OffloadPermissionManager.SettingsGateRequest(
                        ...
                        message = "Solos needs Notification access to read the status-bar notifications. Open Settings and enable \"Solos\" under Notification access.",
                        settingsAction = SolosNotificationListenerService.SETTINGS_ACTION,
                        requiresPackageUri = false,
                        positiveLabel = "Open Settings",
                    ),
                    check = { SolosNotificationListenerService.isEnabled(context) },
                )
            }
            when (result) {
                OffloadPermissionManager.AndroidPermissionResult.GRANTED -> {} // continue
        ...
        val active: Array<StatusBarNotification> =
            SolosNotificationListenerService.getActiveNotifications()
                ?: return NativeOffloadResult(
                    77,
                    OffloadOutput.formatBody(
                        JSONObject()
    ...
    companion object {
        ...
        private const val CHANNEL_ID = "solos_agent_notifications"
...
Notes:
  ...
  - list needs Notification access (Settings → Apps → Special app
    access → Notification access → Solos). First list call opens it.
"""
    }
}
