package com.solos.app.backup.remote

import android.content.Context
import com.solos.app.backup.BackupFormat
 ...
 * `.solos-parts/<backupId>/` with a resume journal, because rclone has no
 ...
 * and invisible to the iOS restore picker, which lists `.solosbak` files only.
 ...
 * self-contained `.solosbak` a user can grab with any client, and an
 * interrupted upload simply re-runs. A failure is surfaced per-destination by
 * the caller, so it is visible, not silent.
 *
 * **New uploads never write `.solos-parts` again.** Reading it back is kept as
...
class RcloneChunkedUpload(private val context: Context) {
    ...
    fun upload(
        ...
        // mistaken for a backup: the restore list matches `.solosbak` exactly.
     ...
     * are hidden from the restore list (the filter requires a `.solosbak`
     ...
     * `.partial` scratch objects, never a user's `.solosbak` and never the
     * historical `.solos-parts` directory.
     */
     ...
     *
     * A `.solos-parts/<id>/` upload is a DIRECTORY, and `deletefile` on it
    ...
    data class RemotePackage(
        ...
        val modified: Long?,
        /**
         * >1 only for a `.solos-parts` upload written by an older build.
     ...
     *
     * Dotfiles are skipped — that hides `.solos-parts` scratch and the
    ...
    /**
     * Everything restorable in [remote]: whole `.solosbak` packages, plus any
     * chunked upload a PREVIOUS build left behind.
     *
     * In-flight scratch files never show up here: they are named
     * `<package>.solosbak.partial`, and the filter below requires the name to
     * END in `.solosbak`.
     */
    fun listPackages(remote: RcloneRemoteStore.Remote): List<RemotePackage> {
        ...
        // BACKWARD COMPATIBILITY ONLY. Nothing writes `.solos-parts` any more,
    ...
    fun download(
        ...
        val scratch = File(context.cacheDir, "solos-dl-${pkg.displayName}").apply { mkdirs() }
        try {
    ...
    companion object {
         ...
         */
        const val PARTS_DIR = ".solos-parts"
