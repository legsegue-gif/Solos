...
import com.solos.app.accessibility.SolosAccessibilityService
import com.solos.app.accessibility.NodeRegistry
import com.solos.app.accessibility.RestrictedSettingsManager
 ...
 * [SolosAccessibilityService]. Mirrors the design doc
...
First-run: enable "Solos" under Settings → Accessibility, then `service ping`.
"""
...
"""
    ...
    override fun handle(request: NativeOffloadRequest): NativeOffloadResult {
                ...
                return err(
                    ...
                    if (restricted) {
                        "Android is blocking the accessibility toggle for this install " +
                            ...
                            "settings, then enable Solos under Settings → Accessibility. " +
                        ...
                        "Accessibility permission was revoked (this happens after a force-stop). " +
                            "Re-enable Solos under Settings → Accessibility, or use Settings → " +
        ...
        return try {
            ...
            err(args, "SERVICE_NOT_RUNNING",
                e.message ?: "Accessibility service is not running. Enable Solos under Settings → Accessibility.",
                exit = 77)
        } catch (e: Throwable) {
    ...
    private fun serviceSub(args: OffloadArgs): NativeOffloadResult {
        return when (args.positional.getOrNull(1)) {
            ...
            "status" -> {
                val svc = SolosAccessibilityService.getInstance()
                val data = JSONObject()
                    .put("running", svc != null)
                    .put("serviceName", SolosAccessibilityService.SERVICE_ID)
            ...
            "ping" -> {
                if (SolosAccessibilityService.getInstance() != null)
                    ...
                    NativeOffloadResult(77, "✗ Accessibility service is not running — go to Settings → Accessibility → Solos to enable\n")
            }
    ...
    private fun tapXYRaw(svc: SolosAccessibilityService, x: Int, y: Int, args: OffloadArgs): NativeOffloadResult {
    ...
    private fun resolveTargetEditable(svc: SolosAccessibilityService, args: OffloadArgs): AccessibilityNodeInfo? {
    ...
    private fun treeSignature(svc: SolosAccessibilityService): Int {
        var h = 0
        for (root in svc.rootNodes()) {
    ...
    private fun eventWatch(args: OffloadArgs, once: Boolean): NativeOffloadResult {
        ...
        val collected = java.util.concurrent.ConcurrentLinkedQueue<SolosAccessibilityService.RecordedEvent>()
        val listener: (SolosAccessibilityService.RecordedEvent) -> Unit = { collected.offer(it) }
    ...
    private fun svcOrThrow(): SolosAccessibilityService =
        SolosAccessibilityService.getInstance()
            ?: throw NotRunning("Accessibility service is not running. Enable Solos under Settings → Accessibility.")
