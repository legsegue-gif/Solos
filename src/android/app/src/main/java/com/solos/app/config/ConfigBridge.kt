...
import com.solos.app.config.audit.ConfigAuditActor
import com.solos.app.config.audit.ConfigAuditEntry
import com.solos.app.config.audit.ConfigAuditLog
import com.solos.app.config.audit.ConfigAuditStatus
import com.solos.app.config.confirm.ConfigConfirmationGate
import com.solos.app.config.confirm.ConfirmOutcome
import com.solos.app.config.confirm.PendingConfigChange
import com.solos.app.config.confirm.PendingConfigChangeItem
 ...
 * in [com.solos.app.sandbox.NativeOffloadResult]. The handler
...
object ConfigBridge {
    ...
    fun isEnabled(): Boolean = SolosConfigPermissionStore.isEnabled
    ...
    /** Standardised "solos-config disabled" envelope. Includes user_message. */
    fun disabledErrorEnvelope(): JSONObject = JSONObject().apply {
        put("ok", false)
        put("error", "permission_denied")
        put("reason", "solos-config is disabled in Settings → Permissions.")
        put(
            "user_message",
            "I tried to change a setting but solos-config is currently disabled. " +
                "You can enable it at [Settings → Permissions](solos://settings/permissions), " +
    ...
    private fun readFieldRaw(path: String): JSONObject {
        if (!SolosConfigPermissionStore.isEnabled) return disabledErrorEnvelope()
        ...
        if (field.access == ConfigAccess.HIDDEN) return JSONObject().apply {
            put("ok", false)
            put("error", "permission_denied")
            put("reason", "'$path' is intentionally not exposed to solos-config.")
        }
        ...
        return try {
            ...
            // [T-solos-config-provider-add] Forward PermissionDenied verbatim
    ...
    private fun paginationHint(
        ...
        if (page > totalPages) {
            ...
            return "Page $page is out of range (only $totalPages $pgWord available). " +
                "Try: solos-config get $path$filterFragment --page $totalPages"
        }
        if (totalPages <= 1) {
        ...
        if (page < totalPages) {
            return "Showing page $page of $totalPages ($pageCount of $total items). " +
                "To get more, use: solos-config get $path$filterFragment --page ${page + 1} --page-size $pageSize"
        }
    ...
    suspend fun performWriteBatch(
        ...
        for (i in 0 until items.length()) {
            ...
            if ((isAppend || isRemove) && ConfigRegistry.get().collection(resolvePath) != null) {
                ...
                // [T-solos-config-provider-add] Redact credential values
            ...
            if (field.access != ConfigAccess.READWRITE) {
                val reason = if (field.access == ConfigAccess.HIDDEN) {
                    "'$rawPath' is intentionally not exposed to solos-config."
                } else {
            ...
            // [T-solos-config-provider-add] Mask credential writes the
            ...
            is ConfirmOutcome.Approved -> {
                ...
                for ((idx, row) in outcome.items.withIndex()) {
                    ...
                    try {
                        ...
                        if (r.collection != null && r.collectionVerb != null) {
                            ...
                            // [T-solos-config-provider-add] If this is a
                ...
                if (applied.length() == 0 && earlyError != null) {
                    ...
                    JSONObject().apply {
                        ...
                        put("audit_url", "solos://settings/logs?tab=config-audit")
                        put("user_message", "Settings updated. Review or revert at [Logs → Config Changes](solos://settings/logs?tab=config-audit).")
                    }
                }
            }
        }
    ...
    private data class Resolved(
        ...
        val newValue: ConfigValue,
        // [T-solos-config-provider-add] When non-null, audit / display
    ...
    fun auditRevert(
        ...
        val res = runBlocking {
            performWriteBatch(
                items = items,
                caption = "solos-config audit revert ${entry.id.take(8)}",
                actorRaw = actorRaw,
                sessionId = sessionId,
