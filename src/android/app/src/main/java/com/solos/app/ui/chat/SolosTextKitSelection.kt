...
/**
 * SolosTextKit — a self-contained text selection layer that survives
 ...
 *
 * SolosTextKit avoids that by keeping the *authoritative selection state*
 ...
 * [SolosMarkdownView] and consumed by [StreamingMarkdownText] via
 * [LocalSolosSelectionController].
 */
...
class SelectionController {
     ...
     * (e.g. a user message bubble, which intentionally skips SolosTextKit
    ...
    private fun spliceNonShardSpan(
        ...
        // composables (no SolosTextKit shard), so the per-shard walk drops them
     ...
     * markdown slice contain a block that doesn't register as a SolosTextKit
 ...
 */
val LocalSolosSelectionController = compositionLocalOf<SelectionController?> { null }
...
fun ProvideSelectionController(
    ...
    CompositionLocalProvider(LocalSolosSelectionController provides controller) {
        content()
    }
}
...
fun RegisterSelectionShard(shard: TextShard?) {
    val controller = LocalSolosSelectionController.current ?: return
