package com.solos.app.scheduled
...
import com.solos.app.debug.HeadlessChatRunner
import com.solos.app.logging.AppLogger
import com.solos.app.service.AgentForegroundService
...
object ScheduledAgentRunner {
     ...
     *   e.g. the solos-scheduled CLI, which runs in its own offload thread.
     ...
     *   target chat gone, SolosApp not initialized).
     */
    suspend fun run(
        context: Context,
        task: ScheduledTask,
        ...
        // [T-android-scheduled-lateinit-crash-156] `as? SolosApp` only rules out
        ...
        val app = context.applicationContext as? SolosApp ?: run {
            AppLogger.error(TAG, "Application is not SolosApp — skipping task ${task.id}")
            return null
        }
        if (!app.subsystemsReady()) {
            AppLogger.error(
                TAG,
                "SolosApp subsystems not initialized (safe-mode or failed init) — skipping task ${task.id}",
            )
            return null
        }
    ...
    private suspend fun resolveSessionId(app: SolosApp, task: ScheduledTask): String? {
        return when (val mode = task.targetMode) {
            ...
            ScheduledTargetMode.NewSession -> {
                ...
                val memoryOn = com.solos.app.data.MemoryGlobalPrefs.isGlobalEnabled(app)
    ...
    private fun postCompletionNotification(
        ...
        val deepLink = Uri.parse("solos://session/$sessionId")
        ...
        val title = "Solos: ${task.label.ifBlank { "Scheduled task" }}"
