 ...
 *     also accepts a Linux file path under /var/solos/... resolved via
...
class SpeechOffloadHandler(private val context: Context) : NativeOffloadHandler {
    ...
    private fun cmdTranscribe(args: OffloadArgs): NativeOffloadResult {
        ...
        // --source: default mic, accept "mic" / "system-mic" / any /var/solos/... path.
    ...
    /**
     * Resolve a `/var/solos/...`-style Linux path to a host File via
            ...
            if (r == OffloadPermissionManager.AndroidPermissionResult.DENIED) {
                r = OffloadPermissionManager.requestSettingsGate(
                    OffloadPermissionManager.SettingsGateRequest(
                        ...
                        message = "Solos needs microphone permission to transcribe speech. Open Settings to allow it.",
...
EXAMPLES:
  ...
  android-speech transcribe --source /var/solos/attachments/meeting.m4a
  android-speech languages
