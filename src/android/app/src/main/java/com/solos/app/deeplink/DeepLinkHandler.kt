package com.solos.app.deeplink

import android.net.Uri
import com.solos.app.ui.navigation.Routes

/**
 * Parses solos:// deep link URIs into navigation actions.
 *
 * Supported routes (matching iOS):
 *   solos://share                              → open share flow
 *   solos://views/alarm                        → open alarm list
 *   solos://open_terminal?init_command=...      → open terminal with command
 *   solos://session/<id>                        → open specific session
 *   solos://settings                            → Settings home
 *   solos://settings/providers                  → Provider list
 *   solos://settings/providers/<instanceId>     → Provider detail
 *   solos://settings/model-groups               → Model Groups (incl. Agent Loop section)
 *   solos://settings/model-groups/<groupId>     → Model Group detail
 *   solos://settings/usage                      → Token usage
 *   solos://settings/skills                     → Skills management
 *   solos://settings/memory                     → Memory management
 *   solos://settings/storage                    → Storage management
 *   solos://settings/mount-external             → Mount External Folders list
 *   solos://settings/mounts                     → alias for mount-external
 *   solos://settings/shared-folders             → Shared Folders list (T235)
 *   solos://settings/shared_folders             → alias for shared-folders
 *   solos://settings/logs                       → Log management
 *   solos://settings/appearance                 → Appearance
 *   solos://settings/background                 → Background settings
 *   solos://settings/about                      → About
 *   solos://settings/permissions                → Permissions
 *   solos://settings/environments[?create_key=...&create_value=...&create_note=...]
 *                                               → Environment variables
 *   solos://settings/rootfs                     → Rootfs management (mirror config lives here)
 *   solos://settings/mirrors                    → alias for rootfs (mirrors live inside Rootfs UI)
 *
 ...
 *
 * Resource-class URIs (`solos://workspace/...`, `solos://skills/...`,
...
sealed class DeepLinkAction {
     ...
     *
     * Encoded as `solos://action/<name>` so the static shortcuts XML can
     * point to them via plain Intent.data without any custom extras —
     * matches the existing solos:// deep-link conventions.
     */
     ...
     * [com.solos.app.ui.navigation.Routes] does the rest. Routes
     ...
     * shortcut.
     *
     * Encoded as `solos://session/<sessionId>/<resource-path>` —
     * [sessionId] selects which chat to land in; [resourcePath] is the
     * resource path under `/var/solos/` (e.g. `/browser/snake.html`).
...
object DeepLinkHandler {
    fun parse(uri: Uri?): DeepLinkAction {
        if (uri == null || uri.scheme != "solos") return DeepLinkAction.Unknown
        ...
        return when (host) {
            ...
            "session" -> {
                // solos://session/<sessionId>                → OpenSession
                // solos://session/<sessionId>/<resource-path> → OpenHtmlPreview
    ...
    /**
     * T183: walk a `solos://settings/<path>` URI to the right NavHost
    ...
    private fun parseSettingsPath(uri: Uri): DeepLinkAction {
        ...
        return when (head) {
            ...
            "environments" -> {
                ...
                //   solos://settings/environments?create_key=GH_TOKEN&create_value=
