...
object MountedFolderCoordinator {
    ...
    private const val MOUNTS_PREFIX = "/var/solos/mounts/"

    /**
     * Throws [ReadOnlyMountException] when [linuxPath] is inside an
     * effectively read-only mount. Paths outside `/var/solos/mounts/` and
    ...
    /**
     * True when [linuxPath] is `/var/solos/mounts/<name>/...` AND the
    ...
    fun isLinuxPathUnderReadOnlyMount(linuxPath: String, store: MountedFoldersStore): Boolean {
        ...
        // `/var/solos/mounts/foobar` as inside a mount named `foo`.
