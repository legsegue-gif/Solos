...
class SessionForkManager(
     ...
     * `<filesDir>/solos-global/memory/<fileName>`. Mirrors iOS
     * `SessionForkManager.copyRemoteMemory` which writes under
     * `solosMemoryPersistentDir`. Overwrites if the file already exists.
     */
    fun copyMemory(fileName: String, content: String): Boolean {
        ...
        val dir = File(filesDir, "solos-global/memory").apply { mkdirs() }
        return try {
