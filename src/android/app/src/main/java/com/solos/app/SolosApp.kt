...
import com.solos.app.data.db.AppDatabase
import com.solos.app.data.db.DatabaseVersionGuard
import com.solos.app.data.repository.BackgroundSettingsRepository
import com.solos.app.data.repository.ChatRepository
import com.solos.app.data.repository.EnvVarRepository
import com.solos.app.data.MountedFoldersStore
import com.solos.app.data.repository.MemoryRepository
import com.solos.app.data.repository.ProviderRepository
import com.solos.app.data.repository.WebAppShortcutRepository
import com.solos.app.data.repository.MCPRepository
import com.solos.app.data.repository.SkillRepository
import com.solos.app.notification.BackgroundTaskNotifier
import com.solos.app.logging.AppLogger
import com.solos.app.network.NetworkMonitor
import com.solos.app.offload.OffloadPermissionManager
import com.solos.app.provider.ModelsDevApi
import com.solos.app.sandbox.ExecutionCoordinator
import com.solos.app.sandbox.MountedFolderCoordinator
import com.solos.app.sandbox.NativeOffloadServer
import com.solos.app.sandbox.PRootKernel
import com.solos.app.sandbox.RootfsManager
import com.solos.app.sandbox.offload.AccessibilityOffloadHandler
import com.solos.app.sandbox.offload.AlarmOffloadHandler
import com.solos.app.sandbox.offload.BrowserUseOffloadHandler
import com.solos.app.sandbox.offload.CalendarOffloadHandler
import com.solos.app.sandbox.offload.ClipboardOffloadHandler
import com.solos.app.sandbox.offload.ContactsOffloadHandler
import com.solos.app.sandbox.offload.DeviceOffloadHandler
import com.solos.app.sandbox.offload.LocationOffloadHandler
import com.solos.app.sandbox.offload.ModelUseOffloadHandler
import com.solos.app.sandbox.offload.SessionsOffloadHandler
import com.solos.app.sandbox.offload.ShizukuOffloadHandler
import com.solos.app.sandbox.offload.NotificationOffloadHandler
import com.solos.app.sandbox.offload.OpenOffloadHandler
import com.solos.app.sandbox.offload.PhotosOffloadHandler
import com.solos.app.sandbox.offload.PlayerOffloadHandler
import com.solos.app.sandbox.offload.SpeakOffloadHandler
import com.solos.app.sandbox.offload.SpeechOffloadHandler
import com.solos.app.sandbox.offload.WeatherOffloadHandler
import com.solos.app.service.SessionActivityTracker
import com.solos.app.ui.SolosImageFetcher
import kotlinx.coroutines.launch

class SolosApp : Application(), ImageLoaderFactory {
    /**
    ...
    /**
     * Application-scoped BrowserTabPool for shell-invoked `solos-browser-use`.
    ...
    val sharedBrowserTabPool: BrowserTabPool by lazy {
        BrowserTabPool(this).also { it.setSession("solos-browser-use") }
    }
    ...
    override fun onCreate() {
        ...
        if (ACRA.isACRASenderServiceProcess()) {
            Log.i("SolosApp", "skipping app init in :acra reporter process")
            return
        }
        ...
        com.solos.app.data.FastModePrefs.prime(this)
        ...
        // Activity context.
        com.solos.app.data.AutoCompactPrefs.prime(this)
        ...
        // libs first execute.
        try {
            com.solos.app.crash.NativeCrashHandler.install(
                java.io.File(filesDir, "logs"),
            )
        } catch (t: Throwable) {
            Log.w("SolosApp", "NativeCrashHandler install failed: ${t.message}")
        }
        ...
        try {
            ...
            Thread.setDefaultUncaughtExceptionHandler { thread, throwable ->
                try {
                    ...
                    if (isFgsTimeout) {
                        ...
                        runCatching {
                            val intent = Intent(this, com.solos.app.service.AgentForegroundService::class.java)
                            stopService(intent)
                        }
                    }
                } catch (t: Throwable) {
                    Log.w("SolosApp", "FGS-timeout handler internal failure: ${t.message}")
                }
            ...
            Log.w("SolosApp", "install FGS-timeout handler failed: ${t.message}")
        }
        ...
        com.solos.app.crash.CrashFrequencyDetector.checkAtLaunch(this)
        ...
        if (com.solos.app.crash.CrashFrequencyDetector.isSafeMode()) {
            Log.w("SolosApp", "safe-mode ON — skipping app subsystem init")
            return
        }
        ...
        // `LoggingManager.startIfEnabled()` (called from SolosApp.swift:143).
        AppLogger.init(this)
        ...
        // [com.solos.app.diagnostics.LaunchCycleBeacon].
        try {
            com.solos.app.diagnostics.LaunchCycleBeacon.recordLaunch(this)
        } catch (t: Throwable) {
            Log.w("SolosApp", "LaunchCycleBeacon.recordLaunch failed: ${t.message}")
        }
        ...
        com.solos.app.diagnostics.HangDetector.start(this)
        ...
        memoryRepository = MemoryRepository(java.io.File(filesDir, "solos-global/memory"))
        ...
        subsystemsInitialized = true
            ...
            Log.e("SolosApp", "subsystem init failed — app will start in degraded mode", t)
            return
        }
        ...
        com.solos.app.agent.SoulStore.ensureExists(this)
        com.solos.app.agent.SoulStore.refreshCache(this)

        // T-config: solos-config CLI surface — registry / audit log /
        ...
        com.solos.app.config.SolosConfigPermissionStore.init(this)
        com.solos.app.config.audit.ConfigAuditLog.init(this)
        com.solos.app.config.ConfigRegistry.init(
        ...
        com.solos.app.data.EnvVarPrivacyStore.init(this)
        com.solos.app.data.EnvVarRedactor.envVarRepository = envVarRepository
        ...
        // Register global /var/solos/{memory,skills,shared} bind mounts up-front
        ...
        // `shell_execute` already has `/var/solos/mounts/<name>/` visible.
        ...
        // /var/solos/mounts/<name> placeholder dirs that PRoot's `-b` needs.
        ...
        // T323: UI-layer automation backed by SolosAccessibilityService.
        NativeOffloadServer.register("android-a11y-cli", AccessibilityOffloadHandler(this))
        NativeOffloadServer.register("solos-model-use", ModelUseOffloadHandler(this, providerRepository))
        // T-config: solos-config — agent-facing settings management
        ...
        NativeOffloadServer.register(
            "solos-config",
            com.solos.app.sandbox.offload.ConfigOffloadHandler(),
        )
        NativeOffloadServer.register("solos-browser-use", BrowserUseOffloadHandler(this))
        // T188: solos-sessions-cli — agent-side query of chat history.
        // Registers next to the other solos-* tools so PRootKernel.
        // installHandlerStubs() picks it up on the next rootfs boot
        // (writes a 17-byte exit-0 stub at /usr/local/bin/solos-sessions-cli
        // so PATH lookup succeeds; PRoot intercepts the execve before
        // the stub runs and routes to this handler).
        NativeOffloadServer.register("solos-sessions-cli", SessionsOffloadHandler(chatRepository))
        // [T-android-scheduled-tasks-full] solos-scheduled — create/list/run
        // timed AI tasks (new chat / follow-up / re-run), mirroring the in-app
        // Scheduled Tasks editor and the iOS Shortcuts intent set.
        NativeOffloadServer.register(
            "solos-scheduled",
            com.solos.app.sandbox.offload.ScheduledTaskOffloadHandler(this),
        )
        ...
        com.solos.app.offload.ShizukuManager.init(this)

        // T-android-solos-debug-cli: shell-side CLI wrapper around the in-app
        // DebugServer (127.0.0.1:5321) JSON-RPC. DEBUG-only — Release builds
        // ship neither the DebugServer nor this handler, so the
        // `/usr/local/bin/solos-debug` stub is also absent (PRootKernel.
        // installHandlerStubs enumerates currently-registered handlers).
        if (BuildConfig.DEBUG) {
            NativeOffloadServer.register(
                "solos-debug",
                com.solos.app.sandbox.offload.DebugOffloadHandler(this),
            )
        }
        ...
        com.solos.app.service.SessionBadgeStore.init(this)
        ...
        kotlinx.coroutines.CoroutineScope(kotlinx.coroutines.Dispatchers.IO).launch {
            ...
            com.solos.app.service.SessionBadgeStore.reconcileInterruptedSessions(interrupted - active)
        }
        ...
        // notifier into the (Context-free) gate, so a solos-config approval that
        ...
        val configConfirmNotifier = com.solos.app.notification.ConfigConfirmNotifier(
            context = this,
            backgroundSettings = backgroundSettingsRepository,
            isAppForeground = ::isAppForeground,
        )
        com.solos.app.config.confirm.ConfigConfirmationGate.backgroundNotifier = {
            configConfirmNotifier.notifyIfBackgrounded(it)
        }
        com.solos.app.config.confirm.ConfigConfirmationGate.cancelNotification = {
        ...
        registerActivityLifecycleCallbacks(object : ActivityLifecycleCallbacks {
            ...
            override fun onActivityResumed(activity: Activity) {
                ...
                com.solos.app.crash.CrashFrequencyDetector.maybeShowOnActivity(activity)
            ...
            override fun onActivityStopped(activity: Activity) {
                ...
                if (foregroundActivityCount == 0) {
                    ...
                    com.solos.app.config.confirm.ConfigConfirmationGate.notifyPending()
                }
            }
        ...
        com.solos.app.speech.SpeechRecognitionManager.init(this)

        // Refresh model lists once per calendar day (mirrors iOS SolosApp.swift).
                ...
                when (intent.action) {
                    Intent.ACTION_TIMEZONE_CHANGED -> scope.launch {
                        try {
                            ...
                            Log.w("SolosApp", "broadcastTimezoneChange failed: ${t.message}")
                        }
                    }
                    android.net.Proxy.PROXY_CHANGE_ACTION -> scope.launch {
                        try {
                            ...
                            Log.w("SolosApp", "broadcastProxyChange failed: ${t.message}")
                        }
                    }
                }
            }
        ...
        if (BuildConfig.DEBUG) {
            try {
                com.solos.app.debug.DebugServer(this).start()
            } catch (e: Exception) {
                Log.w("SolosApp", "Failed to start debug server: ${e.message}")
            }
        }

        // T268: one-shot migration of pre-T266 internal alarms into the
        // system Clock app. Pre-T266 builds wrote alarms into Solos's own
        // SharedPreferences + AlarmManager; T266 retired that path but old
        // installs still have ghost entries that fire only inside Solos.
        ...
        runCatching { migrateGhostAlarms() }
            .onFailure { Log.w("SolosApp", "ghost alarm migration failed: ${it.message}") }
    }

    /**
     * T268: replay any pre-T266 internal alarm/timer entries from
     * solos_alarms_prefs through SET_ALARM / SET_TIMER, then clear the
    ...
    private fun migrateGhostAlarms() {
        val prefs = getSharedPreferences("solos_alarms_prefs", Context.MODE_PRIVATE)
        ...
        prefs.edit().remove("alarms_json").apply()
        Log.i("SolosApp", "T268 ghost alarm migration: migrated=$migrated skipped=$skipped (prefs cleared)")
    }

    /**
     * Coil global ImageLoader — registers [SolosImageFetcher] so `solos://`
     * URIs in Markdown images (e.g. `![alt](solos://attachments/x.png)`)
     * resolve to local files under /var/solos/.
     */
    override fun newImageLoader(): ImageLoader =
        ImageLoader.Builder(this)
            .components {
                add(SolosImageFetcher.Factory())
                add(SolosImageFetcher.UriFactory())
                // T-image-cache-mtime-35133: include File.lastModified() in
                // memory + disk cache key so Grok-style in-place rewrites of
                // solos://attachments/foo.jpg invalidate Coil's cached bitmap.
                add(SolosImageFetcher.MtimeKeyer())
                add(SolosImageFetcher.StringMtimeKeyer())
            }
            .build()
    ...
    override fun onTrimMemory(level: Int) {
        ...
        Log.i("SolosApp", "onTrimMemory(level=$level): releasing formula bitmap caches")
        runCatching { com.solos.app.ui.chat.KatexWebViewPool.evictAll() }
            .onFailure { Log.w("SolosApp", "KatexWebViewPool.evictAll failed: ${it.message}") }
        runCatching { com.solos.app.ui.markdown.KaTeXRendererCache.evictAll() }
            .onFailure { Log.w("SolosApp", "KaTeXRendererCache.evictAll failed: ${it.message}") }

        if (level >= TRIM_MEMORY_COMPLETE) {
            Log.i("SolosApp", "onTrimMemory(level=$level): tearing down the offscreen KaTeX WebView")
            runCatching { com.solos.app.ui.chat.KatexWebViewPool.releaseWebView() }
                .onFailure { Log.w("SolosApp", "KatexWebViewPool.releaseWebView failed: ${it.message}") }
        }
    }
    ...
    override fun onTerminate() {
        ...
        try {
            com.solos.app.diagnostics.LaunchCycleBeacon.recordCleanExit(this)
        } catch (t: Throwable) {
            Log.w("SolosApp", "LaunchCycleBeacon.recordCleanExit failed: ${t.message}")
        }
        super.onTerminate()
    }
