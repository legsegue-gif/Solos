package com.solos.app.provider
...
import com.solos.app.provider.anthropic.AnthropicProvider
import com.solos.app.provider.gemini.GeminiProvider
import com.solos.app.provider.openai.OpenAIProvider
        ...
        val provider: LLMProvider = when (instance.providerType) {
            ...
            ProviderType.openAI, ProviderType.openAIResponses -> {
                ...
                val manualBearer = if (context != null &&
                    ...
                    com.solos.app.auth.OAuthManager.forInstance(context, instance)?.loadManualBearerToken()
                } else null
                ...
                if (instance.credentialType == ProviderCredential.oauth && manualBearer.isNullOrEmpty()
                    ...
                    OpenAIProvider(
                        oauthTokenProvider = { oauthManager.validAccessToken() ?: throw com.solos.app.data.model.LLMError.InvalidApiKey() },
                        model = model,
            ...
            ProviderType.openRouter -> {
                ...
                OpenAIProvider(
                    ...
                    extraHeaders = mapOf(
                        "HTTP-Referer" to "https://github.com/legsegue-gif/Solos",
                        "X-Title" to "Solos App",
                    ),
                )
            }
            ProviderType.xAI -> {
                ...
                if (instance.credentialType == ProviderCredential.oauth && manualBearer.isNullOrEmpty()
                    && context != null) {
                    val oauthManager = com.solos.app.auth.XAIOAuthManager(context, instance.id)
                    OpenAIProvider.oauthOpenAICompat(
                        oauthTokenProvider = {
                            oauthManager.validAccessToken()
                                ?: throw com.solos.app.data.model.LLMError.InvalidApiKey()
                        },
                        model = model,
                        basePath = base,
            ...
            ProviderType.kimiCode -> {
                ...
                val base = basePath ?: "${com.solos.app.auth.KimiDeviceFlow.CODING_API_BASE}/v1"
                val manualBearer = if (context != null &&
                    instance.credentialType == ProviderCredential.oauth) {
                    com.solos.app.auth.OAuthManager.forInstance(context, instance)?.loadManualBearerToken()
                } else null
                if (instance.credentialType == ProviderCredential.oauth && manualBearer.isNullOrEmpty()
                    && context != null) {
                    val oauthManager = com.solos.app.auth.KimiOAuthManager(context, instance.id)
