...
/**
 * Builds a `.solosbak` package on Android (docs/backup-restore-design.md §2,
...
class BackupExporter(
                ...
                BackupJsonlWriter(dataDir, "compact_markers").use { markers ->
                    BackupJsonlWriter(dataDir, "folders").use { folders ->
                        for (session in dao.listSessions()) {
                            ...
                            val dir = File(context.filesDir, "solos-sessions/${session.id}")
                            val r = trees.export(
    ...
    /**
     * §3.2 — the cross-session `/var/solos/shared` bucket. Host-side this is
     * `<filesDir>/solos-global/shared`, NOT anything inside the rootfs.
     */
    private fun exportSharedFiles(trees: BackupFileTreeExporter): BackupManifest.CategoryStat {
        val r = trees.export(
            File(context.filesDir, "solos-global/shared"), "shared", BackupCategory.SHARED_FILES
        )
    ...
    private fun exportSkills(trees: BackupFileTreeExporter): BackupManifest.CategoryStat {
        val root = File(context.filesDir, "solos-global/skills")
    ...
    private fun exportMemory(dataDir: File): BackupManifest.CategoryStat {
        val source = File(context.filesDir, "solos-global/memory")
    ...
    private fun exportMcpServers(dataDir: File): BackupManifest.CategoryStat? {
        val source = File(context.filesDir, "solos-global/mcp-servers/servers.json")
    ...
    private val app: com.solos.app.SolosApp?
        get() = context.applicationContext as? com.solos.app.SolosApp
    ...
    private suspend fun exportProviders(
        ...
        val config = repo?.config?.value ?: com.solos.app.data.model.ProviderConfig()
        val url = File(dataDir, "provider_config.json")
        url.writeText(
            BackupFormat.json.encodeToString(
                com.solos.app.data.model.ProviderConfig.serializer(), config
            )
        )
    ...
    private suspend fun exportThinkingRules(dataDir: File): Int {
        val dao = com.solos.app.data.db.ProviderDatabase
    ...
    private fun archive(staging: File, backupId: String): File {
        ...
        // perfectly valid `.solosbak` name, which then shows up in the restore
    ...
    companion object {
         ...
         * directories, NOT inside `solos-global/shared` — that path is
         * bind-mounted into the guest at `/var/solos/shared`, so a package
