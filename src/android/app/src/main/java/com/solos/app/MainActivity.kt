...
import com.solos.app.accessibility.AccessibilityRecoveryManager
...
import com.solos.app.deeplink.DeepLinkHandler
import com.solos.app.logging.AppLogger
import com.solos.app.service.SessionActivityTracker
import com.solos.app.ui.navigation.AppNavigation
import com.solos.app.ui.navigation.Routes
import com.solos.app.ui.navigation.safeNavigate
import com.solos.app.ui.NewerDatabaseGuidanceScreen
import com.solos.app.ui.settings.KEY_FONT_APP_BASE
import com.solos.app.ui.settings.KEY_KEEP_SCREEN_AWAKE
import com.solos.app.ui.settings.KEY_LANGUAGE
import com.solos.app.ui.settings.KEY_LAUNCH_SESSION
import com.solos.app.ui.settings.KEY_THEME_MODE
import com.solos.app.ui.settings.PREF_APPEARANCE
import com.solos.app.ui.settings.getAppearancePrefs
import com.solos.app.ui.settings.fontScaleForLevel
import com.solos.app.ui.settings.keepScreenAwakeEnabled
import com.solos.app.ui.theme.SolosTheme

private const val KEY_CURRENT_CHAT_SESSION_ID = "solos.current_chat_session_id"
...
class MainActivity : ComponentActivity() {
     ...
     * [com.solos.app.ui.navigation.AppNavigation]'s LaunchedEffect that
    ...
    override fun attachBaseContext(newBase: android.content.Context) {
        super.attachBaseContext(com.solos.app.i18n.LocaleWrap.wrap(newBase))
    }

    /**
     * T-android-safemode-lateinit-crash: escape hatch for a process whose
     * [SolosApp.onCreate] early-returned under safe-mode.
     *
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // SolosApp.onCreate (≥THRESHOLD recent crash files), the
        // Application skipped all heavy init — no DB, no repos, no
        // offload server. We must NOT call setContent() / ChatViewModel /
        // any code that touches SolosApp's lateinit deps; doing so would
        ...
        // AlertDialog (system-level) which doesn't touch SolosApp state.
        ...
        // dismisses the share dialog, but SolosApp.onCreate already
        ...
        // genuinely unsafe.
        val solosApp = application as? SolosApp
        ...
        // out.
        if (solosApp != null &&
            solosApp.dbVersionDecision ==
            com.solos.app.data.db.DatabaseVersionGuard.Decision.SHOW_NEWER_DB_GUIDANCE
        ) {
        ...
        if (solosApp == null || !solosApp.subsystemsInitialized) {
            android.util.Log.w(
                "MainActivity",
                "app subsystems not initialized (safeMode=" +
                    "${com.solos.app.crash.CrashFrequencyDetector.isSafeMode()}) — " +
                    "showing crash share dialog and finishing",
            )
            com.solos.app.crash.CrashFrequencyDetector.maybeShowOnActivity(
                activity = this,
                ...
                // tapped, reading as "Solos won't open at all". The process
        ...
        restoredChatSessionId = savedInstanceState?.getString(KEY_CURRENT_CHAT_SESSION_ID)
            ?.takeUnless {
                com.solos.app.crash.CrashFrequencyDetector.shouldForceHomeOnLaunch(this)
            }
        ...
        // (SolosApp.swift:279) rather than keying off a launch parameter.
        ...
        com.solos.app.share.ShareCoordinator.processPendingShare(this)
        ...
        if (BuildConfig.DEBUG) {
            com.solos.app.debug.DebugRPCHandler.currentActivity = java.lang.ref.WeakReference(this)
        }

        // Non-null and fully initialized — proven by the guard above.
        val app = requireNotNull(application as? SolosApp)
        ...
        setContent {
            ...
            SolosTheme(darkTheme = darkTheme, fontScale = fontScale) {
                ...
                // T-config: root-level solos-config confirm dialog.
                ...
                // agent triggers a change. Mirrors iOS SolosApp.swift
                // root-level `.sheet(item: gate.pending)`.
                com.solos.app.ui.settings.ConfigConfirmDialogHost()
            }
        }
    }
     ...
     * is already covered by [com.solos.app.ui.navigation.AppNavigation]
    ...
    private fun handleDeepLink(uri: Uri?) {
        ...
        when (action) {
            ...
            is DeepLinkAction.OpenAlarmList -> {
                // T297: solos://views/alarm now opens the system Clock app
