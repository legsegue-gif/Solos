...
object AppLogger {
     ...
     *
     * [init] is called from SolosApp.onCreate AFTER the safe-mode
    ...
    private fun startCapture() {
        ...
        // post-crash one.
        appContext?.let { com.solos.app.diagnostics.EnvironmentBanner.log(it) }
    }
     ...
     * itself produced (tag prefix `Solos.`) are skipped — [log] already wrote
    ...
    private fun writeLogcatLine(rawLine: String) {
        ...
        if (slashIdx >= 0 && parenIdx > slashIdx) {
            ...
            if (tag.startsWith("Solos.") || tag == "AppLogger") return
        }
        try {
            val now = Date()
    ...
    private fun getWriter(date: String): PrintWriter {
        if (date != currentDate || writer == null) {
            ...
            val file = File(dir, "solos-$date.log")
     ...
     *
     * - `prefix`: filename starts-with filter (e.g. `"solos-"` for daily
    ...
    fun clearLogs() {
        ...
        // getWriter() reopens a fresh solos-<date>.log on the following write.
