...
/**
 * On-the-wire types for the `.solosbak` backup package.
 *
...
object BackupFormat {
    ...
    const val CURRENT = "solosbak/1"

    /** File extension registered to the app for "open to import". */
    const val FILE_EXTENSION = "solosbak"

    /**
     * MIME type for a `.solosbak` package.
     *
     * Matches the `application/x-solosbak` iOS declares for the
     * `com.solos.app.solosbak` UTI (src/ios/Info.plist), so both platforms
     ...
     */
    const val MIME_TYPE = "application/x-solosbak"
