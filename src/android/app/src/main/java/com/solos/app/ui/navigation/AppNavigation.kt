package com.solos.app.ui.navigation
...
import com.solos.app.deeplink.DeepLinkAction
import com.solos.app.deeplink.DeepLinkCoordinator
import com.solos.app.ui.settings.KEY_LAUNCH_SESSION
import com.solos.app.ui.settings.getAppearancePrefs
...
import com.solos.app.ui.chat.ChatScreen
import com.solos.app.ui.sessions.SessionListScreen
import com.solos.app.ui.settings.AboutScreen
import com.solos.app.ui.settings.AddAgentLoopGroupsScreen
import com.solos.app.ui.settings.AddAgentLoopModelsScreen
import com.solos.app.ui.settings.AddCustomModelScreen
import com.solos.app.ui.settings.BackgroundSettingsScreen
import com.solos.app.ui.settings.AddModelsToGroupScreen
import com.solos.app.ui.settings.ShadowVoiceDetailScreen
import com.solos.app.ui.settings.AddProviderScreen
import com.solos.app.ui.settings.ModelEntryDetailScreen
import com.solos.app.ui.settings.ModelGroupDetailScreen
import com.solos.app.ui.settings.ModelGroupsScreen
import com.solos.app.ui.settings.ProviderDetailScreen
import com.solos.app.ui.settings.ProviderListScreen
import com.solos.app.ui.sandbox.FileBrowserScreen
import com.solos.app.ui.sandbox.FileBrowserViewModel
import com.solos.app.ui.sandbox.FileItem
import com.solos.app.ui.sandbox.FilePreviewScreen
import com.solos.app.ui.sandbox.RootfsManagementScreen
import com.solos.app.ui.settings.EnvironmentVariablesScreen
import com.solos.app.ui.settings.AppearanceScreen
import com.solos.app.ui.settings.SettingsScreen
import com.solos.app.ui.settings.SystemPermissionsScreen
import com.solos.app.ui.settings.SessionStorageDetailScreen
import com.solos.app.ui.settings.SkillDetailScreen
import com.solos.app.ui.settings.StorageManagementScreen
import com.solos.app.ui.settings.SkillFileViewerScreen
import com.solos.app.ui.settings.UsageStatsScreen
import com.solos.app.ui.settings.SolosSkillsBrowserScreen
import com.solos.app.ui.settings.MountDetailScreen
import com.solos.app.ui.settings.MountedFoldersScreen
import com.solos.app.ui.settings.SharedFolderDetailScreen
import com.solos.app.ui.settings.SharedFoldersScreen
import com.solos.app.ui.settings.SkillsManagementScreen
...
import com.solos.app.ui.settings.LogDetailScreen
import com.solos.app.ui.settings.LogManagementScreen
import com.solos.app.ui.settings.MemoryFileEditScreen
import com.solos.app.ui.settings.MemoryManagementScreen
import com.solos.app.ui.settings.OffloadPermissionScreen
import com.solos.app.ui.settings.ShizukuPermissionScreen
import com.solos.app.sandbox.RootfsManager
import com.solos.app.sandbox.TerminalSession
import com.solos.app.ui.terminal.TerminalScreen
import com.solos.app.ui.onboarding.OnboardingModelSelectionScreen
...
object Routes {
    ...
    const val SOLOS_SKILLS_BROWSER = "solos_skills_browser"
    ...
    /** Chat-files browser: opens FileBrowser rooted at /var/solos for the session. */
...
fun AppNavigation(
    ...
    // T219-5: use the application-scoped singleton from SolosApp so UI
    ...
    // the application-scoped singleton in SolosApp.
    val mountedFoldersStore = remember {
        (context.applicationContext as com.solos.app.SolosApp).mountedFoldersStore
    }
    ...
    LaunchedEffect(Unit) {
        ...
        val hasPendingShare =
            com.solos.app.share.ShareCoordinator.bufferVersion.value > 0
        ...
        val mode = if (
            com.solos.app.diagnostics.HangDetector.shouldForceHomeOnLaunch(context) ||
            com.solos.app.crash.CrashFrequencyDetector.shouldForceHomeOnLaunch(context) ||
            com.solos.app.diagnostics.LaunchCycleBeacon.shouldForceHomeOnLaunch()
        ) 3 else rawMode
    ...
    // Pinned-shortcut cold start: when launched via
     // `solos://session/<id>/<resource-path>`, set the pending HTML
    ...
    NavHost(
        ...
        composable(Routes.BACKUP) {
            com.solos.app.ui.settings.backup.BackupAndRestoreScreen(
                onBack = { navController.safePopBackStack() },
        ...
        composable(
            ...
            val history = remember { com.solos.app.backup.BackupHistory.get(ctx) }
            ...
            if (record == null) {
                ...
                // the screen.
                val vm: com.solos.app.ui.settings.backup.BackupViewModel =
                    androidx.lifecycle.viewmodel.compose.viewModel()
                com.solos.app.ui.settings.backup.BackupHistoryDetailScreen(
                    record = record,
                    onBack = { navController.safePopBackStack() },
                    onRemove = {
                        history.remove(id)
        ...
        composable(
            ...
            val remote = remember(name) {
                com.solos.app.backup.remote.RcloneRemoteStore(ctx).remote(name)
            }
            if (remote == null) {
                ...
                val vm: com.solos.app.ui.settings.backup.BackupViewModel =
                    androidx.lifecycle.viewmodel.compose.viewModel()
                com.solos.app.ui.settings.backup.BackupDestinationBrowseScreen(
                    remote = remote,
                    vm = vm,
                    onBack = { navController.safePopBackStack() },
                )
        ...
        composable(Routes.RESTORE_SERVERS) {
            com.solos.app.ui.settings.backup.RestoreServersScreen(
                onBack = { navController.safePopBackStack() },
                onPickServer = { name ->
                    navController.safeNavigate(
                        "${Routes.RESTORE_BROWSE}/" + android.net.Uri.encode(name),
        ...
        composable(
            ...
            if (remote == null) {
                ...
                com.solos.app.ui.settings.backup.RestoreBrowseScreen(
                    remote = remote,
                    vm = vm,
                    onBack = { navController.safePopBackStack() },
        ...
        composable(Routes.BACKUP_DESTINATIONS) {
            com.solos.app.ui.settings.backup.RcloneDestinationsScreen(
                onBack = { navController.safePopBackStack() },
            )
        }
        ...
        composable(
            ...
            SharedFolderDetailScreen(
                ...
                onBrowseFiles = {
                    ...
                    val hostPath = java.io.File(rootfs.rootfsDir, "var/solos/$folderId")
                    val label = when (folderId) {
                        "shared" -> ctx.getString(com.solos.app.R.string.shared_folder_name_shared)
                        "skills" -> ctx.getString(com.solos.app.R.string.shared_folder_name_skills)
                        "memory" -> ctx.getString(com.solos.app.R.string.shared_folder_name_memory)
                        else -> folderId
                    }
                    FilePreviewHolder.fileBrowserViewModel = FileBrowserViewModel(
                        ...
                        // dirs that back /var/solos/{shared,skills,memory}
                        // resolve, matching how chat-files browse works.
                        linuxRootPath = "/var/solos/$folderId",
        ...
        composable(
            ...
            SessionStorageDetailScreen(
                ...
                onBrowseFiles = { rootPath ->
                    ...
                    // the per-session host dir (filesDir/solos-sessions/<sid>),
                    // whose immediate children (workspace/ attachments/ offloads/
                    // browser/) are exactly the PRoot /var/solos/* subdirs. The
                    // host listing already resolves correctly so we keep rootPath
                    // host-based (no linuxRootPath re-routing — that would redirect
                    // /var/solos to the global/empty placeholder dir). We only pass
                    // displayLinuxPrefix = "/var/solos" so "Copy Absolute Path"
                    // emits the agent-visible /var/solos/workspace/foo.py instead of
                    // the opaque /data/user/0/.../solos-sessions/<sid>/... host path.
                    FilePreviewHolder.fileBrowserViewModel = FileBrowserViewModel(
                        rootPath = java.io.File(rootPath),
                        rootLabel = "Session Files",
                        displayLinuxPrefix = "/var/solos",
                    )
                    navController.safeNavigate(Routes.FILE_BROWSER)
                },
            )
        ...
        composable(
            ...
            val category = com.solos.app.ui.sandbox.MirrorCatalog.categoryFromKey(categoryKey)
                ?: return@composable
            com.solos.app.ui.sandbox.MirrorCategoryDetailScreen(
                category = category,
                onBack = { navController.safePopBackStack() },
            )
        }
        ...
        // Linux root, focused on /var/solos. Matches AIChatView.swift L490:
        //   FileBrowserView(rootPath: dataPath, initialPath: dataPath/var/solos,
        //                   rootLabel: "/")
        // so the user can navigate up out of /var/solos into the broader rootfs.
        composable(
            ...
            val context = androidx.compose.ui.platform.LocalContext.current
            val rootfs = RootfsManager.getInstance(context.applicationContext)
            val varSolos = java.io.File(rootfs.rootfsDir, "var/solos")
            val sessionId = backStackEntry.arguments?.getString("sessionId") ?: return@composable
            val vm = remember(rootfs.rootfsDir.absolutePath, varSolos.absolutePath, sessionId) {
                FileBrowserViewModel(
                    rootPath = rootfs.rootfsDir,
                    initialPath = varSolos.takeIf { it.exists() },
                    rootLabel = "/",
                    // T121: route directory listings through PRootKernel bind
                    // mounts so /var/solos/{skills,memory,shared} resolve to
                    // their backing host dirs (filesDir/solos-global/<subdir>).
                    // Without this the browser walks the rootfs tarball
                    // directly and shows the empty placeholder dirs that ship
                    // inside Alpine's var/solos/ — every subdir reads as
        ...
        composable(Routes.SKILLS) {
            if (skillRepository != null) {
                SkillsManagementScreen(
                    ...
                    onSolosSkillsClick = { navController.safeNavigate(Routes.SOLOS_SKILLS_BROWSER) },
                )
            }
        }
        ...
        composable(Routes.SOLOS_SKILLS_BROWSER) {
            if (skillRepository != null) {
                SolosSkillsBrowserScreen(
                    skillRepository = skillRepository,
                    onBack = { navController.safePopBackStack() },
                )
            }
        ...
        composable(Routes.MCP) {
            if (mcpRepository != null) {
                com.solos.app.ui.settings.MCPIntegrationsScreen(
                    mcpRepository = mcpRepository,
                    onBack = { navController.safePopBackStack() },
                    envVarRepository = envVarRepository,
                )
        ...
        composable(Routes.SOUL) {
            com.solos.app.ui.settings.SoulSettingsScreen(
                onBack = { navController.safePopBackStack() },
            )
        }
        ...
        composable(Routes.SCHEDULED_TASKS) {
            com.solos.app.ui.scheduled.ScheduledTasksScreen(
                onBack = { navController.safePopBackStack() },
                onEditTask = { taskId ->
        ...
        composable(
            ...
            com.solos.app.ui.scheduled.ScheduledTaskRunsScreen(
                taskId = taskId,
                onBack = { navController.safePopBackStack() },
                onOpenSession = { sessionId ->
                    navController.safeNavigate(Routes.chat(sessionId))
        ...
        composable(
            ...
            com.solos.app.ui.scheduled.ScheduledTaskEditScreen(
                taskId = taskId,
                onBack = { navController.safePopBackStack() },
                onOpenSession = { sessionId ->
                    navController.safeNavigate(Routes.chat(sessionId))
