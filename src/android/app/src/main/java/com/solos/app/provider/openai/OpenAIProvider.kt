package com.solos.app.provider.openai
...
import com.solos.app.data.model.LLMMediaAttachment
import com.solos.app.data.model.LLMMessage
import com.solos.app.data.model.LLMModel
import com.solos.app.data.model.LLMResponse
...
import com.solos.app.provider.thinking.ThinkingResolveContext
import com.solos.app.provider.thinking.ThinkingRuleResolver
import com.solos.app.provider.LLMProvider
import com.solos.app.provider.applyUserAgentOverride
...
import com.solos.app.provider.failOnSilentEmptyCompletion
...
class OpenAIProvider private constructor(
     ...
     * user-authored custom rules from [com.solos.app.provider.thinking.ThinkingRuleResolver]'s
     ...
     * the app-level [com.solos.app.data.FastModePrefs] toggle that Codex
    ...
    internal fun resolvedServiceTier(): String? =
        if (supportsPriorityProcessing && com.solos.app.data.FastModePrefs.isEnabled()) {
            "priority"
        } else {
            null
        }
     ...
     * `solos-model-use` can pass provider-specific params our fixed schema never
    ...
    // solos-2026-05-27.log around 13:28 — 3:00 of silence then trip).
    ...
    private val client = OkHttpClient.Builder()
        ...
        .connectionPool(com.solos.app.network.NetworkMonitor.sharedLLMConnectionPool)
    ...
    /**
     * [Solos#191] OpenRouter does NOT enable Anthropic prompt caching
    ...
    /**
     * [T-android-mistral-reasoning-422] (GH Solos#87, iOS 29065ca0)
    ...
    /**
     * [Solos#163] Talking to xAI's own API (api.x.ai), as opposed to a relay
     ...
     *   • Venice.ai (`api.venice.ai`) — [Solos#86] resells deepseek / claude /
    ...
    override suspend fun sendMessageClamped(
        ...
        // solos-model-use (ModelUseOffloadHandler) — get them on
    ...
    private fun rawStreamMessage(
        ...
        val memBefore = com.solos.app.diagnostics.MemorySnapshot.capture()
        val serStartNs = System.nanoTime()
        val bodyStr = try {
            body.toString()
        } catch (t: Throwable) {
            com.solos.app.diagnostics.LargeAllocProbe.report(
                "openai.body.toString", -1, "model=${model.id} messages=${messages.size}",
                memBefore, serStartNs, failure = t,
            )
            throw t
        }
        if (bodyStr.length >= com.solos.app.diagnostics.LargeAllocProbe.NOTABLE_BYTES) {
            com.solos.app.diagnostics.LargeAllocProbe.report(
        ...
        if (!response.isSuccessful) {
            ...
            com.solos.app.logging.AppLogger.error(
                "OpenAIProvider",
            ...
            if (com.solos.app.BuildConfig.DEBUG) {
                com.solos.app.debug.LLMRequestLog.add(
                    com.solos.app.debug.LLMRequestLog.Entry(
                        provider = "openai",
                ...
                run {
                    ...
                    if (delta != null) {
                        ...
                        if (cLen + rcLen + rLen + tcLen > 0 || delta.has("role")) {
                            com.solos.app.logging.AppLogger.debug(
                                "OpenAIProvider",
     ...
     * solos-model-use returned `image_edit_not_supported` for every
    ...
    internal fun buildRequestBody(
        ...
        // Solos#87) — the request-parameter half of that fix was never
        ...
        // [Solos#191] Opt this request into Anthropic prompt caching.
                    ...
                    msg.role == LLMMessage.Role.USER -> {
                        ...
                        for (tr in toolResults) {
                            ...
                            if (trBytes != null && trBytes.isNotEmpty() && supportsImages) {
                                val safeBytes = com.solos.app.provider.ImageBudget
                                    ...
                                    when (part) {
                                        ...
                                        is AgentContentPart.ImageData -> {
                                            if (supportsImages) {
                                                ...
                                                val safeBytes = com.solos.app.provider.ImageBudget.compressUnderBudget(part.data)
    ...
    private suspend fun buildRequest(bodyStr: String): Request {
        ...
        if (isOAuth && !forceChatCompletions) {
            ...
            // Solos UA here: the ChatGPT OAuth backend validates the
    ...
    private fun injectThinkingParams(body: JSONObject, level: ThinkingLevel, maxTokens: Int) {
        ...
        val ctx = ThinkingResolveContext(
            ...
            // [Solos#163] null (catalog silent) must read as false here —
        ...
        // [T-thinking-rules-observability] Design §8 / GH Solos#100: which rule
        // actually won must be inspectable, or a rule layer just replaces one hidden
        // variable with a more complicated one. solos-config exposure is Phase 2.
        com.solos.app.logging.AppLogger.info(
            "Thinking",
    ...
    private fun responsesImageBlock(
        ...
        // T-imgsize: provider-boundary backstop.
        val safeBytes = com.solos.app.provider.ImageBudget.compressUnderBudget(data)
    ...
    internal fun buildResponsesAPIBody(
         ...
         * caller that supplies images this way (solos-model-use's `image_url`
        ...
        when {
            ...
            // GH Solos#87). The gate added alongside injectThinkingParams
        ...
        if (!body.has("service_tier") &&
            com.solos.app.data.FastModePrefs.isEnabled() &&
        ...
        for ((msgIndex, msg) in messages.withIndex()) {
            ...
            if (msg.contentParts.isNotEmpty()) {
                        ...
                        if (imgParts.isNotEmpty()) {
                            ...
                            for (part in msg.contentParts) {
                                when (part) {
                                    ...
                                    else -> com.solos.app.logging.AppLogger.error(
                                        "OpenAIProvider",
                            ...
                            // supplied top-level (solos-model-use / Vision
                ...
                // the solos-model-use path. The Responses API keeps the SAME
                ...
                // solos-model-use's image_url blocks produce. This builder had
    ...
    private fun derivePromptCacheKey(messages: List<LLMMessage>): String {
        for (msg in messages) {
            ...
            if (text.isNotEmpty()) {
                ...
                val hex = digest.joinToString("") { "%02x".format(it) }
                return "solos-${hex.take(32)}"
            }
        }
        return "solos-${java.util.UUID.randomUUID().toString().lowercase()}"
    }
