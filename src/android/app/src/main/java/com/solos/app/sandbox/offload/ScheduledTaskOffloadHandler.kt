...
import com.solos.app.scheduled.ScheduledRepeatMode
import com.solos.app.scheduled.ScheduledTargetMode
import com.solos.app.scheduled.ScheduledTask
import com.solos.app.scheduled.ScheduledTaskManager
...
/**
 * [T-android-scheduled-tasks-full] solos-scheduled — CLI surface for the
 ...
 *
 *   solos-scheduled list
 *   solos-scheduled create --label L --time HH:MM --prompt "..."
 ...
 *                          [--disabled]
 *   solos-scheduled delete --id <taskId>
 *   solos-scheduled enable  --id <taskId>
 *   solos-scheduled disable --id <taskId>
 *   solos-scheduled run     --id <taskId>     (fire immediately, off-schedule)
 *
...
class ScheduledTaskOffloadHandler(private val context: Context) : NativeOffloadHandler {
    ...
    override fun handle(request: NativeOffloadRequest): NativeOffloadResult {
        ...
        return try {
            when (val sub = args.positional.firstOrNull() ?: "list") {
                ...
                "run" -> handleRun(args)
                else -> NativeOffloadResult(2, "solos-scheduled: unknown subcommand '$sub'\n$HELP")
            }
        } catch (e: IllegalArgumentException) {
            NativeOffloadResult(2, "solos-scheduled: ${e.message}")
        } catch (e: Throwable) {
            AppLogger.warning(TAG, "handle failed: ${e.message}")
            NativeOffloadResult(1, "solos-scheduled: ${e.message}")
        }
    }
    ...
    private fun handleDelete(args: OffloadArgs): NativeOffloadResult {
        ...
        if (manager.get(id) == null) return NativeOffloadResult(1, "solos-scheduled: no task with id=$id")
        manager.delete(id)
    ...
    private fun handleRun(args: OffloadArgs): NativeOffloadResult {
        val id = args.get("id") ?: throw IllegalArgumentException("--id required")
        val task = manager.get(id) ?: return NativeOffloadResult(1, "solos-scheduled: no task with id=$id")
        ...
        val sessionId = runBlocking {
            com.solos.app.scheduled.ScheduledAgentRunner.run(context, task)
        }
        val out = JSONObject()
            .put("id", id)
    ...
    companion object {
        ...
        private val HELP = """
            solos-scheduled — manage timed AI tasks (mirrors the in-app Scheduled Tasks).
