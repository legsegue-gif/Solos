package com.solos.app.sandbox.offload
...
import com.solos.app.provider.safeOptString
import com.solos.app.sandbox.NativeOffloadHandler
import com.solos.app.sandbox.NativeOffloadRequest
import com.solos.app.sandbox.NativeOffloadResult
...
/**
 * `solos-model-use` — list, search, and invoke LLM models from Alpine shell.
 * Mirrors iOS ModelUseOffload.m + ModelUseOffloadBridge.swift.
 *
 * Subcommands:
 *   solos-model-use list [--provider <name>] [--modality <m>]
 *   solos-model-use search <query> [--provider <name>] [--modality <m>]
 *   solos-model-use run --model <id_or_name> [--input <path>] [--output <path>]
...
class ModelUseOffloadHandler(
    ...
    override fun handle(request: NativeOffloadRequest): NativeOffloadResult {
        ...
        return try {
            when (val sub = args.positional[0]) {
                ...
                else -> NativeOffloadResult(2, "solos-model-use: unknown subcommand '$sub'\n$HELP")
            }
        } catch (e: Throwable) {
    ...
    private fun cmdSearch(args: OffloadArgs): NativeOffloadResult {
        val query = args.positional.getOrNull(1)
            ?: return NativeOffloadResult(
                2,
                "solos-model-use search: no query. Usage: solos-model-use search <query>\n",
            )
    ...
    private fun cmdRun(args: OffloadArgs, request: NativeOffloadRequest): NativeOffloadResult {
        val modelArg = args.get("model")
            ?: return NativeOffloadResult(2, "--model is required. Usage: solos-model-use run --model <id_or_name>\n")
            ...
            ?: return NativeOffloadResult(
                ...
                JSONObject().put("error", "model_not_found")
                    .put(
                        "message",
                        if (providerFilter != null)
                            "No model '$modelArg' under provider '$providerFilter'. Use 'solos-model-use list' to see available combinations."
                        else
        ...
        if (outputPath != null && !outputPath.startsWith("/") && !outputPath.startsWith("file://")) {
            return NativeOffloadResult(
                ...
                JSONObject().put("error", "invalid_output_path")
                    .put(
                        "message",
                        "--output must be an absolute path (e.g. /var/solos/workspace/out.jpg). " +
                            "Got the relative path '$outputPath', which cannot be resolved because " +
                            "solos-model-use does not inherit the shell's working directory.",
                    )
                    .toString() + "\n",
            )
        }
        ...
        if (requiredModality != null) {
            ...
            return NativeOffloadResult(
                ...
                JSONObject().put("error", "modality_not_supported")
                    .put("message", "Model '${entry.model.displayName}' does not support $requiredModality. Supported modalities: ${supported.joinToString(", ")}. Use 'solos-model-use list' to find a model with the required capability.")
                    .toString() + "\n",
            )
        }
        ...
        val inputText = when {
            args.get("input") != null -> readLinuxPath(args.get("input")!!)
                ?: return NativeOffloadResult(
                    2,
                    "solos-model-use run: cannot read --input '${args.get("input")}'\n",
                )
            else -> ""
        }
        val parsed = try {
                ...
                JSONObject().put("error", "modality_not_supported")
                    .put(
                        ...
                        "Model '${entry.model.displayName}' does not support audio_input, " +
                            ...
                            "Use 'solos-model-use list --modality audio' to find an audio-capable model.",
                    )
                    .toString() + "\n",
            )
        }
        ...
        val instance = providerRepository.instance(entry.providerInstanceId)
            ?: return NativeOffloadResult(2, "solos-model-use run: provider instance not found\n")
        ...
        if (hasAudioInput && provider !is com.solos.app.provider.openai.OpenAIProvider) {
            return NativeOffloadResult(
                2,
        ...
        if (ptSpec.active) {
            val openAI = provider as? com.solos.app.provider.openai.OpenAIProvider
                ?: return NativeOffloadResult(
                    2,
        ...
        (provider as? com.solos.app.provider.openai.OpenAIProvider)?.let { openAI ->
        ...
        if (outputPath != null) {
            ...
            // sessionId is null or the path isn't a session-scoped /var/solos
            ...
            val hostFile = sessionScopedHostFile(outputPath, sessionId)
                ...
                ?: return NativeOffloadResult(
                    2,
                    "solos-model-use run: cannot resolve --output '$outputPath'\n",
                )
            ...
            val attachDir = sessionScopedHostFile("/var/solos/attachments", sessionId)
                ?: PRootKernel.resolveHostPath("/var/solos/attachments")
            if (attachDir != null) {
                attachDir.mkdirs()
                for ((idx, media) in response.mediaAttachments.withIndex()) {
                    ...
                    val responsePath = "/var/solos/attachments/$fileName"
    ...
    private data class ImageGenConfig(
        ...
        val endpointOverride: com.solos.app.data.model.ImageEndpointMode? = null,
    )
    ...
    private fun parseImageGenConfig(inputJson: String): ImageGenConfig {
        ...
        val endpoint = when (rawEndpoint.lowercase()) {
            "auto" -> com.solos.app.data.model.ImageEndpointMode.auto
            "images_generations", "images-generations", "images-gen" ->
                com.solos.app.data.model.ImageEndpointMode.imagesGenerations
            "chat_completions", "chat-completions", "chat" ->
                com.solos.app.data.model.ImageEndpointMode.chatCompletions
            else -> null
        }
    ...
    private fun performRawPassthrough(
        ...
        entry: ModelEntry,
        openAI: com.solos.app.provider.openai.OpenAIProvider,
        systemPrompt: String?,
    ...
    private fun tryImageGenerationRoute(
        entry: ModelEntry,
        instance: com.solos.app.data.model.ProviderInstance,
        provider: com.solos.app.provider.LLMProvider,
        inputJson: String,
        ...
        if (instance.credentialType == com.solos.app.data.model.ProviderCredential.oauth) return null
        ...
        val effectiveMode = when {
            ...
            isPureImageGenerator -> com.solos.app.data.model.ImageEndpointMode.imagesGenerations
            instance.imageEndpointMode == com.solos.app.data.model.ImageEndpointMode.auto &&
        ...
        if (effectiveMode == com.solos.app.data.model.ImageEndpointMode.chatCompletions) return null
        ...
        if (effectiveMode == com.solos.app.data.model.ImageEndpointMode.imagesGenerations) {
            val response = try {
                runBlocking {
        ...
        val response = try {
            runBlocking {
                ...
                providerRepository.setImageEndpointResolved(
                    instance.id, com.solos.app.data.model.ImageEndpointMode.imagesGenerations,
                )
            }
        } catch (e: Throwable) {
            val msg = e.message ?: ""
            val routeMissing = (e is com.solos.app.data.model.LLMError.ProviderError ||
                e is com.solos.app.data.model.LLMError.InvalidApiKey) && looksLikeEndpointMissing(msg)
            if (routeMissing) {
                Log.i(TAG, "[ModelUseRoute] auto probe → route missing (${msg.take(120)}) — caching chat_completions, falling back")
                providerRepository.setImageEndpointResolved(
                    instance.id, com.solos.app.data.model.ImageEndpointMode.chatCompletions,
                )
    ...
    private fun writeImageResult(
        entry: ModelEntry,
        response: com.solos.app.data.model.LLMResponse,
        outputPath: String?,
        outputExt: String,
        endpointUsed: String,
        sessionId: String?,
    ...
    /**
     * [T-android-model-use-session-scoped-write] Resolve a `/var/solos/<sub>/...`
     ...
     * build, so PRootKernel.resolveHostPath("/var/solos/attachments") returns
     ...
     * response reports the abstract `/var/solos/attachments/...` path, which A's
     * shell (mounted to A's dir) can't read. Mirrors iOS da4b6c5d, which routes
     * the write to solosAttachmentsPersistentDir(for: callerSid).
     *
     * Session-scoped subdirs are attachments/offloads/workspace/browser (see
     * buildSessionBindMounts). For those, host dir = filesDir/solos-sessions/
     ...
     * isn't a session-scoped `/var/solos/<sub>` path.
     */
    private fun sessionScopedHostFile(linuxPath: String, sessionId: String?): File? {
        if (sessionId == null) return null
        val m = Regex("^/var/solos/(attachments|offloads|workspace|browser)(/.*)?$").find(linuxPath)
            ?: return null
        val sub = m.groupValues[1]
        val rest = m.groupValues[2].removePrefix("/")
        val base = File(context.filesDir, "solos-sessions/$sessionId/$sub")
    ...
    private fun logModelUseWrite(responsePath: String, hostFile: File, sessionId: String?) {
        Log.i(
            ...
            "[ModelUseWrite] path=$responsePath hostFile=${hostFile.absolutePath} " +
                "sessionId=$sessionId globalAttachMount=${PRootKernel.bindMounts["/var/solos/attachments"]}",
        )
    }
    ...
    private fun entryDict(entry: ModelEntry): JSONObject =
        com.solos.app.offload.ModelUseManager.entryDict(
            entry,
     ...
     *  - `/var/solos/<scope>/<path>` or `/<abs/linux/path>` → via
     *    [PRootKernel.resolveHostPath] (which already handles
     *    `/var/solos/` bind mounts longest-prefix)
    ...
    private fun resolveImageUrl(url: String): LLMMessage.ImagePart {
        ...
        if (url.startsWith("http://") || url.startsWith("https://")) {
            throw ImageInputError(
                "http(s):// image URLs are not supported by solos-model-use. " +
                    "Download first with `shell_execute` (curl/wget) into " +
                    "/var/solos/workspace/, then reference the local path."
            )
        }

        // file:///abs/path → strip prefix, treat as host path; fall back
        // to resolveHostPath so bind-mounted /var/solos/* still works
        // when callers write file:///var/solos/... .
        val linuxPath = when {
            ...
            url.startsWith("/") -> url
                ...
                "Unsupported image_url '$url'. Use a data: URL, file:///host/path, " +
                    "/var/solos/<scope>/<path>, or an absolute Linux path."
            )
        }
    ...
    companion object {
        ...
        private const val USAGE_HINT =
            "To invoke a model, pass `--model <model_id>` to `solos-model-use run`. " +
        ...
        private const val HELP = """solos-model-use — list, search, and invoke LLM models

Usage:
  solos-model-use list [--provider <name>] [--modality <mod>]
  solos-model-use search <query> [--provider <name>] [--modality <mod>]
  solos-model-use run --model <id_or_name> [--provider <label_or_id>]
...
Examples:
  solos-model-use list
  solos-model-use list --modality image_input
  solos-model-use search gemini
  solos-model-use run --model claude-sonnet-4-6 --input /var/solos/workspace/prompt.json
  solos-model-use run --model deepseek/deepseek-v4-flash --input msgs.json   # qualified form
  solos-model-use run --model deepseek-v4-flash --provider deepseek --input msgs.json   # equivalent
  echo 'What is 2+2?' | solos-model-use run --model gpt-4o
  solos-model-use run --model gemini-2.5-flash --system 'You are a poet' \
                      --input msgs.json --output /var/solos/workspace/out.txt
"""
    }
}
