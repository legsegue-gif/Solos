...
/**
 * Root-level confirmation dialog for every solos-config write.
 * Mirrors iOS `ConfigConfirmSheet`. Mounted in [com.solos.app.MainActivity]
...
private fun ConfigConfirmDialog(change: PendingConfigChange) {
    ...
    AlertDialog(
        ...
        dismissButton = {
            SolosTextButton(onClick = { ConfigConfirmationGate.userReject() }) {
                Text("Cancel")
            }
        },
    )
