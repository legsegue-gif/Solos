...
object SoulIcon {
    ...
    /**
     * [T-android-soul-icon-config-images] Turn a `solos-config` value into
     * bitmap bytes.
     *
    ...
    fun classifySource(raw: String): Source {
        ...
        return when {
            v.startsWith("http://", true) || v.startsWith("https://", true) ->
                Source.Unsupported(
                    "remote URLs aren't supported on Android — download the file first, " +
                        "then pass a path like /var/solos/attachments/icon.png",
                )
            v.startsWith("solos://") -> {
                // solos://attachments/x.png -> /var/solos/attachments/x.png
                val rest = v.removePrefix("solos://").trimStart('/')
                if (rest.isEmpty()) Source.Unsupported("empty solos:// path")
                else Source.LinuxPath("/var/solos/$rest")
            }
            v.startsWith("data:") -> {
                val comma = v.indexOf(',')
            ...
            looksLikeBareBase64(v) ->
                ...
                "not an emoji, a data URI, base64, a solos:// resource or a /var/solos path",
            )
        }
    }
     ...
     * model-supplied `/var/solos/attachments/../../../databases/x` would walk
     * straight out of the sandbox.
     */
    val ALLOWED_LINUX_ROOTS = listOf(
        "/var/solos/attachments",
        "/var/solos/workspace",
        "/var/solos/offloads",
        "/var/solos/shared",
        "/var/solos/memory",
