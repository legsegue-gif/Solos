 ...
 * null/blank, we now apply [SolosUserAgent.DEFAULT] instead of leaving
 * the builder UA-less (which lets OkHttp insert its own `okhttp/4.12.0`).
 * The default carries the Solos version so request logs upstream can be
...
fun Request.Builder.applyUserAgentOverride(
    ...
    defaultUserAgent: String? = SolosUserAgent.DEFAULT,
): Request.Builder {
...
/**
 * [T-android-default-ua] Branded User-Agent used by every Solos-originated
 * outbound request that doesn't have a SDK-specific UA requirement.
 *
 * Format mirrors iOS exactly:
 *   `Solos/<version> (Android <release>; <model>)`
 *
 * e.g. `Solos/0.14-preview (Android 13; Pixel 4a)` — same shape as iOS's
 * `Solos/1.10 (iOS 26.5; iPhone)`.
 *
...
object SolosUserAgent {
    val DEFAULT: String by lazy {
        ...
        "Solos/${BuildConfig.VERSION_NAME} (Android $release; $model)"
    }
}
