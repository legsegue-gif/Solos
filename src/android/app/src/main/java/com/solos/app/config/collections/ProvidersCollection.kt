package com.solos.app.config.collections

import com.solos.app.config.ConfigCollection
import com.solos.app.config.ConfigError
import com.solos.app.config.ConfigField
import com.solos.app.config.ConfigRisk
import com.solos.app.config.ConfigSchema
import com.solos.app.config.ConfigValue
import com.solos.app.config.fields.ClosureField
import com.solos.app.config.fields.HiddenField
 ...
 *
 * [T-solos-config-provider-add] Add is OPEN (per user decision: writes
 ...
 *     from a multi-step browser flow that solos-config cannot drive.
 */
class ProvidersCollection(
    ...
    override fun fields(forId: String): List<ConfigField> {
        ...
        return listOf(
            ...
            HiddenField(
                ...
                reason = "OAuth tokens cannot be set via solos-config — use Settings UI",
            ),
        )
    }
        ...
        val resolved = envVars.getValue(key)
            ?: throw ConfigError.InvalidValue(
                "$fieldName references env var \$\$$key, but no such env var exists. " +
                    "Create it first via [Set $key](solos://settings/environments?create_key=$key&create_value=)."
            )
        if (resolved.isEmpty()) {
    ...
    /**
     * [T-android-solosconfig-custom-useragent] Per-provider User-Agent
     * override. Empty string clears the override and falls back to the
     * branded default (Solos/<version> (Android <release>; <model>) — see
     * SolosUserAgent.DEFAULT) at request build time. Cloned from
     ...
     * `read_failed`, thanks to the explicit catch added in T-solos-
    ...
    private fun apiKeyField(id: String): ConfigField =
        ClosureField(
            ...
            reader = {
                throw ConfigError.PermissionDenied(
                    "API keys are never read back via solos-config. To change one, write a new value."
                )
            },
            writer = { v ->
                val s = (v as? ConfigValue.Str)?.value ?: throw ConfigError.TypeMismatch("string")
