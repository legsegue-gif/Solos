...
import com.solos.app.auth.OpenAIOAuthManager
import com.solos.app.auth.OpenRouterOAuthManager
...
import com.solos.app.ui.components.RowLabel
...
fun AddProviderScreen(
    ...
    var selectedVoiceTemplate by remember {
        mutableStateOf<com.solos.app.data.model.VoiceProviderTemplate?>(null)
    }
...
private fun ChooseProviderScreen(
    ...
    onSelectVoiceTemplate: (com.solos.app.data.model.VoiceProviderTemplate) -> Unit = {},
) {
    SettingsScaffold(
        ...
        val templates = com.solos.app.data.model.VoiceProviderTemplate.all
        ...
        SettingsSection(
            ...
            templates.forEachIndexed { index, template ->
                val capabilityRes = when (template.capability) {
                    com.solos.app.data.model.VoiceProviderTemplate.Capability.TTS ->
                        R.string.add_provider_voice_capability_tts
                    com.solos.app.data.model.VoiceProviderTemplate.Capability.ASR ->
                        R.string.add_provider_voice_capability_asr
                    com.solos.app.data.model.VoiceProviderTemplate.Capability.BOTH ->
...
private fun ConfigureProviderScreen(
    ...
    providerRepository: ProviderRepository,
    voiceTemplate: com.solos.app.data.model.VoiceProviderTemplate? = null,
    onBack: () -> Unit,
    onSaved: () -> Unit,
) {
                            ...
                            try {
                                when (providerType) {
                                    ProviderType.kimiCode -> {
                                        ...
                                        val key = com.solos.app.auth.KimiOAuthManager.login(
                                    ...
                                    ProviderType.anthropic -> {
                                        val key = com.solos.app.auth.ClaudeOAuthManager.login(context, pendingInstanceId, providerRepository)
                                        maskedToken = maskOAuthToken(key)
                                    }
                                    ProviderType.openAI -> {
                                    ...
                                    ProviderType.xAI -> {
                                        val key = com.solos.app.auth.XAIOAuthManager.login(context, pendingInstanceId, providerRepository)
                                        maskedToken = maskOAuthToken(key)
                                    }
                                    else -> {
                                ...
                                // signal.
                                errorMessage = if (e is com.solos.app.auth.OAuthNetworkUnreachableException) {
