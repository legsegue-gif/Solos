 ...
 *
 * Routing order:
 *  1. Recognized solos:// deep-link action  → DeepLink (delegated to MainActivity via Intent.ACTION_VIEW)
 *  2. solos://<sandbox path>, file://, or absolute /var/solos|/root path → SandboxFile
...
object ChatLinkResolver {
    ...
    fun resolve(rawUrl: String, sessionId: String? = null, context: Context? = null): ChatLinkAction {
        ...
        // 1. solos:// deep links — only branch out when the URL maps to a known action,
        ...
        if (com.solos.app.ui.browser.BrowserExternalSchemeHandler.shouldHandleExternally(trimmed)) {
     ...
     * Accepts:
     *   solos://attachments/foo.png        → /var/solos/attachments/foo.png
     *   solos:///var/solos/workspace/x.csv → /var/solos/workspace/x.csv (absolute)
     *   file:///path/to/file               → /path/to/file
     *   /var/solos/workspace/x.csv         → resolved via bind mount
    ...
    private fun resolveSandboxFile(
        ...
        return when (scheme) {
            "solos" -> {
                ...
                // `solos://` URLs don't use fragments, so stripping at '#'
                // would truncate filenames like `foo #China.mp4`.
                val stripped = raw.removePrefix("solos://").substringBefore('?')
                // [T-android-solos-url-double-encoding] Try each decode
                // candidate and take the first that exists on disk. See
                // [solosPathCandidates] for why one decode pass isn't enough.
                solosPathCandidates(stripped)
                    .asSequence()
                    .map { candidate ->
                        val linuxPath = if (candidate.startsWith("/")) candidate else "/var/solos/$candidate"
                        lookup(linuxPath)
                    }
                    ...
                    ?: lookup(
                        solosPathCandidates(stripped).first().let {
                            if (it.startsWith("/")) it else "/var/solos/$it"
                        },
                    )
            }
            "file" -> {
    ...
    /**
     * [T-android-solos-url-double-encoding] Decode candidates for the path part
     * of a `solos://` URL, in priority order. Ported from iOS
     * `SolosURLPathDecoding` (T-fix-double-encoding).
     *
     * A correctly-formed solos URL percent-encodes each segment exactly once,
     ...
     * and a web preview of a `solos://` URL renders nothing at all. There is
     ...
     */
    internal fun solosPathCandidates(strippedPath: String): List<String> {
