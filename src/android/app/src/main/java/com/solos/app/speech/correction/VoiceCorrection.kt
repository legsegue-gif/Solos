...
import com.solos.app.shared.TextSegmenter
...
object VoiceCorrection {
    ...
    fun ensureInitialized(context: Context) {
        ...
        val repository = (app as? SolosApp)
        ...
        // SolosApp — AppDatabase.getInstance is already the shared handle.
        val dao = runCatching {
            com.solos.app.data.db.AppDatabase.getInstance(app).chatDao()
        }.getOrNull()
        if (dao != null) {
    ...
    fun buildConversationContext(
        context: Context,
        messages: List<com.solos.app.ui.chat.ChatMessage>,
    ): ConversationContext {
        ensureInitialized(context)
