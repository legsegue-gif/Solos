...
object ReadImageTool {
    ...
    fun definition(): AgentToolDefinition = AgentToolDefinition(
        ...
        parameters = mapOf(
            ...
            "path" to AgentToolParam("string", "Linux path (e.g. /var/solos/attachments/chart.png) or solos:// URL (e.g. solos://attachments/chart.png)"),
     ...
     * (`/var/solos/{attachments,workspace,offloads,browser}/...`) resolve
    ...
    fun execute(argsJson: String, sessionId: String? = null, context: Context? = null): ToolExecutionResult {
        return try {
            ...
            val path = if (rawPath.startsWith("solos://")) {
                "/var/solos/" + java.net.URLDecoder.decode(rawPath.removePrefix("solos://"), "UTF-8")
            } else rawPath
