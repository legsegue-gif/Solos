...
/**
 * Long-press + drag selection gesture for SolosTextKit. Mounted on the
 ...
 */
fun Modifier.solosTextKitSelectionGesture(
 ...
 * [solosTextKitSelectionGesture]. See that function's docs for why mouse
 ...
 * actual drag logic lives in [solosTextKitSelectionGesture] which routes
...
private fun SelectionHandleDot(
    ...
    Popup(
        ...
        // [solosTextKitSelectionGesture] — popups are separate windows. The
 ...
 * [SolosMarkdownTextToolbarHost] (which is driven by Compose's
 ...
 * the older [SolosMarkdownTextToolbar] so the per-button UX stays the
...
data class SelectionToolbarActions(
     ...
     * through Solos TTS. Null hides the button. Mirrors iOS's "Read Selection".
     */
    ...
    Popup(
        ...
        Surface(
            ...
            Row(
                ...
                val labelCopy = androidx.compose.ui.res.stringResource(com.solos.app.R.string.selection_copy)
                val labelAddToInput = androidx.compose.ui.res.stringResource(com.solos.app.R.string.selection_add_to_chat_input)
                val labelCopyFullText = androidx.compose.ui.res.stringResource(com.solos.app.R.string.selection_copy_full_text)
                val labelCopyAsMarkdown = androidx.compose.ui.res.stringResource(com.solos.app.R.string.selection_copy_as_markdown)
                val labelCopyAsRichText = androidx.compose.ui.res.stringResource(com.solos.app.R.string.selection_copy_as_rich_text)
                val labelCopyAsPlainText = androidx.compose.ui.res.stringResource(com.solos.app.R.string.selection_copy_as_plain_text)
                val toastCopiedAsPlainText = androidx.compose.ui.res.stringResource(com.solos.app.R.string.selection_copied_as_plain_text_toast)
                ...
                val labelReadSelection = androidx.compose.ui.res.stringResource(com.solos.app.R.string.selection_read_aloud)
                val labelReadFromStart = androidx.compose.ui.res.stringResource(com.solos.app.R.string.selection_read_from_start)
                val labelReadAloud = androidx.compose.ui.res.stringResource(com.solos.app.R.string.read_aloud_group)
                val toastCopiedAsMarkdown = androidx.compose.ui.res.stringResource(com.solos.app.R.string.selection_copied_as_markdown_toast)
                val toastCopiedAsRichText = androidx.compose.ui.res.stringResource(com.solos.app.R.string.selection_copied_as_rich_text_toast)
                // [T-android-markdown-table-copy-actions] Copy Table / Copy
                // Table Image, when the selected message contains a table.
                //
                // This is the toolbar that ACTUALLY shows for a SolosTextKit
                // selection (a table-cell long-press). The table actions were
                // previously wired only into SolosMarkdownTextToolbar — the
                // Compose-SelectionContainer toolbar that this SolosTextKit
                ...
                val labelCopyTable = androidx.compose.ui.res.stringResource(
                    com.solos.app.R.string.markdown_table_copy_table)
                val labelCopyTableImage = androidx.compose.ui.res.stringResource(
                    com.solos.app.R.string.markdown_table_copy_table_image)
                    ...
                    add(SelectionAction(labelCopy) {
                        ...
                        if (text.isNotEmpty()) {
                            ...
                            toast(context.getString(
                                com.solos.app.R.string.selection_copied_toast, preview(text)))
                        }
                        controller.clearSelection()
                    })
                    ...
                    add(
                        SelectionAction(
                            ...
                            children = listOf(
                                ...
                                // (SolosMarkdownTextToolbar) already did, so the
                ...
                inlineItems.forEachIndexed { index, item ->
                    if (index > 0) SolosToolbarDivider()
                    if (item.children.isEmpty()) {
                        SolosToolbarButton(label = item.label, onClick = item.onClick)
                    } else {
...
@Composable
private fun SolosToolbarDivider() {
    Box(
        modifier = Modifier
...
@Composable
private fun SolosToolbarButton(
    label: String,
