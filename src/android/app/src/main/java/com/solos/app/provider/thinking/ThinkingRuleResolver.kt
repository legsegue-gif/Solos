package com.solos.app.provider.thinking
...
data class ThinkingResolveContext(
    ...
    /**
     * [Solos#163] The catalog affirmatively declares this model has NO effort
...
/**
 * Why a particular wire shape was chosen. Design §8 / GH Solos#100: the resolved
...
object ThinkingRuleResolver {
    ...
    fun builtInRules(ctx: ThinkingResolveContext): List<ThinkingRule> = buildList {
        // Mistral — GH Solos#87 / iOS 4592ca9b / 29065ca0. Total prohibition: the
    ...
    fun anthropicThinkingShape(
        ...
        val adaptive = com.solos.app.provider.anthropic.AnthropicProvider
            .modelUsesAdaptiveThinking(modelId)
        if (level.isEnabled && supportsReasoning != false) {
            if (adaptive) {
                return mapOf(
                    "effort" to com.solos.app.provider.anthropic.AnthropicProvider
                        .thinkingEffort(level),
                )
            }
            val budget = com.solos.app.provider.anthropic.AnthropicProvider
