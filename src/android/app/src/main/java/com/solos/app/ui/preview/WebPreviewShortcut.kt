package com.solos.app.ui.preview
...
/**
 * Helper that pins a `file:///var/solos/...` HTML preview as a launcher
 * shortcut. Clicking the shortcut from the home screen sends a
 * `solos://preview/html?path=...&title=...` deep link back to
 * [MainActivity], which [com.solos.app.deeplink.DeepLinkHandler] parses
 * into [com.solos.app.deeplink.DeepLinkAction.OpenHtmlPreview]; the
...
object WebPreviewShortcut {
    ...
    fun pin(
        ...
        // file:///var/solos/browser/snake.html → /browser/snake.html
        val resourcePath = absPath.removePrefix("/var/solos").let {
        ...
        // solos://session/<sessionId>/<resource-path>?title=<title>
