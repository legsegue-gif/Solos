 ...
 * the `solos-sessions-cli list` offload command). The SELECT shape is
 ...
 * the `solos-sessions-cli search` offload command). Same RawQuery
...
interface ChatDao {
    ...
    // ─── T188: solos-sessions-cli backing queries ─────────────────────────────
     ...
     * table. Backs `solos-sessions-cli search`. SELECT must produce columns
    ...
    /**
     * Paginated message page for `solos-sessions-cli messages --offset --limit`.
    ...
    /** Used by `solos-sessions-cli messages` to surface the total count
