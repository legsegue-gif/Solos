...
import com.solos.app.offload.ShizukuManager
...
fun OffloadPermissionScreen(
    ...
    val configEnabled by com.solos.app.config.SolosConfigPermissionStore.enabled.collectAsState()
    ...
    LaunchedEffect(Unit) {
        ...
        while (true) {
            a11yEnabled = isA11yServiceEnabled(context) || SolosAccessibilityService.getInstance() != null
    ...
    SettingsScaffold(
        ...
        onBack = onBack,
        actions = {
            SolosTextButton(onClick = { showResetConfirm = true }) {
                Text(stringResource(R.string.perm_reset_all))
            }
        },
    ) {
        // T-config: master switch for the solos-config CLI surface.
        SettingsSection(
            header = stringResource(R.string.perm_section_config_tool),
            footer = stringResource(R.string.perm_solos_config_desc),
        ) {
            SettingsSwitchRow(
                title = stringResource(R.string.perm_allow_solos_config),
                checked = configEnabled,
                onCheckedChange = {
                    com.solos.app.config.SolosConfigPermissionStore.setEnabled(it)
                },
                showDivider = false,
            )
        }
        ...
        if (a11yRestricted) {
            SettingsSection(
                ...
                SettingsRow(
                    ...
                    // Lands on Solos' own App info page, where "Allow
        ...
        AlertDialog(
            ...
            confirmButton = {
                SolosTextButton(onClick = {
                    ...
                    com.solos.app.config.SolosConfigPermissionStore.setEnabled(true)
                    AppLogger.info("PermissionsScreen", "user confirmed Reset All — all tool permissions cleared, solos-config switch reset to default")
            ...
            dismissButton = {
                SolosTextButton(onClick = { showResetConfirm = false }) {
...
private fun AgentPolicyRow(
    ...
    Box {
        ...
        SolosMenu(expanded = expanded, onDismissRequest = { expanded = false }, alignEnd = true) {
...
private fun isA11yServiceEnabled(context: Context): Boolean {
    val expected = "${context.packageName}/${SolosAccessibilityService::class.java.name}"
...
/**
 * [T-android-restricted-settings] Open Solos' own App info page — "Allow
