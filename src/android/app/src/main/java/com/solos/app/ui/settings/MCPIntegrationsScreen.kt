...
import com.solos.app.ui.components.DialogTextField
...
fun MCPIntegrationsScreen(
    ...
    envVarRepository: com.solos.app.data.repository.EnvVarRepository? = null,
) {
    val servers by mcpRepository.servers.collectAsState()

    // [T-android-mcp-list-reload-on-appear] MCPRepository reads servers.json
    // only in init() (app launch). A server the agent writes via solos-mcp-cli
    ...
    if (deleteId != null) {
        ...
        AlertDialog(
            ...
            dismissButton = {
                SolosTextButton(onClick = { deleteId = null }) { Text(stringResource(R.string.cancel)) }
            },
        )
    }
}
...
private fun MCPAddSheet(
    mcpRepository: MCPRepository,
    envVarRepository: com.solos.app.data.repository.EnvVarRepository?,
...
private fun MCPFormTab(
    ...
    val appEnvEntries by (envVarRepository?.entries
        ?.collectAsState() ?: remember { mutableStateOf(emptyList<com.solos.app.data.repository.EnvVarRepository.EnvVarEntry>()) })
    ...
    var oauthClientSecret by remember {
        mutableStateOf(
            editServer?.id?.let {
                com.solos.app.mcp.oauth.MCPOAuthStore.clientSecret(context, it)
            } ?: "",
        )
    }
    ...
    var oauthAuthorized by remember {
        mutableStateOf(
            editServer?.id?.let {
                com.solos.app.mcp.oauth.MCPOAuthStore.isAuthorized(context, it)
            } ?: false,
        )
    }
    var oauthBusy by remember { mutableStateOf(false) }

    fun currentOAuthConfig(): com.solos.app.mcp.oauth.MCPOAuthConfig? {
        val cfg = com.solos.app.mcp.oauth.MCPOAuthConfig(
    ...
    Column(verticalArrangement = Arrangement.spacedBy(10.dp)) {
                    ...
                    if (oauthAuthorized) {
                        SolosTextButton(
                            ...
                            onClick = {
                                val sid = name.trim()
                                if (sid.isNotEmpty()) {
                                    com.solos.app.mcp.oauth.MCPOAuthStore.signOut(context, sid)
                        ...
                        onClick = {
                            ...
                            when {
                                ...
                                cfg == null ->
                                    ...
                                    com.solos.app.mcp.oauth.MCPOAuthStore.setClientSecret(
                                        context, sid, oauthClientSecret.trim().ifBlank { null },
                                    )
                                    oauthBusy = true
                                    oauthScope.launch {
                                        val result = com.solos.app.mcp.oauth.MCPOAuthController(context)
                                            .authorize(sid, url.trim(), cfg)
                                        oauthBusy = false
                                        when (result) {
                                            is com.solos.app.mcp.oauth.MCPOAuthController.Result.Success -> {
                                            ...
                                            is com.solos.app.mcp.oauth.MCPOAuthController.Result.Cancelled -> {}
                                            is com.solos.app.mcp.oauth.MCPOAuthController.Result.Failed -> {
        ...
        Row(modifier = Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) {
            SolosTextButton(onClick = {
                val server = buildServerFromForm() ?: return@SolosTextButton
        ...
        Row(modifier = Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) {
            ...
            if (isEdit) {
                SolosTextButton(onClick = onRequestDelete) {
                    Text(stringResource(R.string.delete), color = MaterialTheme.colorScheme.error)
                }
            }
            Spacer(Modifier.weight(1f))
            SolosTextButton(onClick = onDone) { Text(stringResource(R.string.cancel)) }
            SolosTextButton(onClick = {
                val server = buildServerFromForm() ?: return@SolosTextButton
    ...
    if (showClearEnvConfirm) {
        AlertDialog(
            ...
            dismissButton = {
                SolosTextButton(onClick = { showClearEnvConfirm = false }) {
