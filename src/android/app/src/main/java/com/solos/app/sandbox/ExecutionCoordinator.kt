...
object ExecutionCoordinator {
    ...
    private suspend fun getOrCreateShell(sessionId: String): PersistentShell {
        ...
        if (existing != null && existing.isAlive) {
            Log.w(TAG, "[diag] reuse existing shell for sessionId=$sessionId attachmentsMount=${existing.debugBindMount("/var/solos/attachments")}")
            return existing
        }
        ...
        return globalLock.withLock {
            ...
            if (recheck != null && recheck.isAlive) {
                Log.w(TAG, "[diag] reuse existing shell (post-lock) for sessionId=$sessionId attachmentsMount=${recheck.debugBindMount("/var/solos/attachments")}")
            ...
            Log.w(TAG, "[diag] new shell created sessionId=$sessionId attachmentsMount=${bindMounts["/var/solos/attachments"]}")
            shell
        }
    }
    ...
    private fun buildSessionBindMounts(sessionId: String): Map<String, String> {
        ...
        val prevAttachments = PRootKernel.bindMounts["/var/solos/attachments"]

        // Session-specific directories
        val sessionBase = File(filesDir, "solos-sessions/$sessionId")
        listOf("attachments", "offloads", "workspace", "browser").forEach { subdir ->
            val hostDir = File(sessionBase, subdir).also { it.mkdirs() }
            val linuxPath = "/var/solos/$subdir"
        ...
        Log.w(TAG, "[diag] buildSessionBindMounts sessionId=$sessionId " +
            "attachments: prev=$prevAttachments new=${mounts["/var/solos/attachments"]}")
        ...
        // shell that runs solos-mcp-cli — /var/solos/mcp-servers/servers.json
        // then resolves to the empty rootfs placeholder and `solos-mcp-cli list`
        ...
        // below.
        val globalBase = File(filesDir, "solos-global")
        ...
        // is invisible to the shell — `ls /var/solos/mounts/<name>/` then shows
        ...
        PRootKernel.mountedFoldersStore?.entries?.value?.forEach { entry ->
            ...
            val linuxPath = "/var/solos/mounts/${entry.name}"
