...
Solos ships a full Linux sandbox inside the app, so a first build is not just
...
|---|---|---|
| `deps/ish` | [Solos/ish-arm64](https://github.com/legsegue-gif/solos-ish) | iOS sandbox kernel |
| `deps/proot` | [Solos/proot](https://github.com/legsegue-gif/solos-proot) | Android sandbox |
...
Select the **Solos** scheme and build. For a device build, set your own team
...
```sh
xcodebuild -project src/ios/Solos.xcodeproj -scheme Solos \
...
`Solos` (app), `SolosShare` (share extension), `AgentWidgetExtension`,
`SolosFileProvider`, plus `SolosTests` / `SolosUITests`.
...
Solos is **GPLv3** because it links iSH (GPLv3) and PRoot (GPLv2). If you
