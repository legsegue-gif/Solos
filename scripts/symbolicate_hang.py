...
Resolves Solos frames to source-line via `atos` against the most recent
...
def _find_solos_dsym(prefer_archive_after: datetime | None = None) -> Path | None:
    """Return the most recent Solos.app.dSYM."""
    ...
    for date_dir in ARCHIVES_ROOT.iterdir():
        if not date_dir.is_dir():
            continue
        for archive in date_dir.glob("Solos*.xcarchive"):
            dsym = archive / "dSYMs" / "Solos.app.dSYM"
            if dsym.exists():
...
def _atos_batch(
    ...
    if not addresses:
        return []
    binary = dsym / "Contents/Resources/DWARF/Solos"
...
def _symbolicate_event(event: dict[str, Any], dsym: Path | None) -> str:
    ...
    # Group Solos frames for batch atos call.
    solos_idx: list[int] = []
    solos_offsets: list[int] = []
    for i, frame in enumerate(frames):
        if frame.get("image") == "Solos":
            ...
            try:
                ...
                continue
            solos_idx.append(i)
            solos_offsets.append(ATOS_LOAD_ADDR + off)

    solos_resolved: dict[int, str] = {}
    if dsym is not None and solos_offsets:
        resolved = _atos_batch(dsym, solos_offsets)
        for idx, sym in zip(solos_idx, resolved):
            solos_resolved[idx] = sym
    ...
    for i, frame in enumerate(frames):
        ...
        if image == "Solos" and i in solos_resolved:
            line += f"\n      {solos_resolved[i]}"
        elif symbol:
...
def main() -> int:
    ...
    if args.dsym:
        ...
        dsym = _find_solos_dsym()
        if dsym is None:
            _eprint("WARN: no Solos.app.dSYM found under ~/Library/Developer/Xcode/Archives;")
            _eprint("      Solos frames will not be symbolicated.")
