...
TMP="$OUT.tmp"
{
  ...
  emit_const "clientPythonB64"   "$SKILL_DIR/examples/solos_rpc.py"
  emit_const "clientNodeB64"     "$SKILL_DIR/examples/solos_rpc.mjs"
  emit_const "clientBashB64"     "$SKILL_DIR/examples/solos_rpc.sh"
  echo '}'
  echo '#endif'
} > "$TMP"
