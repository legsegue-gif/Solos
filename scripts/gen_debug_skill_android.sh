...
# X-Solos-Token header, NOT the iOS protocol-v1 encrypted envelope, so shipping
...
cat > "$OUT_DIR/examples/solos_rpc_android.py" <<'PYCLIENT'
#!/usr/bin/env python3
"""Solos ANDROID debug server client (stdlib only).

Usage:
    python3 solos_rpc_android.py [--host HOST:PORT] [--token TOKEN] <method> [params-json]

Examples:
    adb forward tcp:5321 tcp:5321
    python3 solos_rpc_android.py debug.appInfo '{}'
    python3 solos_rpc_android.py --host 10.0.0.5:5321 --token "$TOK" debug.logs.list '{}'
...
Transport notes (Android differs from iOS!):
  ...
  - Auth: loopback (via `adb forward`) needs no token. NON-loopback (LAN)
    clients must send the per-install device token:
        adb shell run-as com.solos.app cat files/debug_server_token
    Passed as X-Solos-Token (Authorization: Bearer also accepted).
    Or set SOLOS_DEBUG_TOKEN in the environment.
"""
    ...
    try:
        ...
        if e.code == 401:
            raise SystemExit(
                ...
                "  adb shell run-as com.solos.app cat files/debug_server_token\n"
                "then pass --token / SOLOS_DEBUG_TOKEN. (Over `adb forward` no token is needed.)\n"
...
def main() -> None:
    args = sys.argv[1:]
    host = "localhost:5321"
    token = os.environ.get("SOLOS_DEBUG_TOKEN")
...
TOK=$(adb shell run-as com.solos.app cat files/debug_server_token)
...
curl localhost:5321/skill/examples/python > solos_rpc_android.py
```
CURLDOC
