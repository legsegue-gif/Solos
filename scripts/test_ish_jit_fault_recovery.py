...
Requires solos_rpc.py from the device: curl -o solos_rpc.py <host>/skill/examples/python
"""
...
def rpc(host, method, params):
    out = subprocess.run(
        [sys.executable, "solos_rpc.py", "--host", host, method, json.dumps(params)],
...
def main():
    ...
    if not os.path.exists("solos_rpc.py"):
        print("❌ solos_rpc.py not found in cwd.\n"
              f"   curl -o solos_rpc.py http://{args.host}/skill/examples/python")
        return 1
